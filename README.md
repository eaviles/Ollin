...
- The `packCircles`/`relaxCircles` circle packing implements the two classic grow-to-touch and front-relaxation forms from the technique: each circle grown to the largest radius that clears its neighbors (the greedy gap-filling pack popularized in Jared Tarbell's generative work), and overlapping circles separated by repeated pairwise pushes. Written from the technique.
