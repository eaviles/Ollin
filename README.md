...
<!--
  ...
  only gets made by someone solving a real problem:
    ...
    - eighteen on-device trackers and a custom iPhone capture app
...
| [**Text & color**](Docs/Drawing/Text.md) | Bitmap, outline, and single-line plotter fonts through one `drawText`, text in [any script](Docs/Drawing/Text.md#scripts) (right-to-left, Devanagari, Thai, Japanese, emoji), set [down the page in columns](Docs/Drawing/Text.md#vertical) as readily as across it, [justified](Docs/Drawing/Text.md#justify) to a measure with its [punctuation hung](Docs/Drawing/Text.md#hanging) outside the margin, run [along a path](Docs/Drawing/Text.md#onpath), moved along a [variable font's](Docs/Drawing/Text.md#variable) own axes, or taken as vector `Shape`s that also [extrude into 3D solids](Docs/3D/3D.md#solid-type). Then [OKLab mixing](Docs/Drawing/Color.md), [spectral color and paint mixing](Docs/Drawing/Spectrum.md) (pigments that meet in green, thin-film and diffraction filters), palette import and extraction, colormaps, gradients, [color vision simulation](Docs/Helpers/Accessibility.md) so you can see a palette as somebody colorblind does, `describe` so a piece can say what it shows to a screen reader, [wide-gamut and HDR output](Docs/Drawing/ColorOutput.md) (Display P3 on screen, highlights brighter than white, HDR10 video, and stills that keep those highlights in a gain map), and [images](Docs/Drawing/Images.md) with pixel access, dithering, [pixel sorting](Docs/Drawing/PixelSorting.md), [seam carving](Docs/Drawing/SeamCarving.md), [halftone screens](Docs/Drawing/Halftone.md), [glyph mosaics](Docs/Drawing/GlyphMosaic.md), [photo mosaics](Docs/Drawing/PhotoMosaic.md), and [autostereograms](Docs/Drawing/Autostereogram.md) |
