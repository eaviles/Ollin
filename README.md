...
Coming from p5.js or Processing? [Appendix C of the Guide](Guide/C-ComingFromP5.md) maps the API you already know onto Ollin. From openFrameworks or OPENRNDR, [Appendix E](Guide/E-ComingFromOpenFrameworksAndOPENRNDR.md) covers the big moves.
...
dependencies: [
    ...
    .package(url: "https://github.com/eaviles/Ollin.git", .upToNextMinor(from: "0.10.0"))
],
targets: [
    .executableTarget(
        name: "MySketch",
...
The `platforms` entry matters: without it SwiftPM targets its oldest macOS default, and the build fails against Ollin's macOS 26 floor. `.upToNextMinor(from: "0.10.0")` accepts 0.10.x fixes and keeps out 0.11.0, where APIs may change. Read the [changelog](CHANGELOG.md) before raising that bound.
...
- **Or running in the system.** The project generator wraps a sketch as a signed [Mac app](Docs/Output/App.md), [screen saver](Docs/Output/ScreenSaver.md), [desktop wallpaper](Docs/Output/Wallpaper.md), [menu-bar piece](Docs/Output/MenuBar.md), or [desktop widget](Docs/Output/Widget.md). Each comes with a script to build, sign, and install it; the sketch stays ordinary Swift. `ollin phone` runs it on a paired [iPhone or iPad](Docs/Tools/OnThePhone.md), reinstalling on save with its clock and parameters carried across. You can also generate an [iOS app of your own](Docs/Tools/OnThePhone.md#in-an-app-of-your-own).
