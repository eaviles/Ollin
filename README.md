...
- [Parameters](Docs/Helpers/Parameters.md), the `@Param` inspector controls, and [Tuning toward a look](Docs/Helpers/Tuning.md), a phrase that moves them
...
- **It's a tool for making art.** The AI helped build the framework itself. It plays no part in the work you make with Ollin, with one exception you ask for: the inspector can move the parameters you declared toward a look you describe, on your Mac's own model, inside the ranges you set, with every move named and undoable. It writes values, never code or pictures, and [Tuning toward a look](Docs/Helpers/Tuning.md) says exactly what it is told. No AI-generated artwork ships in Ollin, and every bundled asset is credited, licensed human work. The vision examples that use published ML models download them from their own sources, and each model is named and credited in [`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md). Ollin is not a generative-art model.
