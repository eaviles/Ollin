...
- [Layered effects](Docs/Effects.md) - `renderTarget` / `withTarget` to draw into off-screen layers, then `filtered` / `postProcess` to run GPU filters (blur, bloom) over them, composited back with blend modes — glow, soft backdrops, post-processing.
