...
| **Plays in your rig** | [Syphon](Docs/Integration/Syphon.md) out and in, a system-wide [virtual camera](Docs/Integration/VirtualCamera.md) any webcam app can read, [screen and window capture](Docs/Integration/ScreenCapture.md) that takes any app on the Mac as live material whether or not it cooperates, [DMX lighting](Docs/Integration/DMX.md) driven from `draw()` over Art-Net or sACN (and a console driving the sketch back), [LED strips and matrices](Docs/Integration/DMX.md#ledmap) lit from the canvas's own pixels sampled on the GPU each frame, [show lasers](Docs/Integration/Laser.md) drawn from `draw()` as an optimized point stream over a network DAC or an ILDA file, [video playback](Docs/Video/Video.md) as live GPU textures, [slit-scan time displacement](Docs/Video/SlitScan.md) over any frame history, and [haptics](Docs/Integration/Haptics.md) so a sketch also puts a designed pattern of taps and hums under the hand |
