...
- **Output that leaves the screen.** Headless PNG stills and deterministic sequences, MP4 and GIF straight from the CLI, vector SVG and PDF with optional hatched fills for pen plotters, [print separations](Docs/Output/PrintSeparations.md) with a real ink model for riso and screen printing, [fabrication files](Docs/Output/Fabrication.md) (STL, OBJ, 3MF) so a generated mesh can be 3D-printed at a real size, and [spatial output](Docs/Output/Spatial.md): USDZ so a 3D piece opens in Quick Look, sends in a message, and stands on a real table through AR, or spatial video so its motion plays in depth on a headset.
...
| [**3D**](Docs/3D/3D.md) (opt-in) | Orbit and [cinematic cameras](Docs/3D/Camera.md), a solid-primitive catalog plus meshes from file (OBJ, glTF, USDZ) or [whole scenes](Docs/3D/Scenes.md) (glTF, USD) imported with their named nodes, authored cameras, lights, and keyframe animations played on the sketch's clock (or, with [`ollin new --from-scene`](Docs/Tools/SceneImport.md), written out as the placement calls that draw them), point clouds, [instanced meshes](Docs/3D/Instancing.md) (one mesh drawn thousands of times in one call, placed per copy on the GPU, even from a compute kernel), stylized and physically based materials including [glass](Docs/3D/3D.md#glass) (transmission, refraction, absorption), the full texture-map set through [projected decals](Docs/3D/3D.md#decals) (pictures stamped across whatever surfaces a box touches) and [detail maps](Docs/3D/3D.md#detail-maps) (texture that survives a close look), area lights (glowing panels, disks, and tubes), light shaping with IES photometric profiles and projected cookies, image-based lighting from bundled HDRIs or a procedural sky (which can carry raymarched volumetric cloudscapes), soft shadows, ray-traced reflections (and refractions) and [real-time global illumination](Docs/3D/3D.md#global-illumination) (bounce light through a live-retraced probe field) on RT GPUs, [temporal anti-aliasing](Docs/3D/3D.md#temporal-antialiasing) (jittered accumulation refining edges past MSAA, on any GPU), cinematic [motion blur](Docs/3D/3D.md#motion-blur) (per-object and camera streaks with a photographic shutter dial, deterministic in exports), and [atmosphere](Docs/3D/Atmosphere.md): height fog, aerial perspective (far geometry veiling blue toward the sky's own sun), and volumetric light beams. 2D drawing sits inside the depth buffer via [depth compositing](Docs/3D/DepthCompositing.md), and [Combining 3D features](Docs/3D/Combining.md) covers how the pieces stack. A 2D sketch never pays for any of it |
...
| [**Perception**](Docs/Vision/Vision.md) | Eighteen on-device trackers over the Mac's camera or any [video](Docs/Video/Video.md): face, hand, and body pose in 2D and 3D, segmentation (automatic or from a click), contours, optical flow, OCR, saliency, typed phrases scored against the picture, and custom Core ML models. An [iPhone](Docs/3D/Phone.md) extends that with LiDAR depth clouds, the reconstructed room as a labelled surface with the flat planes in it, the room's own light, ARKit body and face capture, hand skeletons and readable text lifted to metric 3D, and device motion over USB, live or from [recorded clips](Docs/3D/Record3D.md), all as metric [RGBD frames](Docs/3D/RGBD.md) |
| [**Sound & control**](Docs/Helpers/Audio.md) | FFT analysis with band and beat detection, [listening](Docs/Helpers/Listening.md) for words and for named sounds (speech as a live caption, a clap or a bark as a trigger), a [synthesizer](Docs/Helpers/Synthesis.md) the sketch itself plays (voice presets over a shaped and filtered oscillator, delay and reverb), [algorithmic composition](Docs/Helpers/Composition.md) that works out what to play (Euclidean rhythms, scales and chords, arpeggios, Markov sequences), [sonification](Docs/Helpers/Sonification.md) that reads a table column or a terrain profile out as notes, [MIDI](Docs/Integration/MIDI.md) and [OSC](Docs/Integration/OSC.md) in and out (bindable to `@Param` knobs), [game controllers](Docs/Integration/Controller.md) read in `draw()` (sticks, triggers, buttons, and motion and a touchpad where the hardware has them, several players at once), and tempo sync so a set runs on the DJ's clock |
| [**Data as material**](Docs/Helpers/Data.md) | `loadTable` reads a CSV or TSV with typed reads by column name, `loadJSON` reads a document you reach through by name and index, and [SVG import](Docs/Drawing/SVG.md) brings in vector artwork, so a drawing can be driven by a file instead of hard-coded numbers |
| [**Text & color**](Docs/Drawing/Text.md) | Bitmap, outline, and single-line plotter fonts through one `drawText`, text in [any script](Docs/Drawing/Text.md#scripts) (right-to-left, Devanagari, Thai, Japanese, emoji), text as vector `Shape`s, [OKLab mixing](Docs/Drawing/Color.md), palette import and extraction, colormaps, gradients, [color vision simulation](Docs/Helpers/Accessibility.md) so you can see a palette as somebody colorblind does, `describe` so a piece can say what it shows to a screen reader, [wide-gamut and HDR output](Docs/Drawing/ColorOutput.md) (Display P3 on screen, highlights brighter than white, HDR10 video, and stills that keep those highlights in a gain map), and [images](Docs/Drawing/Images.md) with pixel access, dithering, [pixel sorting](Docs/Drawing/PixelSorting.md), [halftone screens](Docs/Drawing/Halftone.md), and [glyph mosaics](Docs/Drawing/GlyphMosaic.md) |
| **Plays in your rig** | [Syphon](Docs/Integration/Syphon.md) out and in, a system-wide [virtual camera](Docs/Integration/VirtualCamera.md) any webcam app can read, [screen and window capture](Docs/Integration/ScreenCapture.md) that takes any app on the Mac as live material whether or not it cooperates, [DMX lighting](Docs/Integration/DMX.md) driven from `draw()` over Art-Net or sACN (and a console driving the sketch back), [LED strips and matrices](Docs/Integration/DMX.md#ledmap) lit from the canvas's own pixels sampled on the GPU each frame, [video playback](Docs/Video/Video.md) as live GPU textures, and [slit-scan time displacement](Docs/Video/SlitScan.md) over any frame history |
