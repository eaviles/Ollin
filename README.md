...
- [3D](Docs/3D.md) - opt into a 3D camera and depth buffer: orbit a `Camera3D` (perspective or orthographic) and draw `PointCloud`s as instanced disc splats, including a live webcam depth cloud (the Mac-side preview of the iPhone LiDAR cloud to come).
- [Record3D](Docs/Record3D.md) - `import OllinRecord3D` to open an RGBD clip recorded by the Record3D iOS app and unproject its color-plus-depth frames into a 3D point cloud — an iPhone scan orbited on the Mac, from a file (the device-free first step toward the live iPhone sensor stream).
...
- Record3D RGBD recordings (`OllinRecord3D`) open `.r3d` clips captured by [Record3D](https://record3d.app) (Marek Šimoník), an ARKit color-plus-depth recorder, and unproject them into 3D point clouds. The format is read clean-room from its public structure with Apple-native frameworks only (`Compression` for LZFSE depth, ImageIO for JPEG color, a minimal ZIP read); the `record3d` library that documents it is LGPL-2.1, so it's a product reference, never copied or vendored. The kind of world-facing depth feed an Intel RealSense once gave openFrameworks, by way of an iPhone.
