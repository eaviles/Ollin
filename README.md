...
- **Shapes that merge.** SDF combinators: smooth union, subtract, morph, machined chamfer/stairs joints, and domain mirror/tile/radial via `drawSDF` or a scoped `smoothUnion { }` block, in 2D and in raymarched 3D with shadows; the 3D side sculpts too, with twist, bend, surface displacement, and free-point armature strokes.
