...
- [`Replay`](./Replay.md) - record a run's seed, clock, inputs, and knob moves as a take (`--record-take`), play it back exactly (`--replay`, with transport keys), scrub it, and re-render the performance through any export flag
- [`Automation`](./Automation.md) - keyframed parameters: a knob's values written down over time (`automate`), carried by named or Bezier curves, looped and played at any speed, read from a file (`--automation`) and rendered exactly by any export
