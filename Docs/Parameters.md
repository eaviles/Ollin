#### <sup>[Ollin](../README.md) → [Documentation](./README.md) → `Parameters`</sup>
...
Each incoming value is mapped into the parameter's range and assigned, and a bound knob updates on its own as messages arrive. The inspector slider, the binding, and plain assignment in code all drive the same value; whichever moved most recently wins. The `from:` input ranges and the rest of the details are on the [OSC](./OSC.md#binding-to-a-param) and [MIDI](./MIDI.md#binding-to-a-param) pages.
