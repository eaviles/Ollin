...
`mesh(...)` emits a solid terrain. It is a `width × depth` grid centered on the origin in the ground plane, with heights lifted to `height · value` on +y, smooth normals, and `0…1` UVs. Because UVs map the sheet in plan view, a texture built by walking each sample's height through a [`Ramp`](../Drawing/Color.md) paints altitude bands, and that is one call: `image(_ ramp:in:)` paints each sample through the ramp (the optional `range` maps a height span onto it), and `coloredMesh(width:depth:height:_:in:)` hands back the mesh already wearing it. Build it when the field changes, not per frame. The `3D/Geometry/Terrain` example uses exactly this. `image()` is the heightmap as pixels, ready to inspect, [dither](../Drawing/Color.md#dithering), or feed the image form of [`isolines`](Isolines.md). The field form draws a contour map directly:
