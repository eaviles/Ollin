...
**Weighted-Voronoi stippling** places a fixed budget of dots so their local density reproduces a picture's tone: shadow packs them tight, highlight spreads them out, and from a small distance the scatter *is* the picture. It's the classic hand-stippled illustration look, and because the output is plain points it feeds anything: dots, marks, [Voronoi cells](../Drawing/Voronoi.md), and the pen-plotter and [SVG](../Output/Export.md) paths.
...
The same engine over any field: the closure is sampled in canvas coordinates once up front (0 = keep out, higher = denser), so noise, a distance falloff, or pure math all work.
...
- **Compute once, hold the points.** The iteration sweeps every pixel of a working grid `iterations` times; it's `setup()` work, not `draw()` work. Animate dot size or color, not the placement.
- **`iterations` trades grain for calm.** 40 (the default) is visually converged; a handful keeps more of the initial scatter's texture.
- **Size dots by darkness for extra tone.** Density carries the tone by itself, but the classic stippling move is to also scale each dot a little by the darkness under it (sample the image at the dot).
- **No ink means no dots.** An all-white or fully transparent image, an all-zero density, or a texture-backed `Image` with no CPU pixels (call `snapshot()` first) all yield an empty array.
