...
The edges can come from anywhere: a hand-written list, a [Delaunay triangulation](../Drawing/Voronoi.md), a [maze](../Drawing/Tiling.md), or a word-adjacency map. `idealDistance` is the spacing constant `k` that every force is measured against. It defaults to `√(area / count)`. Settled edges come out near `k` only in a small or dense graph. In a large sparse web, the combined repulsion of all the nodes compresses the spacing below `k`. For that reason, treat `k` as a size control rather than an exact length. Raise it to open the web out, and lower it to tighten it. A per-edge `weight`, set through `layout.edges` or `connect`, multiplies that edge's pull, so its two ends settle closer together than the rest. An edge is a `ForceLayout.Edge(_:_:weight:)`, its two node indices and that weight.
