...
The simplest systems that turn chaotic: a one-dimensional map `x' = f(x, r)` iterated over and over, with a dial. At small `r` the orbit settles to a point; turn the dial and it forks into a 2-cycle, then 4, then 8, faster and faster, until it never repeats at all. `IteratedMap` holds the rule and answers the three classic questions about it: what one orbit does (`orbit`, `cobweb`), where orbits settle across every dial position (`bifurcation`, the fig-tree diagram), and whether a position is truly chaotic (`lyapunovExponent`).
...
Each family carries its canonical sweep window (`parameterRange`), the interval the orbit lives in (`valueRange`, the diagram's vertical axis), and a default `start`. All three are ordinary `var`s; retune them freely.
...
The **cobweb** is the classic way to *watch* one orbit. `cobweb(at:steps:from:)` returns the staircase polyline in map space (both axes spanning `valueRange`): from the floor up to the curve, across to the diagonal, up to the curve again. Draw it over `graph(at:)` (the rule's curve) and the diagonal:
...
A converging orbit spirals into the crossing; a 2-cycle traces a square; chaos scribbles the whole box.
...
- **`drawBifurcation(_:over:in:columns:perColumn:settle:)`** draws the raw settled values as dots in the current `fill` at the current `pointSize`, mapped into `rect` (default: the whole canvas). A translucent fill shades the dense regions darker on its own, the dots export to SVG/PDF as real geometry (the plotter path), and since the diagram never changes frame to frame, heavy ones belong in a retained batch. The data form behind it is `bifurcation(over:columns:perColumn:settle:) -> [Vector2]`, each point `(r, x)`.
- **`bifurcationImage(width:height:over:samplesPerColumn:settle:)`** bins one long orbit per pixel column and tones the counts with a single global log factor: periodic orbits print as crisp dark lines, chaotic bands wash gray, ink on white. Render it once in `setup()` and `drawImage` it.

Both sample the sweep at column *centers*, never the range's exact endpoints (some families are numerically degenerate exactly there; see the notes). Zooming is just a narrower `over:` range. Try `3.82...3.87`: the period-3 window contains its own complete doubling cascade, the whole diagram again in miniature.
...
The average of `ln |f′(x)|` along the orbit: negative where nearby orbits converge (a stable cycle), positive where they separate exponentially, which is the working definition of chaos. Traced across the sweep window it dips below zero at every periodic window and kisses zero at every fork; for the logistic map it reaches exactly `ln 2` at `r = 4`, and for the tent map it is exactly `ln r`. The estimate uses the family's analytic `derivative` when present, else a central difference. A visit to a point with `f′ = 0` (a superstable orbit) is floored rather than sent to negative infinity, so clamp the trace to your panel when you plot it.

<a name="custom"></a>

### Your own family

Any rule drops in; the parameter can mean whatever you like:
...
- **Settle is what makes it a diagram.** The sweep discards `settle` iterations per column (default 1000) so only the attractor prints, the standard practice for these figures. Near a fork, convergence slows to a crawl; if a fork looks smeared, raise `settle`, not `perColumn`.
- **Endpoints can be degenerate; the sweep avoids them.** At exactly `r = 2` a floating-point tent orbit collapses to 0, one doubled bit at a time, and the logistic critical orbit dies in two steps at exactly `r = 4` (which is why the factories start at 0.3 rather than the critical point 0.5). Column-center sampling means a sweep *to* an endpoint never lands *on* it.
...
- **Costs are trivial.** A full 1080-column image is a few million arithmetic steps, milliseconds of `setup()` work. The one thing worth avoiding is recomputing a sweep every frame; hold the `Image`, or record the dots into a `Batch` (see [Batches](../Drawing/Batches.md)).
