...
Paper marbling in closed form. A **`Marbling`** bath holds colored ink regions as vector outlines. **`drop`** floats a new circle of ink and pushes everything already floating aside. The raking tools bend every region at once: **`tine`**, **`comb`**, a circular tine, and **`swirl`**.
...
`Marbling` is a value type. Operations mutate it in place, and a copied bath is a snapshot you can keep raking separately. `spacing` is the refinement grain. As a transform stretches an outline, its segments subdivide until none is longer than about this. The new points land exactly on the transformed curve. `inks` is the ordered stack, oldest first. Each `MarbledInk` carries a fillable `shape` and its `color`.
...
A drop pushes every floating point radially away from its center. A point at distance `d` lands at `√(d² + r²)`, the displacement that preserves the area of everything around the drop. Earlier regions therefore thin into crescents but never vanish. Concentric drops of alternating color build the bull's-eye that every classic pattern starts from. Scattered drops make "stones."
...
`add` floats any outline onto the bath as-is, since only drops displace. Every later tool rakes it like any other ink. Floating [text outlines](../Drawing/Text.md) and combing them is the classic trick, and a `Shape` keeps its holes, so glyph counters survive.
...
`drawMarbling` honors the current stroke state on every outline, and restores the fill afterward. Leave the stroke off for flat marbled paper, since a hairline stroke reads as engraved veining. For anything custom, walk `bath.inks` yourself: each ink's `shape` fills, strokes, clips, and exports like any other. The whole bath is vector geometry, so `--export-svg` and `--export-pdf` write true paths. A [hatching pass](../Output/Export.md#hatching-solid-fills-for-a-pen-plotter) then turns the fills into plotter line work.
