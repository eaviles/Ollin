...
- **It does not tile.** Marching cubes builds each cube from its own eight corners, so a field meshed in pieces cut on lattice planes, each with a cube of overlap and kept by triangle centroid, meets itself exactly at every seam. Dual contouring places a cube's vertex from its neighbors as well, and its choices depend on computed positions. Pieces meshed that way leave holes along each seam: 3,896 open edges over six pieces of a blob field, and still 127 with two cubes of overlap, where marching cubes left none. Mesh a dual-contoured surface in one piece.
