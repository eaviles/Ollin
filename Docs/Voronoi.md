#### <sup>[Ollin](../README.md) → [Documentation](./README.md) → `Voronoi & Delaunay`</sup>
...
Everything here is driven by the seedable [`random`](./Random.md)/[`noise`](./Noise.md) helpers, so the same seed always yields the same tessellation.
...
See also [`Geometry`](./Geometry.md) for the `Shape`/`Contour` types these produce (and the shape booleans and offsetting that consume them), and [`Random`](./Random.md)/[`Noise`](./Noise.md) for seeding the sites.
