...
`--param name=value` can repeat. The value is read against the parameter's own kind. You can pass a color, a vector, a menu choice, or a swatch strip as easily as a number. The value is on the parameter before the canvas is sized and before `setup()` runs. So a `canvasSize` that reads a parameter takes the run's size, and anything built in `setup()` from it reads the run's value. It is applied again after `setup()`, so a `setup()` that sets the parameter itself does not undo it. Both passes go through the same `restore` path described below. Every export writes its parameter values into the file's recipe, so this flag is how a frame re-renders from the recipe it carries. The kinds and their spellings are on the [Export](../Output/Export.md#setting-a-parameter-for-the-run) page.
