...
- Worked examples: [`Examples/Shaders/ComplexPlane`](../../Examples/Shaders/ComplexPlane/Sketch.swift), a shader coloring the ratio of two moving points and `draw()` laying the circles that ratio is made of over it; [`ImaginaryLog`](../../Examples/Shaders/ImaginaryLog/Sketch.swift), the imaginary part of that ratio's logarithm through a cosine palette, with the branch cut as a seam; and [`Meromorphic`](../../Examples/Shaders/Meromorphic/Sketch.swift), a ratio of two cubics built from six roots that move as `Complex` values.
