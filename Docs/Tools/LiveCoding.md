...
- **Tuned parameters carry.** A `@Param` value you dragged in the inspector (or bound over MIDI/OSC) is re-applied to the fresh sketch before it draws. Untouched params take whatever default the code now declares, so editing a default in source still works.
