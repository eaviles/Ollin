#### <sup>[Ollin](../README.md) → [Documentation](./README.md) → `RGBD`</sup>

---

## RGBD frames and depth-lifted pose

A color image, a depth map, and the intrinsics that tie them together — an **RGBD frame** — is the shape every depth source produces: a [Record3D](./Record3D.md) `.r3d` file, its live USB stream, a webcam paired with a depth model, an iPhone capture app. `RGBDFrame` is that shape, in the `Ollin` core, so anything that lifts the 2D picture into 3D reads one type regardless of where the depth came from.

Two things come out the other end: a **point cloud** (every depth pixel unprojected — see [3D](./3D.md#clouds)) and a **single lifted point** — a tracked joint, a tapped pixel — back-projected to its true metric position. The latter is what makes **depth-lifted pose**: a flat 2D skeleton placed at its real distance in space.
...
`RGBDFrame` carries a color [`Image`](./Images.md), a metric depth map (meters, row-major from the top-left), an optional per-pixel confidence map, and the depth-grid [`CameraIntrinsics`](#space).
...
You rarely build one by hand — a depth source hands it to you ([`Record3DRecording.frame(at:)`](./Record3D.md), [`Record3DDevice.latestFrame`](./Record3D.md#live-usb)). `DepthConfidence` is the `.low`/`.medium`/`.high` grade `pointCloud(...)` uses as a floor to drop noisy samples (ignored when a frame has no confidence map).

<a name="unproject"></a>
## Lifting one point

`unproject(normalized:)` takes a **Vision-normalized** image point — `0…1` across the frame, origin at the **lower-left**, y up, the convention [`Body`](./Vision.md) and the other trackers report points in — and returns its metric 3D position, or `nil` if there's no valid depth there.
...
`Body.lifted(through:)` is the per-joint sugar over `unproject`: it back-projects every joint of a 2D [`Body`](./Vision.md) through a frame's depth into a `LiftedPose` — the skeleton in metric 3D.
...
Everything lifted lands in the camera's **right-handed, y-up space**: +x right, +y up, the camera looking down −z (a point in front of the lens has negative z, so metric depth maps to −z). That's the **same space** [`PointCloud`](./3D.md) and [`Camera3D`](./3D.md#camera) use, which is why a depth-lifted skeleton drops straight into the person's own cloud and both draw through one camera.
