...
A sketch can take the Mac's own screen as material. Any display, any app, or any single window arrives as a live image, so the sketch can draw it, filter it, and analyze it. That means anything running on the machine can become material, whether that is a browser, a map, a video call, a terminal, or another sketch.

[Syphon](Syphon.md) is the counterpart, and the difference is who has to agree. Syphon carries frames between apps that have both chosen to speak it. That is why it is fast and clean, and why it only reaches apps that publish. Screen capture asks nothing of the other app, so it reaches everything on the screen. In exchange it needs the one thing Syphon never does, which is the user's permission to record the screen.

Screen capture lives in a separate library. The drawing core therefore stays free of ScreenCaptureKit, and the permission belongs to the sketches that ask for it. Add `import OllinScreen` alongside `import Ollin`.
...
- [Permission](#permission) - what the prompt actually covers for a sketch, which is not what you would expect
- [Choosing what to capture](#choosing-what-to-capture) - naming a display, an app, or a window
- [Listing what is there](#listing-what-is-there) - finding the names to write into the sketch
...
Recording the screen needs the user's consent, and macOS grants that consent to an *application*, not to a piece of code. That distinction matters a lot for a creative-coding framework, because a sketch is usually not an application.

**A sketch run from the terminal inherits the terminal's permission.** Ollin builds plain executables with no bundle identifier of their own. That covers `swift run`, the `ollin` command, `OllinLive`, and the examples gallery. So macOS attributes their request to the process that launched them, which is Terminal, iTerm, Ghostty, or whichever you use. The prompt you see names your terminal, and so does the entry in System Settings. Allow it there once, and every sketch you run from that terminal can capture, with no further prompt. That single approval covers all of them.

That is convenient, and it is worth being clear about what you are agreeing to. If you allow a terminal to record the screen, everything you run from that terminal can record the screen. It is the same trade as giving a terminal Full Disk Access. That is why a sketch has to opt into the capability, by importing a library and calling `start()`. The drawing core never carries it.
...
`start()` asks for you, so most sketches never call `requestAccess()`. A capture with no permission is not an error, and it does not throw. It just has no frames. `waitingMessage` says why there are none, and `drawFrame` draws that message on the canvas for you.
...
A `ScreenSource` is a plain value you write in the sketch. The same line always picks the same thing, so the sketch itself stays the record of what was captured:
...
Capturing a **window** gives you that window alone, at its own size, with nothing in front of it, even when something covers it on screen. Capturing an **app** gives you its windows against their display, and keeps their positions and their overlap.
...
**A source that names something not on screen yet is not an error.** The capture keeps looking. It starts by itself the moment the window appears, so you can name a window and then go and open it. `isRunning` stays `true` through the wait, where it means "wanted", and `isReceiving` tells you whether frames are actually arriving.

<a name="listing-what-is-there"></a>

### Listing what is there

To find out what a machine can capture right now, ask it. All three calls need the permission, and they return an empty array without it. All three are ordered, so a listing reads the same way twice:
...
By default the sketch's own windows are cut out of a display capture. Without that, a full-screen capture contains the window the sketch is drawn in, so the picture feeds back into itself:

```swift
screen.excludesOwnWindows = true    // the default
```

Turn the exclusion off and that is exactly what happens, which is the point:

```swift
screen.excludesOwnWindows = false
```

The sketch draws a screen containing a window, which draws a screen containing a window, receding until the innermost copy is a few pixels across. This is video feedback, the effect people have been getting by pointing cameras at monitors since the 1960s, and here it costs one boolean. How far it recedes depends on how fast the sketch draws compared with the capture. Move the window and the recursion drifts and smears, which is the part worth watching.

Self-exclusion works on the process, not on the window title. So it holds for a sketch with several windows, and for a sketch with no bundle identifier. It has no effect when you capture a single window or another app, because the sketch was never in that picture to begin with.
...
`scale` is the setting that matters on a large display. At `1` a capture arrives at the display's true backing resolution. On a Retina screen that is twice its size in points, so a 5K display gives you a 5120-pixel-wide texture every frame. Halving the scale quarters the number of pixels, which is the cheap way to feed a heavy effect chain.
...
A sketch can then read the words on a page as they scroll, or find faces in a video call. It can follow motion across a map, or trace contours out of anything on screen. Map the results into the rectangle `drawFrame` returns, exactly as you would with a camera.

A tracker needs a CPU copy of each frame. That copy is made only while a tap is installed, so a capture you only draw never pays for it.

<a name="what-is-not-here"></a>

### What is not here

**System audio.** ScreenCaptureKit can record what the machine is playing, and this capture does not. It captures pictures only. Sound belongs with the [audio](../Helpers/Audio.md) analyzer surface rather than with a video frame source, and it has not been built yet.
...
**Capturing while the screen is locked or asleep.** The system does not allow it, and a capture reports itself as stopped.

### See also

- [Syphon](Syphon.md) - the cooperative counterpart, faster and cleaner, and it reaches only apps that publish
- [Virtual camera](VirtualCamera.md) - the other direction, a sketch *as* a camera
- [Vision](../Vision/Vision.md) - the trackers that can read a captured feed
- [Video](../Video/Video.md) - recorded footage as the same kind of live image
- [Effects](../Drawing/Effects.md) - the filters you can put a captured frame through
