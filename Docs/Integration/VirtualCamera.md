...
Color is carried as display-ready (sRGB-encoded) bytes, so tones in the camera match the sketch window. The camera runs at 30 fps, so a faster sketch publishes every other frame, and a slower one just updates the camera at its own pace.
