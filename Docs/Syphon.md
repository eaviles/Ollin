#### <sup>[Ollin](../README.md) → [Documentation](./README.md) → `Syphon`</sup>
...
- a **[client](#receiving-a-feed)** subscribes to a source from another app and hands you each frame as an [`Image`](./Images.md) you draw with `drawImage`.
...
Pairs naturally with [OSC](./OSC.md) or [MIDI](./MIDI.md): visuals over Syphon, control over OSC, both directions at once.
...
Call `publishSyphon(name:)` in `setup()` and the sketch's frames are shared from then on, under that name. The name is how consumers identify the source (usually paired with the app's name in their menus); it needn't be unique. That's all it takes:
...
`publishSyphon` builds a `SyphonServer` and registers it as a [`SketchExtension`](./Sketch.md), then returns it so you can check `hasClients` — handy if a frame is expensive to draw and you'd rather skip the work when nobody's watching:
...
Make a client, then read its frames in `draw()`. `newFrame()` hands you the latest frame as an [`Image`](./Images.md) backed by the source's live GPU texture, so you draw it like any other image — scaled, into a `Rectangle`, tinted, under the transform stack:
...
Call `newFrame()` each frame and draw the result; don't hold onto it across frames (the next call gives you the current frame). A source can come and go — if the publishing app quits, `isActive` goes `false` and `newFrame()` returns `nil`; call `reconnect()` to look again. Because the returned `Image` wraps a live texture, its CPU side (the `[x, y]` pixel subscript, `cgImage`) isn't meaningful; it's for drawing.
