...
Print separations produce both: the masters, and an honest on-screen preview of the overprint.
...
`halftoned` rotates each layer's dot grid to its own angle, the darkest ink taking the least visible 45 degrees, so the drums overprint into the traditional rosette instead of moire (pass `angles:` in radians to choose your own). Both passes preserve tone exactly: a 30% gray becomes dots covering 30% of the paper. Both also drop coverage below 2% to bare paper (and above 98% to solid ink), the minimum dot a press can hold; without it, the invisible sub-1% residue that 8-bit rounding leaves in "white" areas would screen into stray specks.

Separation and screening are per-pixel CPU work, memoized per distinct color: run them in `setup()` and hold the result, not once per frame. A GPU-backed image has no CPU pixels to read and returns an empty separation (`snapshot()` first). Everything is deterministic: the same image, inks, and paper always give the same masters, so separations are safe to snapshot and export.
...
The average coverage each layer reports (`layer.averageInk`, also printed at export) is worth a glance before printing: layers much above a third of the sheet are heavy for most presses.
