...
The inline form (`--inline`) is the same canvas and the same script block with no page around them, for a page you already have. In code the two are `WebPageForm.standalone` and `.inline`:
