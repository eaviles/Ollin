...
- [Pictures and text on the page](#pictures-and-text-on-the-page) - a picture once, as its file or its pixels; atlas text over the font's page; a gradient on a shape from the strip
- [Fields on the page](#fields-on-the-page) - a composed field's program walked by the same machine; a raymarched field traced through its camera and lit by its lights
...
The page carries the closed analytic shapes: `drawCircle`, `drawEllipse`, `drawRect`, `drawArc`, `drawTriangle`, `drawPolygon` by count, `drawStar`, `drawRing`, `drawPoint` and the markers, and the rest of the [shape catalog](../Drawing/Drawing.md). Each crosses with its fill, its stroke, the stroke's weight and alignment, a hollow band, and the transform it was drawn under. Symmetry folds cross as the copies they are. Every stroke crosses too: `drawLine`, `drawPolyline`, `drawBezier`, `drawCurve`, a `drawShape` outline, and an elliptical or full-turn `drawArc`. A stroke keeps its weight, joins, and caps, a gradient along it, and a width that varies. So does every fill the triangle path tessellates: `drawPolygon` from points, `drawShape` with its holes, `drawCurve` closed, and text in the default outline mode. Text through the glyph atlas (`textMode(.atlas)`) crosses as its glyph quads over the font's page. A gradient on an analytic shape crosses with its ramp, linear, radial, or along the path, read from the same strip the Mac bakes it into. A picture drawn with `drawImage` crosses too: a file you loaded, a picture painted or edited in memory, one a transform made, the color glyph in a line of text. It travels once however many frames draw it. It is drawn under its `tint`, its `blendMode`, a crop, and the transform it was drawn under, from its smaller levels when drawn small. A composed field drawn with `drawSDF` crosses as its program. Every leaf, melt, carve, joint, modifier, and domain transform of [the field catalog](../Drawing/Combinators.md) is in it, with the current fill or the leaves' own colors, a gradient fill or stroke by field position, and the stroke along the merged outline. A raymarched field drawn with `drawSDF3D` crosses too, with the camera it was seen through and the lights it was lit by. Those are directional, point, and spot lights, the ambient, the self-shadow of `castShadows()` toward the key, and the four shading models with their finishes, the specular, a subsurface bleed, the iridescent sheen, the sparkle, and the rim. A gradient fill by screen position crosses, and a `raymarchResolution` below one is traced at the fraction the Mac traces at. A `blendMode` applies to shapes, strokes, fills, text, pictures, and fields with the factors the Mac's pipelines use. A `Batch` recorded from any of those replays under its draw-time transform. `background` is each frame's clear, and a sketch that accumulates (`noClear`) plays back frame on frame. The tone map and the exposure ride along, and the present pass on the page dithers and encodes the way the Mac's does.
...
That covers a picture that is a live texture (a camera frame, a video frame, a compute texture, a layer the frame did not fill), clipping, `depth(at:)`, the rest of 3D (a mesh, a point cloud, a field drawn inside a layer), compute work, and wide-gamut or HDR output. Around a raymarched field it covers what the page's lighting does not carry. That is an environment, fog and aerial perspective, an area light, a light with a profile or a cookie, contact shadows, global illumination, ray-traced reflections, the scene through glass, caustics, and a finish that is transmissive, clear-coated, sheened, thin-film, brushed, or scattering. Among the effects it covers the passes that are a solve or a ladder on the Mac: `.diffuse`, `.distanceField`, `.boxBlur`, `.adaptiveThreshold`, `.fourier`, `.softProof`, `.liquidMetal`, `.heatmap`, `.gemSmoke`, `.seamlessClone`, `.defocus`, `.ambientOcclusion`, `.light`, and `.screenSpaceReflections`. It covers the simulations with a pipeline of their own: `.fluid`, `.watercolor`, `.selfWarp`, and `.multiScaleTuring`. A sketch that draws those is a video export today. The doors those calls wait on are listed in the [roadmap](../../ROADMAP.md#new-output-surfaces).
...
A picture travels once, whatever the frame count. A file the sketch loaded travels as the file when a browser reads the format (PNG, JPEG, GIF, WebP). A photo then weighs on the page what it weighs on disk. Any other picture travels as a PNG of the pixels the Mac's texture was built from. That is one painted in memory, edited through the pixel subscript, made by a transform, or read from a format a browser does not open. A picture whose pixels change between frames travels once per version, and two images with the same pixels travel once. The browser decodes each picture as it is, with no color conversion and no orientation applied. It lands in an sRGB texture with its smaller levels, the way the Mac holds it, so a picture drawn small reads the same level on both. A JPEG is the one thing decoded twice, by ImageIO on the Mac and by the browser on the page. The two decoders can differ by a few levels along a sharp color edge.

Text through the glyph atlas draws each glyph as a quad over the font's distance-field page. The recorder writes down the quads. When the recording ends, it writes the page's rows down to the last glyph packed, as an 8-bit PNG, and every glyph the frames drew is on it by then. The page's fragment is the Mac's, carried to GLSL. It turns the sampled distance into coverage through its screen-space derivative, remaps that to perceptual alpha, and applies it to the fill the quad's corners carry, so a gradient fill on atlas text crosses too. A font whose page filled up and was rebuilt during the recording stops the export, since the earlier quads would address glyphs that moved. A wall of body text in one or two fonts is nowhere near that.

A gradient on an analytic shape rides the shape's record as its ramp's row in a strip. The strip holds one row of 256 texels per distinct ramp in the recording, baked on the Mac in the ramp's own color space. The page reads it by the same lookup the Mac's fragment makes. The strip is a kilobyte a ramp.

### Fields on the page

A composed field is a small program on the Mac. Its leaves, combines, modifiers, and transform scopes are flattened in evaluation order, and a stack machine in the fragment walks them per pixel. The recorder writes the program down as it is, four rows of four numbers an instruction, with the covering quad's record beside it and the program's place in the frame. The page walks it with the same machine, written for GLSL over the framework's own arithmetic carried across. The distance functions, the joint ops, the combine switch, and the point transforms are the Metal text rewritten. The program sits in a uniform block, one field at a time, 256 instructions at most, which is the framework's own cap. The merged outline's coverage and the paints then resolve as the Mac's fragment resolves them, so a melt's seam and a stroke along it read the same. Symmetry copies and a replayed recording share one program.

A raymarched field crosses the same way, and its scene with it. Each frame that marches one carries a scene block, whole and exact. It holds the camera's view, projection, and inverse, the march budget, the ambient, the eye, up to eight lights with their kinds and cones, and the one caster the field shadows itself toward. It travels apart from the shape records because a camera matrix quantized inside its range would move a silhouette. The page's fragment rebuilds each pixel's world ray through the inverse view-projection and sphere-traces the field's program with the same step scale and the same pixel-cone coverage at the silhouette. It takes the tetrahedron normal, marches the analytic self-shadow toward the caster, and shades the hit through the punctual half of the mesh lighting written for the page. That half is the four shading models under directional, point, and spot lights with the wrap term and the cone, the subsurface bleed, the iridescent sheen in both of its modes, the sparkle, and the rim. A physically based finish prices its specular energy from the same split-sum table the Mac bakes, baked once for the page and carried as an asset when a field wears one. The hit's depth is written as the fragment's own, so two fields occlude each other through a depth buffer on the page as they do on the Mac. The march budget follows the export's. `.default` lifts to `.detail`, an explicit tier or step count holds, and `raymarchResolution` below one traces into a smaller target at the coverage-adaptive fraction the Mac would trace at, then upsamples once, its color bilinear and its depth point-sampled.

Measured against the Mac at the gate's 200-pixel cases, every field reads under a mean of 0.15 levels with no pixel more than 32 levels off. A melt with a carve, a morph, a gradient, a stroke, and a tiling reads at 0.006. A turntable blob reads at 0.13, jade under a spot and a point at 0.06, a self-shadowing plane at 0.04, a half-resolution march at 0.09, and three finishes, a metal, a toon, and a Gooch, at 0.06. Over the framework's own examples, all five Combinators sketches and twelve of the seventeen Raymarching sketches cross at means of 0.05 to 0.15 with no far pixel, each page 220 to 306 KB. The five that stop draw a mesh beside the field or an environment.
