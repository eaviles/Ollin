...
`--export-web` records what a sketch draws over a duration and writes a page that plays it back in a browser. It is an export like video: the sketch runs on the Mac, and the page is a file you put wherever you like. The page draws with the framework's own shaders, carried from Metal to GLSL: the shape shader, and the fragment behind every effect the sketch used. A frame on the page is the frame the Mac would have shown.
...
- [What crosses](#what-crosses) - the analytic shapes, the layered effects, and what stops the export
