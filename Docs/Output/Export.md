...
- [Saying what a sketch declares](#saying-what-a-sketch-declares) - `--list-params`, every `@Param` with its kind, what it accepts, and what it holds
...
The first draw of a frame advances the sketch's clock as always. The next N−1 draws repeat the same moment: `time` does not move, `deltaTime` is zero, and only `frameCount` counts on. So a camera or a motion written against the clock holds still while the layers underneath keep stepping. The mean converges, the frame is written, and the clock moves to the next frame. It costs N times a plain export. The example runs five passes a draw, so 40 draws is 200 passes of the ring sphere, which looks clean. At 120 draws it is the 600-pass still that [depth of field from light](../Drawing/DepthOfField.md) opens with.
...
`--cue <name>` lands a saved [cue](../Helpers/Cues.md) the same way, every parameter it holds at once, before and after `setup()` like `--param` and under the `--param` values, so a value given by hand still wins. `--cues <file>` names the sheet when the sketch is not carrying one; under OllinLive the `Sketch.cues.json` beside the sketch is installed first, so `--cue` alone finds it.

---

### Saying what a sketch declares

`--list-params` prints the sketch's parameters and stops. It is the other half of `--param`: one sets a value, the other says what there is to set.

```sh
swift run --package-path Examples Example-Live-Parameters --list-params
```
...
The values are the ones the first frame would be drawn with. The sketch's `setup()` runs first, and any `--param`, `--cue`, and `--seed` given beside it land too. So the listing describes that run, not the file it was written in.

Every value is printed in the spelling `--param` reads back, so a line can be pasted into a flag. That is exact for every kind but a color, which is said in hex and so comes back to the nearest eight-bit step.
