...
- [`Parameters`](./Helpers/Parameters.md) - `@Param` tunable knobs: typed inspector controls (from sliders and toggles to menus, color wells, text, and geometry fields) in grouped cards (with `.folded` sections that start closed), value scrubbing, optional smoothing, and binding from OSC or MIDI
...
- [`Room`](./Integration/Room.md) - `import OllinRoom` to let several machines on one network draw one piece: they find each other by the room's name with no server, trade values and `@Param` knobs, agree on one clock so motion stays in step, and take a seat each when the piece is split across screens
