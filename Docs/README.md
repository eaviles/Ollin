...
- [`Watercolor`](./Simulation/Watercolor.md) - wet paint on rough paper: a three-layer wash simulation (`watercolor(pigments:)`) with real pigment behavior and Kubelka-Munk glazing. Paint with any drawing call, call `dry()` between washes, and call `blot()` for backrun blooms.
