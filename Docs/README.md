...
- [`Vision`](./Vision.md) - `import OllinVision` for the Mac's camera (built-in, Continuity, or external) plus Apple's on-device perception — face tracking with landmarks, hand and body pose (joint skeletons), rectangle detection, and contour tracing (camera frame → vector `Shape`s), surfaced as typed results you read in `draw()`
