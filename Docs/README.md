...
- [`DMX`](./Integration/DMX.md) - `import OllinDMX` to drive stage lights and dimmers from `draw()` over Art-Net or sACN (a `DMXUniverse` of 512 channels with named-fixture sugar), and to let a lighting console drive a sketch, channels read in `draw()` or bound to a `@Param`
...
- [`Spatial`](./Output/Spatial.md) - write a 3D frame or a `Scene` as USDZ, so a piece opens in Quick Look, sends in a message, and stands on a real table through AR
