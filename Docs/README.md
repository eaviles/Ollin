...
- [`Scenes`](./3D/Scenes.md) - `loadScene` keeps a glTF file's *structure* instead of merging it: a tree of named nodes drawn in place with `drawScene`, reached by name to animate (`scene["lamp"]`), opening on the authored camera and punctual lights as ready `Camera3D`/`Light` values, and playing the file's authored keyframe animations on the sketch clock (`apply(_:at:)`)
