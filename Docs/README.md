...
- [`Physics3D`](./Simulation/Physics3D.md) - rigid bodies inside the 3D scene: a `World3D` of stacking, tumbling, swinging `Body3D`s (backed by Jolt) with joints, mouse grabbing through the camera, and `withBody` drawing
