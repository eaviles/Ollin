...
| `float gaborNoise(float2 p, float wavelength, float bandwidth[, float angle, float spread[, float impulses, float phase, uint seed]])` | Gabor noise in `[0, 1]`: sparse Gabor convolution, a field with one principal `wavelength` (in the units of `p`, pixels on a layer), a band `bandwidth` wide around it as a fraction, and one direction (`angle`, with `spread` 0) or every direction (`spread` π, the short form). `impulses` is the quality (32 in the short forms), `phase` slides the waves (periodic over 2π). Filtered for a one-pixel footprint. Mirrors the CPU `gaborNoise` and the `.gaborNoise` generator. |
