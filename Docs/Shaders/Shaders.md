...
A generator paints from math alone, like the plasma above. A filter transforms a layer you drew. A combine reads two layers at once, which is how you write a blend or a warp of your own in the shader. A fourth use is not a pass in the graph at all: a `Shader` handed to [`Sim.shader`](../Drawing/Effects.md#simfield-shader) is the **step of a simulation field**, run over the field's own state every frame with readers of its own (`cell`, `mark`, `input`) and no color conversion on either side. All three return a [`RenderTarget`](../Drawing/Effects.md), so you can draw it with `.image`, filter it again, or feed it into another effect.
