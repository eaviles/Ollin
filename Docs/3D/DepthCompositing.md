...
Two practical notes. The metric overload **needs a camera**, since it reads the near/far that map meters onto the depth buffer, and `intrinsic` is the matching one. Without a camera it's a no-op. It also **letterboxes the feed into the canvas** by the feed's own aspect. The picture is never stretched, whatever the `canvasSize` or the phone's orientation. The metric camera letterboxes to match, so placed geometry stays glued to the picture, with bars where the aspects differ. The [`3D/DepthOcclusion`](../../Examples/3D/Depth/DepthOcclusion/) example's metric mode (M) floats a grid of markers at a draggable metric plane in a live LiDAR feed. Stepping within that many meters of the camera blocks them.
