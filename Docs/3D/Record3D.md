...
The runnable example is [`Examples/3D/Depth/Record3DCloud`](../../Examples/3D/Depth/Record3DCloud/Sketch.swift). It reads the newest `.r3d` in `~/Downloads`, plays the clip, and orbits the cloud, switching to the live tethered stream the moment a phone connects. Drag to spin it.
