...
- [Lights & materials](#lights) - `directionalLight`/`pointLight`/`spotLight`/`ambientLight`, the `rectangleLight`/`diskLight`/`tubeLight` area lights, IES `profile:` + `cookie:` light shaping, `lightingPreset`, `specular`, `specularSharpness`; [a light that rides the camera](#camera-relative) (`relativeTo(.camera)`, `headlight`); [how far a light carries, and a room full of them](#reach) (`reach:`, `Light.reaching(_:)`); [one frame, several rigs](#light-sets) (`withLights`, `withoutLights`, `addLight`)
- [Materials](#materials) - `material(_:)`, `Material`, the named library (iridescent, velvet, jade, toon, gooch, …), the physically based tier, the [anisotropic brushed streak](#anisotropy) (`anisotropy`, `.brushedMetal`), [glass](#glass) (`Material.glass`, transmission + refraction), the [clearcoat + sheen](#clearcoat-sheen) layered lobes (`.carPaint`, `.lacquer`, `.satin`, `.felt`), the [thin film](#thin-film) (`thinFilm`, `.anodized`, `.oilOnWater`, `.nacre`, `.soapFilm`), and [real subsurface scattering](#subsurface-scattering) (`Material.scattering`, `.skin`, `.marble`), [the ink line](#outlines) (`outline(width:color:)`)
...
- [Detail maps](#detail-maps) - `Mesh.detailMapped(_:normal:scale:amount:)`: texture that survives a close look
...
- `Camera3D.orbiting(target:radius:azimuth:elevation:fieldOfView:near:far:)` places the eye on a sphere around the target. `azimuth` turns around the up axis: 0 puts the eye on +z looking back toward −z, and π/2 puts it on +x. `elevation` tilts above the horizontal, with a positive angle looking down. Both are in radians. It is the simplest way to spin a camera around a scene.
...
Each shape is a `Mesh` value type that holds positions, normals, and a triangle-index list. Every solid above has a generator under the same name with the `draw` prefix dropped, so the catalog reads the same way twice: `Mesh.box(…)`, `.roundedBox(…)`, `.sphere(…)`, `.icosphere(…)`, `.cylinder(…)`, `.cone(…)`, `.capsule(…)`, `.torus(…)`, `.torusKnot(…)`, `.pyramid(…)`, `.helix(…)`, `.plane(…)`, `.tetrahedron(…)`, `.octahedron(…)`, `.icosahedron(…)`, `.dodecahedron(…)`, `.supershape(…)`, `.superellipsoid(…)`, `.mobius(…)`, `.klein(…)`, `.extrude(…)`, `.lathe(…)`, and `.tube(along:…)`. **Build a mesh once and draw it every frame** with `drawMesh(_:)`. The `draw*` calls rebuild the mesh each frame. That is fine for a cube and wasteful for a swept knot or a dense supershape:
...
You can also pass your own arrays through `Mesh(positions:normals:indices:)` and `drawMesh`. Geometry built with no normals of its own lights flat, so a mesh that came from raw positions (a marching-cubes surface, a deforming face mesh) wants `generatingSmoothNormals()`, which returns a copy with area-weighted normals worked out from the positions and the triangle list. Meshes need a camera, like point clouds, and they follow the 3D transform stack, not the 2D affine one.
...
final class LoadedModel: Sketch {
    var model: Mesh?
    override func setup() {
        model = (try? loadMesh("/path/to/duck.usdz"))?.normalized(scale: 3)
    }
    override func draw() {
...
The loaders mirror the image and font ones. `loadMesh(path)` and `loadMesh(url)` sit on `Sketch`. `Mesh(contentsOf:)`, `Mesh(path:)`, and `Mesh(resource:withExtension:in:)` load a bundled model. As always, `in:` has no default, so pass `.module`. Each throws a `FileError` when the file is not there (`missing`) or holds no triangles a reader can find (`unreadable`).
...
- **`noLights()`** turns lighting off, so solids draw flat in their `fill` color, unlit. `ambientLight(.white)` is the other way to a flat, full-bright look. Like every other lighting call it is drawing state, so inside a `withState` block it turns off only what that block draws (see [one frame, several rigs](#light-sets)).

<a id="camera-relative"></a>

#### A light that rides the camera

A light's position and direction are world space, so the sun stays put as the camera orbits. That is what a sun, a sky, and a product shot want, and it is the default. Two cases want the other thing: a fill that keeps the side facing you from ever going black, wherever you look from, and a rig that should hold its look under an orbit instead of turning with the world. `relativeTo(.camera)` reads a light in the camera's own frame, and `headlight()` is the first case in one call:

```swift
camera(.orbiting(radius: 8, azimuth: time * 0.3, elevation: 0.3))

lightingPreset(.threePoint.relativeTo(.camera))   // the rig turns with the view
headlight(Color(white: 0.6), intensity: 0.5)     // and a fill from the eye
```

In camera space the eye sits at the origin, `x` runs to its right, `y` up, and `z` back toward it, so `Vector3(0, 0, -1)` is the direction the camera looks and a point light at `Vector3(2, 1, 0)` hangs two units to the right of the eye and one above it. The numbers on the light are left alone; only the frame they mean changes, and every kind takes it, a panel's `up` hint included. The light is resolved against the frame's `Camera3D` each time the lights pack, so it follows an orbit, a `cameraShowcase` move, and a drag alike, and with no camera set it reads as world space. `Light.relativeTo(_:)` does the same for one light, `LightingPreset.relativeTo(_:)` for a whole rig (the ambient has no frame and stays as it is), and `.relativeTo(.world)` puts a light back.

`headlight(_:intensity:specular:softness:)` is a directional light down the view, read `relativeTo(.camera)`: whatever faces the camera is lit and the side facing away is what falls off. Beside a world-space key it is a fill that never lets a turned-away face go black; on its own it is the flat, even look of a flash. It casts no shadow, because from the eye every shadow it would throw hides behind what throws it, so the pass would buy nothing. The [Inked](../../Examples/3D/Materials/Inked/) example switches between a world sun, the rig riding the camera, and a headlight on one still life.
...
```swift
let downlight = try! IESProfile(resource: "downlight", in: .module)   // parse once, in setup()
...
```swift
let gobo = LightCookie(try! loadImage("blinds.png"))!    // wrap once, in setup()
...
#### How far a light carries, and a room full of them

A point or spot light reaches equally far in Ollin: there is no distance falloff, so `intensity` is the brightness you get, wherever the surface stands. That is the right default for a key light or a sun, which is why it is the default. It is the wrong one for a lamp. Sixty-four lamps that each carry forever are one flat wash, and the night between them is gone.

`reach:` draws the bound. A light with one is full strength at the source, a quarter of that halfway out, and *exactly* nothing at `reach` and beyond. (Real light thins as the inverse square of the distance. That curve has no finite edge, and it blows up at the source. This is the same shape with both ends made usable.)
...
Every light with a position takes it: `pointLight`, `spotLight`, `rectangleLight`, `diskLight`, `tubeLight`, and the matching `Light` factories, plus `Light.reaching(_:)` to set or clear it on a light you already have. `nil`, the default, is the unbounded model, unchanged. A directional light ignores it, having no position to measure from. On an area light the bound multiplies the physical falloff it already has, so it is a cutoff rather than the whole falloff.

Giving lamps a reach is also what makes a frame able to carry many of them. **A frame holds up to eight lights on the plain path and up to 256 in all.** Past eight the renderer divides the screen into 16-pixel tiles and works out, once per tile, which lights can arrive there. A lit pixel then shades against the lights standing over it rather than every light in the scene. Nothing about the API changes at the crossing, and nothing about the picture does either. The tile grid decides only *which* lights a tile lists, never how one of them shades. What changes is the cost. On an M2 at 1080×1080, a courtyard of 64 lamps renders in 16.9 ms against 42.65 ms shading every lamp everywhere. 128 lamps take 31.1 ms against 81.24 ms. (`Scripts/benchmark.sh lights` runs that sweep on your own GPU.)

A light with no `reach` cannot be bounded, so it stays in every tile and costs what it always did. A hundred unbounded lights are a hundred lights on every pixel. The reach is what buys the cull, and it is the same number that makes the picture read.

Some things stay on the frame's first eight lights, by design. **Shadows**: a frame casts from at most four lights, chosen among the first eight, since each caster costs its own pass over the scene. **Fog and volumetric shafts**, **global illumination**, and the **path-traced export** read the same eight. A **render target** is tiled when it is the size of the canvas, which is what `makeRenderTarget()` gives you. Drawing a scene into one and filtering it keeps every lamp. A target of some *other* size draws its own camera view, which the frame's grid was not culled over, and falls back to those eight.

The `3D/Lighting/ManyLights` example is a courtyard at night with 64 drifting lamps in it, with the count and the reach on sliders.

<a id="light-sets"></a>
#### One frame, several rigs

The lamps are drawing state, like the fill and the blend mode. The lights in force when a solid is drawn are the lights that shade it. So one frame can hold several rigs at once, and each surface reads its own.

`withLights` is the scope. Inside the block the lamps you pass are the only ones there are; outside it, the frame's own rig comes back:

```swift
directionalLight(.white, direction: Vector3(-1, -1, -0.5))   // the frame's key
drawPlane(width: 40, depth: 40)                              // the corridor, lit by it

withLights([.point(Color(kelvin: 2700), at: Vector3(-5, 2, 0), intensity: 4, reach: 7)]) {
    withState { translate(-5, 1, 0); drawBox(size: 2) }      // warm, its own bulb
}
withLights([.point(Color(kelvin: 6500), at: Vector3(5, 2, 0), intensity: 4, reach: 7)]) {
    withState { translate(5, 1, 0); drawBox(size: 2) }       // cold, its own bulb
}
```

Nothing from room A reaches room B, and neither room sees the hall's key. It takes an `ambient:` too, and a [`LightingPreset`](#lights) in place of the array, so `withLights(.noir) { … }` puts one body under a whole rig while the frame keeps another.

`withoutLights` is the unlit scope. The block's solids come out flat in their `fill` color while the scene around them stays shaded: a hologram, a flat overlay, a diagram standing in a lit room, a bulb drawn as its own glow:
...
Both are `withState` with the clearing step built in. That step is the one easy to forget by hand: without it the lamps *add* to the frame's rig rather than replace it. The long form, `withState { noLights(); pointLight(…); drawMesh(…) }`, is the same thing.

Three things stay with the frame's own set rather than following a scope.

- **The casters.** A shadow caster renders its own depth pass over the whole scene into its own map, and the map is keyed by the caster's place in the light list that holds it. A scoped set's lamps therefore light what the block draws and throw nothing; `castShadows()` casts from the frame's rig, and those shadows still land on the solids a scoped set drew.
- **The environment, the atmosphere, and the lens flare.** These are properties of the world rather than of one rig, so they are set once for the frame. A scope that is `noLights()` does turn its own image-based lighting off with everything else, which is what makes `withoutLights` flat even under an HDRI.
- **The lamps a sketch sets the ordinary way.** A light set outside any block joins the frame's rig wherever in `draw()` you set it, exactly as before, so nothing about an existing sketch changes.

A frame holds its own set plus fifteen. Each one is another packed uniform, and past the [eight-lamp inline path](#reach) another tile cull with its own buffers. So the count is bounded rather than open. Beyond it the extras draw under the frame's own set, and Ollin says so once.

Splitting a frame into sets costs a batch per set, since a room's solids cannot merge with the next room's. It saves shading, since a surface pays for its own room's lamps rather than every lamp in the building. Which one wins depends on the lamp count. On an M2 at 1080x1080, a hall of 8 rooms with 8 lamps each renders in 2.3 ms as 8 sets, against 13.3 ms as one set of 64 lamps. With 2 lamps each it is about 1.2 ms against 4.0 ms. At 2 rooms and 4 lamps in all the two are within the noise of each other. (`Scripts/benchmark.sh lightsets` runs that sweep on your own GPU.) The two are different pictures as well as different costs, so this is a reason to reach for sets rather than a tax on them.

The `3D/Lighting/LightSets` example is three rooms side by side, each under its own fixture, with the same unlit plate on every back wall.
...
- **shading** is `.standard` (smooth), `.toon` (cel bands), or `.gooch` (warm-to-cool).
  A `.toon` finish bands the light into `toonBands` steps; a `.gooch` one runs between
  `goochWarm` on the lit side and `goochCool` in the shadow, which is the technical
  illustrator's way of keeping a dark side readable.
- **specular** and **specularSharpness** set the highlight's strength and how tight it is.
- **rim** is a glow around the silhouette. `rimColor` tints it and `rimSharpness`
  is its falloff exponent, where a higher number pulls the glow into a thinner edge.
- **subsurface** is light glowing through thin parts, in `subsurfaceColor` (a jade
  green, a warm wax, a fleshy tone). It is strongest with a light
...
`outline(width:color:)` draws an ink line around every mesh that follows, `width` canvas pixels wide, black by default: the cartoon's outline, at home over `.toon` and welcome over any finish.

```swift
material(.toon)
outline(width: 3)                 // the cartoon look
drawSphere(radius: 1)

material(.glossy.outlined(2, color: Color(hex: 0x2A2A40)))   // the same setting on a value
drawTorus(radius: 0.7, tube: 0.3)
noOutline()                       // and off again
```

<img src="../../Guide/Images/21-3DGently/Inked.gif" alt="A red sphere, a blue box, and a magenta torus in hard cel bands, each ringed by a thin dark line, turning under a light that keeps the bands still while the view goes round" width="480">

The line is drawn as an inverted hull: the mesh once more, every vertex pushed out along its normal by the width in screen pixels, with the faces toward the eye culled, so only the rim past the silhouette shows. Three things follow. It holds its width at any distance, like a pen, rather than thinning as a shape recedes. It takes the depth test like the surface it rings, so a nearer shape hides a farther one's line, and an inner silhouette (the hole of a torus) gets its line too. And a hard-edged mesh (a box) opens a small notch at each corner, because each face's hull moves off along its own normal, while a smooth mesh takes a clean line. `Material.outlineWidth` and `outlineColor` carry it on a material value (`outlined(_:color:)` sets both), and a change of width or color opens a new batch, like any change of material. Solid and textured meshes take the line; wireframe, matcap, instanced, and field draws do not, and the path-traced export leaves it out. Drawing state, saved by `withState`.
...
Use a built-in by name, load your own matcap PNG, or generate one. Handing `matcap(_:)` an image wraps it in a `Matcap`, which is the value the built-ins are; hold one as `Matcap(image)` to keep it around.
...
`environment(_:)` lights the scene with an HDRI environment, which is image-based lighting, and `noEnvironment()` clears it again for the frame. A physically based surface then gathers its ambient and reflections from the surroundings. This is what makes a metal read as metal, since a smooth chrome ball reflects the environment instead of going dark.
...
The built-ins light from 1K, and the lighting never needs more than that. For a sharper *backdrop*, `highResolution(_:)` downloads a larger version on first use and caches it, at `.twoK`, `.fourK`, or `.eightK` (`.oneK` is where every built-in starts). The 1K shows in the meantime, then the larger one swaps in, and only the skybox gets sharper. You can also load or download any equirectangular HDRI, either `.exr` or `.hdr`:
...
The weather is a `Clouds` value, and `clouds(coverage:…)` is shorthand for building one; `Clouds(coverage:density:scale:…)` holds a sky's weather to pass around. The pieces:
...
Each of those modifiers returns a copy, because an `Environment` is a value: a `Source` (a bundled `.resource`, a `.remote` URL with a bundled fallback, a plain `.url`, a procedural `.sky`, or a live `.feed`), plus `intensity`, `rotation`, `showsBackground`, `backgroundBlur` (nil to let the resolution choose), and `clouds` for a sky. `Environment(source:…)` builds one directly. A built-in also remembers where it came from: `bundledResource` is the 1K that ships in the framework, and `polyHavenSlug` is the name `highResolution(_:)` fetches the sharper version under. Both are nil for a file or a URL of your own, which is why `highResolution(_:)` has nothing to fetch for those.
...
Each factory is shorthand for `Light(kind:color:…)`, which takes every field of every kind at once: that is the form to reach for when a light is built from stored numbers rather than written out.
...
```swift
let grid = try! loadImage("/path/to/texture.png")
...
Every map below sets a field on that same value, and the `Mesh` helpers that add them (`textured(_:)`, `normalMapped(_:)`, `triplanarTextured(_:…)`, `detailMapped(_:…)`) are shorthands for setting one. Reaching for the fields directly is how a material loaded from a file is adjusted: `normalTexture` and `normalScale`, `metallicRoughnessTexture`, `occlusionTexture` and `occlusionStrength`, `emissiveTexture` and `emissiveColor`, `heightTexture` and `heightScale`, `triplanarScale` (the tile size in world units, `0` for off), and the detail pair `detailTexture` and `detailNormalTexture` with `detailScale` (how many times they tile across one base uv tile) and `detailAmount` (`1` as authored, `0` off).
...
#### `Mesh.detailMapped(_:normal:scale:amount:)`

Returns a copy wearing a detail `color` map, a detail `normal` map, or both. `scale` is how many times the pair tiles across one base uv tile, 8 by default. `amount` fades it, 1 as authored and 0 off:
...
The `3D/Materials/Detail` example sways close past two spheres wearing the same base maps, where only one carries the detail pair. The tile count and the amount are live parameters.
...
- [How far a light carries](#reach) (`reach:`), and with it a frame of up to 256 lights, culled per screen tile so a lit pixel shades against the lamps standing over it.
...
- **Defocus a 3D scene by its own depth.** Draw the scene into a render target and it captures depth, exposed as `target.depth`. That is a gray depth layer you can feed straight to the [`.defocus`](../Drawing/Effects.md#depth) depth-of-field combine, or filter and draw to inspect. Set the camera's `near` and `far` to bracket the scene so the focal plane sweeps it. See [`Effects.md`](../Drawing/Effects.md#depth) and the [`SceneDefocus`](../../Examples/3D/Effects/SceneDefocus/Sketch.swift) example.
