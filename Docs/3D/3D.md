...
> **v1 limits.** Two bounces. A metal seen in a mirror shows the scene around it, so a mirror floor's reflection of a chrome sphere carries the room in miniature. The level beyond that shows the environment. Reflections render at full resolution, and a quality dial is a later step. Inside an off-screen render target they keep the single-ray form, since the anti-aliased path covers the canvas itself. They draw on the live window and through the headless image/snapshot export, the same scope as shadows.
...
- **A surface seen at a glancing angle softens.** One pixel there covers a long thin strip of the picture, and the level is picked for the long side of that strip, so a floor running away from you gives up some sharpness along the view. Anisotropic filtering is the usual answer, and it is deliberately off: switching it on made the same scene render differently on every run, and a frame that cannot be drawn twice is worth more than the sharpness.
...
`strength: 0`, or `scale: 0`, is the off switch, and the frame is byte-identical to one with no detail maps at all. The pair stays uv-mapped, so a triplanar mesh skips it with a one-time note. Like the rest of the mesh texture path, the maps carry no mips. At very high tile counts fine detail can therefore shimmer under minification, so keep the detail scale in the range the framing actually shows.
