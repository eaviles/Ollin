...
- [Lights & materials](#lights) - `directionalLight`/`pointLight`/`spotLight`/`ambientLight`, the `rectangleLight`/`diskLight`/`tubeLight` area lights, IES `profile:` + `cookie:` light shaping, `lightingPreset`, `specular`, `specularSharpness`; [a light that rides the camera](#camera-relative) (`relativeTo(.camera)`, `headlight`)
