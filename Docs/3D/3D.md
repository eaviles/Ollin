...
Shadows are **opt-in**, so a sketch that doesn't call `castShadows()` pays nothing. They are **per-frame** like the lights and camera, so call it in `draw()`. It needs a camera and a caster. The caster is the scene's first **directional** light. With no directional it takes the first **spot** light, then the first **point** light, then the first **rect or disk** [area panel](#area-lights). The auto-lit default's key is directional, so a default-lit scene just works. With no camera or no caster it's a no-op. `noShadows()`, or `castShadows(false)`, turns it back off.
