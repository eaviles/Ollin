...
**A light opts out with `castingShadow(false)`:**

```swift
light(Light.point(.white, at: Vector3(-3, 2, 0)).castingShadow(false))   // a fill, throws nothing
```

Use it on a bounce or fill light, which usually reads better with no shadow of its own. Every caster costs a pass over the scene from its own point of view, so it is also the dial for the cost.
