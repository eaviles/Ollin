...
One caster is the **primary**, and the rest of the renderer follows it alone. It is the scene's first **directional** light, then the first **spot**, then the first **point**, then the first **rect or disk** [area panel](#area-lights). [Volumetric shafts](Atmosphere.md), [subsurface transmission](#subsurface-scattering), and [contact shadows](#contact-shadows) all read that one, and a [marched SDF field](../Drawing/Combinators.md) shadows toward it. The other casters darken solid surfaces and nothing else.
