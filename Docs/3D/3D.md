...
The loaders mirror the image and font ones. `loadMesh(path)` and `loadMesh(url)` sit on `Sketch`. `Mesh(contentsOf:)`, `Mesh(path:)`, and `Mesh(resource:withExtension:in:)` load a bundled model. As always, `in:` has no default, so pass `.module`. Each returns `nil` if the file cannot be read.
