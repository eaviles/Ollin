...
- **A point light casts only as the primary.** It needs a whole cube of depth, or the ray tracer, and the frame holds one of each, both belonging to the primary caster. So a point light on its own casts, and the same light beside a directional key lights the scene and throws nothing.
- **A tube never casts.** It emits radially, so there is no side to render a map from.

The `3D/Lighting/TwoCasters` example runs a key and a stage light together, with a third light kept out of the shadow work.
