...
- [Ray-traced reflections](#rt-reflections) - `rayTracedReflections`, `reflectionQuality`
...
Trade reflection resolution for frame rate. The layer traces one ray per pixel of the drawable, and on a reflective scene that is most of what the frame costs. `.performance` halves the layer in each direction. The trace then does a quarter of the work, and the surfaces read the layer scaled back up. On an M2 the example above goes from 20.2 ms of GPU per frame to 14.3, which is 49 fps to 70.

The picture pays for it in two ways, and both are worth knowing before you reach for it. A curved mirror quantizes, because one traced ray now serves four screen pixels that face different directions. And a mirror reads about 15% weaker, because the layer carries its hit coverage per pixel. A coarser layer smears that coverage at every silhouette, and a smeared pixel falls part of the way back to the environment.
...
> **v1 limits.** Two bounces. A metal seen in a mirror shows the scene around it, so a mirror floor's reflection of a chrome sphere carries the room in miniature. The level beyond that shows the environment. Inside an off-screen render target they keep the single-ray form, since the anti-aliased path covers the canvas itself. They draw on the live window and through the headless image/snapshot export, the same scope as shadows.
