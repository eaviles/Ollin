...
The cost is the second pass: a frame with glass in it draws its scene twice. A solid body adds a short march through the layer's depth per pixel it covers; a thin wall adds nothing. It runs on the live window and through the headless image, snapshot, and video exports, the same scope as shadows.
