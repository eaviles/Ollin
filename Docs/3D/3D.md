...
- **`spotLight(color, at:direction:angle:penumbra:)`** is a point source narrowed to a cone. It is aimed along `direction` and spans the full `angle`. `penumbra` sets the soft edge, from `0` hard to `1` very soft.
...
Ahead is a live camera environment, re-baked from a `FrameSource` so 3D objects reflect the real world. See the [roadmap](../../ROADMAP.md#3d-mode). 2D stays the default throughout.
