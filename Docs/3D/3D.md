...
- [Decals](#decals) - `Decal`, `decal(_:at:...)`: pictures stamped onto the scene
...
- **`rectLight(color, at:direction:width:height:)`** is a flat rectangular panel, a softbox or a window. It is centered at `at` and faces along `direction` like a spot's axis, with an optional `up:` hint orienting its height axis.
...
Three knobs on any `Material` drive it.
...
The built-ins light from 1K, and the lighting never needs more than that. For a sharper *backdrop*, `highRes(_:)` downloads a 2K, 4K, or 8K version on first use and caches it. The 1K shows meanwhile, then it swaps in, and only the skybox gets sharper. You can also load or download any equirectangular HDRI, either `.exr` or `.hdr`:
...
The `3D/Lighting/TwoCasters` example runs a key and a stage light together, with a third light kept out of the shadow work. The `3D/Lighting/PointCasters` example adds two circling lamps to an overhead key, so each prop drops three shadows at once.
...
`0` is a hard edge, the `0.5` default is a pleasant contact-hardening soft, and `1` is very soft. **One knob softens every caster kind.** The directional and spot casters use a variable-kernel filter on their shadow maps, called *Percentage-Closer Soft Shadows*, which searches for the occluder and widens the blur with the receiver's distance from it. The ray-traced point caster's area light softens alike. For a rect or disk area caster the knob scales the panel's physical extent instead. The `0.5` default is the panel's true size, `0` is hard, and `1` is twice as soft. It's persistent, like the other shadow settings, so set it once in `setup()` or `draw()`.
...
The example `3D/Effects/GlossyReflections` stands a satin floor between two colored walls, with a row of balls from mirror to nearly matte. Holding the space bar drops back to the mirror ray and drains the room out of them.
...
The renderer draws the whole frame a second time with every transmissive surface taken out, so what that pass leaves is the picture you would have made with the glass removed. Each piece of glass then reads it where its own refracted view ray leaves the body. A thin wall leaves parallel to the view, so a pane shows what stands behind it undistorted, tinted and frosted but in place. A solid body bends the ray on the way in and travels its `thickness`, so it gathers a narrower, shifted view, the way a glass ball does. `roughness` reads the layer through a coarser mip, so a frosted body softens the scene the same way it softens the sky. `attenuationColor` still deepens over the distance travelled inside.
...
Three things to know, and none of them are hidden:
...
- **A solid body shifts and gathers the scene rather than inverting it.** It reads one point per pixel, where a lens is a whole path with a second bend on the way out. The traced walk has the full lens; this has the displacement, which is most of what the eye reads as glass.
...
The cost is the second pass: a frame with glass in it draws its scene twice. It runs on the live window and through the headless image, snapshot, and video exports, the same scope as shadows.

The example `3D/Materials/SeeThrough` puts a pane, a solid, a frosted solid, and a tinted solid in front of a row of colored blocks. Holding the space bar turns it off and empties them out.
...
The example `3D/Effects/Upscaling` is a mirror floor tracing a ring of columns under cast shadows, with the toggle and the tier on live `@Param` knobs. Watch the FPS readout while flipping them. Measured on an M2 at a 1080-class drawable, that scene runs about twice as fast at `.performance` as at full resolution.
...
The example `3D/Effects/FrameInterpolation` is a ring of orbiting blocks, a fast sweeping arm, and a pendulum, with the toggle and a speed knob on live `@Param` controls. Turn the speed up to find where a made frame stops keeping up.
...
A map is easy to *author* as well as load. Start from a height function and take its slopes. The `3D/Materials/NormalMaps` example builds hammered, woven, and engraved looks this way, in code, with a `relief` knob on the map strength:
...
The `3D/Materials/Parallax` example draws one crater height map three ways: parallax-mapped, displaced, and a bare color-mapped control, with the parallax depth as a live knob. Watch the outlines.
...
The `3D/Materials/Triplanar` example wears one authored vein texture on a morphing metaball skin and a cairn of separate boxes, with the tile size and relief as live knobs.
...
The `3D/Materials/Detail` example sways close past two spheres wearing the same base maps, only one carrying the detail pair, with the tile count and strength as live knobs.
...
#### `Decal` and `decal(_:at:direction:width:height:depth:roll:opacity:)`
