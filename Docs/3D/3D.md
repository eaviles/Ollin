...
`fieldOfView` is the vertical angle in radians, and the default (`.pi / 3`, 60°) is a wide view. When a scene is framed like a photograph, name the lens instead: `.focalLength(_:)` is the vertical angle that focal length sees on a full-frame camera (a 36×24 mm sensor, the format lenses are named for), and `.degrees(_:)` reads the angle in degrees. A shorter lens sees wider:
...
A smaller format sees less through the same glass: pass its `sensorHeight` in millimeters (`.focalLength(23, sensorHeight: 15.6)` for an APS-C camera) and the angle follows.
...
The matrices behind a camera are public: `viewMatrix` takes a world point into camera space (the camera at the origin looking down −z) and `projectionMatrix(aspect:)` takes camera space into Metal clip space, both column-major `simd_float4x4`, with `viewProjectionMatrix(aspect:)` their product and `matrices(aspect:)` the pair as the shared `OllinCameraMatrices`. A compute kernel projects through the sketch's camera by packing them with `cameraParams()` (see the [compute reference](../Shaders/Compute.md#camera)); on the CPU, `project(_:)` is the same map to canvas points.
