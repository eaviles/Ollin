...
- **Call it in `draw()`, every frame.** Both `cameraControl()` and `cameraMove(_:)` set the camera for the current frame (the camera is per-frame state, reset each frame), the same lifetime as `camera(.orbiting(...))`. Calling once in `setup()` does nothing.
