...
- **`.usdz` / `.usdc` / `.usda` / `.usd`**: the node graph with names and per-node transforms, node-local meshes wearing their authored preview-surface colors and textures, cameras (perspective and orthographic), the authored **UsdLux lights** (see [The authored lights](#lights) for the kinds), and the authored **transform animation**, xformOp timeSamples baked into keyframe tracks, both read by Ollin's own USD parser, since no platform importer carries them. A USD file's whole timeline arrives as one (unnamed) animation, so `scene.animations.first` is the way in; its tracks bind to nodes by *name*, so a name duplicated across branches animates its first depth-first match. An exported `.usdz` from a Mac or iOS design tool drops straight in. USD **skinning** isn't read yet; skins and blend shapes stay glTF features for now.
