...
- **`.usdz` / `.usdc` / `.usda` / `.usd`**: the node graph with names and per-node transforms, **children in the file's authored order**, node-local meshes wearing their authored preview-surface colors (linear values shown correctly on screen) and textures (read out of the `.usdz` package itself), cameras (perspective and orthographic), the authored **UsdLux lights** (see [The authored lights](#lights) for the kinds), the authored **transform animation** (xformOp timeSamples baked into keyframe tracks), and the **UsdSkel deforming tier**: skeletons, skinned meshes, and blend shapes, their SkelAnimation joint and weight channels riding the same animation, all read end to end by Ollin's own USD parser. A USD file's whole timeline arrives as one (unnamed) animation, so `scene.animations.first` is the way in; every track binds to the exact prim that authored it, so duplicated names across branches stay unambiguous. A prim hidden in the design tool (`visibility = "invisible"`, or a `guide`/`proxy` purpose) keeps its place in the tree but draws nothing; a scene arrives in its author's own units and orientation (`upAxis`/`metersPerUnit` are not applied); a subdivision-surface mesh draws its control cage (refine with [`subdivided(_:)`](../Generators/SubdivisionSurfaces.md) when the smooth limit matters). An exported `.usdz` from a Mac or iOS design tool drops straight in.
...
All three of glTF's interpolation modes play as authored: stepped holds, linear blends (rotations along the shortest arc), and eased cubic splines. A USD file's timeline arrives the same way, one animation whose tracks were baked from its xformOp timeSamples (USD's runtime default is linear, so that's what the baked tracks are), timeCodes converted to seconds through the layer's `timeCodesPerSecond`; its SkelAnimation joint and blend-shape channels join the same animation. That covers *rigid* motion, whole nodes moving; the deforming tier, meshes that bend and blend, is next.
...
The camera rides its node: it resolves through the tree's *current* transforms every time you read it, so an animation (or your own node mutation) that moves the camera's carrier moves the camera with it.

It's also plain data, and the natural next step is handing it to the viewer. The `from:` forms of the camera rig seed their opening shot from any `Camera3D`, so one call opens on the authored framing and gives the viewer the orbit:
...
- **Cameras and lights ride their nodes.** `scene.cameras` / `scene.lights` resolve through the tree's current transforms on every read, so a moved carrier node (by hand or by an applied animation) carries its camera or light. Assigning either array (tweaking one element in place counts) replaces it with your own fixed world-space array, which no longer follows the nodes.
- **Duplicated names** resolve to the first match, depth-first. Unnamed nodes have an empty name.
- The bundled demos are full worked examples: `3D/Geometry/LoadedScene` (a static stage, `scene.gltf` generated by `Scripts/make-sample-scene.swift`), `3D/Geometry/AnimatedScene` (an orrery playing its authored "spin", generated by `Scripts/make-animated-scene.swift`), `3D/Geometry/SkinnedScene` (a tidepool whose kelp sways on skins while an anemone pulses on morph targets, generated by `Scripts/make-skinned-scene.swift`), `3D/Geometry/USDScene` (a sculpture court from `stage.usda`, lit by its authored UsdLux rig with one light of every mapped kind, generated by `Scripts/make-usd-scene.swift`), `3D/Geometry/USDAnimatedScene` (a kinetic mobile whose authored timeSamples spin, bob, tumble, swing, and breathe its parts, generated by `Scripts/make-usd-animated-scene.swift`), and `3D/Geometry/USDSkinnedScene` (a pond where a sea serpent sways on a five-joint UsdSkel chain and a lotus breathes on blend shapes, generated by `Scripts/make-usd-skinned-scene.swift`); point `OLLIN_SCENE` at any scene file of your own to try any of them.
