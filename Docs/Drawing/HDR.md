...
- **Output is still 8-bit.** The screen and exported PNGs are 8-bit sRGB, and the float precision lives in the *intermediate*, which is why banding disappears and HDR values survive compositing even though the final image is 8-bit. A triangular dither is applied at the 8-bit step to keep gradients smooth.
