...
- **Color can exceed full brightness.** Additive light piling up ([`blendMode(.add)`](../Drawing/Drawing.md#blendmode)), a bright gradient, overlapping glows — the intermediate holds values above 1.0 instead of clipping them mid-render. What happens to those values when the frame reaches an ordinary 8-bit screen is your choice, set with `toneMap(_:)`.
...
See the `Rendering/ToneMapping` example: colored lamps orbit and overlap additively, and a key cycles the three mappings so you can watch the bright cores clip, compress, or roll off.
...
- **It's the foundation of the sandpainting track.** Faint samples summed additively on a [`noClear`](../Drawing/Accumulation.md) surface need float precision to accumulate without quantizing away, and a tone-map to bring the built-up light back to the screen. The depth-of-field particle rendering on the roadmap builds directly on this.
