...
[linear light](../Drawing/HDR.md), so grades and blends are physically correct. Every family is a segment of the `Effects/FilterCatalog` contact sheet
...
and the others transform the whole layer. `Effects/FilterCatalog`'s design family shows six of
...
- **`.light(reach:brightness:bounces:sky:quality:)`** light a flat scene: the base is what light meets (its alpha stops a ray), the aux is what gives light off, and the result is the light arriving at every pixel. Shadows sharp at the shape and soft further away, falloff, beams through a gap, and the color a lit wall gives back all come out of the one measurement, and the cost does not follow how much was drawn. See [Light in a flat sketch](Light.md) and the `Effects/Light` example.
...
and the composed chains; `Effects/MeshGradient` shows the mesh gradient.
...
See `Examples/Effects/GeneratorCatalog` (the fields family; its chains family has
...
Unlike `makeRenderTarget()` (a per-frame handle), a `Feedback` is **persistent**, so make it once in `setup()` and hold it. Its identity is what ties this frame's write to last frame's read, so make a fresh one each `draw()` and it never builds up.
...
- **`.fallingSand(passes:friction:)`** the falling-sand automaton, the toy every sand game is built on: every cell is empty, water, sand, or wall, and gravity is a local rule. A grain of sand drops into an empty cell below it, sinks through water (lifting the water into its place), and when it cannot drop straight down it rolls into an empty diagonal below, so sand builds into heaps with a slope of its own; water spreads sideways into empty cells, so a pool levels out and fills a basin; walls never move, and the field's edge is a closed box. `friction` (0 to 1) is the chance a grain stays put instead of rolling: 0 slumps every heap to the flattest slope the rule allows, higher lets heaps stand steeper, and near 1 sand stacks into towers. The field starts empty, so **draw to fill it**: a mark's brightness picks the material it lays down (black erases, a third gray is water, two-thirds gray is sand, white is wall), and `SandMaterial` names those levels, so a sketch writes `fill(SandMaterial.sand.color)`; a mark held down is a tap that keeps pouring. The raw `image` reads back the same four levels, made for `.gradientMap` with one color per material. Each pass settles the field in 2x2 blocks, the block grid walking through its four positions over four passes: a grain falls one cell per two passes and water spreads one cell per pass, so `passes` (kept a multiple of four, default 16) is the pacing dial. Pour a *thin* stream: sand landing on top of a heap can only leave it from the heap's edges, so a stream that lands more grains than those edges can shed piles up under itself, the way a torrent does. See the `Simulation/Automata` example (its sand rule).
...
`compose { }` is the declarative form of everything above. Instead of making each off-screen layer, filtering it, and compositing it back by hand, you declare the whole stack as one block: each `layer { }` is a drawing, its `.post(...)` filters, and the `.blended(...)` mode it composites with. The intermediate layers are managed for you.
...
- `.scale(_:)` renders the layer at a fraction of the canvas resolution (default `1`), like [`makeRenderTarget(scale:)`](#rendertarget), and drop it for a layer a blur or glow will soften anyway.
...
Build the helper with `aside { }` (the same as `layer { }`, named for how it's used, taking the same `.post(...)` and `.scale(...)` modifiers, though its `.blended(...)` is unused since it never composites) and hand it to a layer's combine modifier:
...
- **`.streaked(along:length:field:)`** brush the layer along the aside read as a direction field (line integral convolution), `field` choosing how the aside is read.
...
They interleave with `.post(_:)` in call order, and an aside can itself carry filters (a blurred mask edge, a softened displacement map). See the `Effects/Aside` example for a displacement map and a spotlight mask in one scene, `Effects/FlowStreaks` for a grainy picture brushed along three kinds of field, and `Effects/Defocus` for racking focus through a depth map.
