...
- **`.bloom(threshold:amount:radius:)`** glow, where pixels brighter than `threshold` bleed light into their surroundings. Ollin extracts the bright parts, blurs them by `radius` pixels, and adds them back at `amount`. The radius is a pixel count, not a share of the canvas: `radius: 0.02` spreads nothing, and Ollin says so once when a sketch writes one. For a glow that keeps its size on any canvas, write `radius: width * 0.02`. The result is the original **plus** its glow, ready to composite, often additively. Brightness here is the **max color channel** (HSV "value"), not luminance, so a vivid full-brightness mark blooms the same whatever its hue. `threshold` runs `0…1` over the linear-light frame, so HDR highlights (values above 1, from additive light) bloom hardest.
- **`.halation(threshold:radius:tint:amount:)`** the fringe film wears around its brightest highlights. Light that gets through the emulsion reflects off the base and exposes the layers again around the point it entered, and it exposes the red-sensitive layer most, since that one sits deepest. The pixels above `threshold` (the max channel over the linear-light frame, as `.bloom` reads it) are blurred by `radius` pixels, colored by `tint` (a warm orange by default), and added back at `amount`. What sets it apart from a glow is where the halo lands. Scattered light changes nothing in a layer that is already fully exposed, so each channel takes the halo in proportion to how far it sits below white. A white core stays white and wears a warm ring, a mid-tone beside a highlight takes the whole tint, and a frame with nothing above the threshold comes back byte for byte. The halo is the blurred highlight times the tint. So `tint: .white` keeps each highlight's own color in its fringe, which is the look a saturated scene wants, where the default dyes every fringe the same orange-red. See the `Effects/FilmLook` example.
...
- **`.edges(amount:)`** Sobel edge magnitude, drawn as bright edges on black. It is a quick ink or outline pass.
...
- **`.hatching(spacing:length:directions:foreground:background:)`** the picture as pen work whose strokes follow it. The direction at every pixel is read from the structure tensor, the same flow `.brushwork` and `.shock` follow, so the marks bend around a form instead of running across it the way the fixed screen of `.crosshatch` does. How many go down is the picture's own tone: the ink covers about as much of the paper as the picture is dark, so the drawing holds its light and shade with nothing but line.

  `spacing` is the distance between strokes in pixels, and so how fine the pen is. `length` is how far one runs before it ends, with a short length making a stipple of dashes and a long one a comb. `directions` is how many layers stack as the tone darkens: 1 hatches one way only, which turns the deepest tones into fat merged marks, 2 crosses the first layer at right angles once it has laid all it may, and 3 lays a third between them. The picture is read as the colors a display shows over white paper, so empty space on a transparent layer counts as paper and is left bare, and a transparent `background` keeps it that way in the result. The cost grows with `directions` times `length`. It runs as three passes, which the [web page](../Output/Web.md) export does not carry. See `Examples/Effects/Hatching`.
...
- **`.shock(iterations:flow:radius:smoothing:threshold:)`** the picture flattened into regions with crisp edges that follow its own flow, the coherence-enhancing filter. Each round smooths the layer along the direction of least change at every pixel and sharpens it across that direction with a shock filter. Where the brightness across an edge bends toward bright, a pixel takes the brightest pixel within reach; where it bends toward dark, the darkest. So a soft transition snaps to a step, and everything along an edge is drawn out into one coherent stroke. No color is invented: every pixel is a blend, along its own contour, of colors the layer holds.

  `iterations` is how many rounds, and so the level of abstraction: 2 is a light clean-up, 10 a poster. `flow` is how far along the flow each round smooths, in pixels, adapted to how clear the direction is (a clear straight edge gets the whole reach, a flat or curved neighborhood a quarter of it). `radius` is how far across an edge the shock reaches, which sets the edge scale, and anything thinner than it is fattened to it. `smoothing` blurs the brightness the shock reads its sign from, so texture finer than it stops making edges and flattens into regions; raise it on a noisy or grainy picture, where the flow would otherwise follow the noise into a maze. `threshold` is the bend under which nothing is sharpened, which keeps nearly flat regions quiet. The picture is read as the colors a display shows over white paper, so empty space on a transparent layer counts as paper, and the result keeps the layer's alpha. The cost grows with `iterations` times `flow`.
- **`.median()`** a 3×3 median, knocking out speckle and stray pixels while keeping edges sharp.
- **`.contour(levels:amount:)`** dark iso-brightness lines (one every `1/levels` of the range), turning tone into a topographic map.
...
- **`.normalMap(amount:)`** read the image as a height field and output its surface normal as an RGB vector, which gives the bluish bump-map look. The result is ready to feed `.displace` (see [combine](#combined)) or a lighting pass.
...
layer.filtered(.shock(iterations: 5))
layer.filtered(.hatching(spacing: 5, directions: 2))
...
- **`.scanlines(count:amount:)`** darken alternating horizontal lines, the CRT look. `count` is how many lines span the height.
...
- **`.predatorPrey(halfSaturation:predatorGrowth:predatorDeath:steps:)`** two populations on one land, the Lotka-Volterra idea in the form Rosenzweig and MacArthur gave it. Prey breed toward the land's capacity. Predators eat them at a rate that saturates when prey are plentiful, then die off at their own rate. Both wander by diffusion. The field rests at full prey and no predators, so **draw green to release predators**. A mark's red sets the prey level and its green the predator level, both read in linear light like every sim's marks, and a black mark clears both. Where predators land on full prey the two chase each other around a cycle. Neighbors run a little behind each other, so the cycle becomes waves: an invasion front, then spiral waves curling through its wake. The defaults sit past the model's oscillation threshold, the regime that makes spirals. Raise `halfSaturation` toward 1, or `predatorDeath` toward `predatorGrowth`, and the cycle calms into a steady coexistence and the waves die out. `steps` is the pace, integration steps a frame. The raw `image` reads prey in red and predators in green, already a picture. A `.gradientMap` reads the mix by brightness, so one ramp paints bare soil, meadow, and the predators' color in turn. Wave size follows the field `scale`. See the `Simulation/PredatorPrey` example.
...
- **`.smoothLife(radius:birth:survival:intervalSoftness:cellSoftness:)`** SmoothLife, Rafler's generalization of the Game of Life to a continuous field, the rule the smooth glider lives in. A cell is a disc rather than a texel, and its neighborhood is the ring around it out to `radius`, with the ring three times the disc's width. Each step reads how full the disc is and how full the ring is. Life's birth and survival counts become two intervals of ring filling. A dead cell is born when its ring's filling lands in `birth`, and a live one survives when it lands in `survival`. The edges of those intervals are soft steps, `intervalSoftness` wide, and `cellSoftness` is how gradually a half-full disc counts as alive. The defaults are the paper's glider regime: blobs that hold their shape and slide in any direction, rings that pulse, colonies that split. Draw white to add living matter. The classic glider seed is a disc a little under the radius with a notch bitten out of one side, and black erases. Each step reads every texel within `radius`, so the field `scale` is the cost lever. A glider is about two radii across, so `scale` also sets its size on the canvas. The `image` is grayscale life, made for `.gradientMap`. See the `Simulation/Automata` example (its smooth rule).
...
- **`.forestFire(growth:lightning:neighborhood:)`** the Drossel-Schwabl forest fire, the automaton that made self-organized criticality visible. Every cell is empty, a tree, or burning. A burning cell is empty next step. A tree catches from any burning neighbor, and otherwise catches on its own with probability `lightning`. An empty cell grows a tree with probability `growth`. Nothing in the rule aims at a density, and one arrives anyway: trees fill in until a stand is connected enough for a strike to run through it, the fire clears exactly that crowd, and the field hovers there forever, throwing fires of every size from a single tree to most of the map. Keep `lightning` far below `growth`, because the ratio between them is what sets how big fires get, and a rate near `growth` burns every tree as soon as it grows. The field **needs no seeding**: it starts bare and grows itself in. The coin each cell throws is a hash of the cell and the field's own frame count, so a run replays exactly. Drawing stamps states, where white sets cells burning, mid-gray plants trees, and black clears a firebreak the flames cannot cross. The raw `image` is grayscale (bare black, trees mid, fire white), made for `.gradientMap` with a ramp running dark ground to green to hot. `.moore` spreads fire through diagonal gaps and reads rounder than the classic four. One texel is one cell, and edges wrap. See the `Simulation/Automata` example (its forest rule).
...
- **`.wireworld()`** Silverman's Wireworld, the automaton that computes. Every cell is empty, a conductor, an electron head, or an electron tail. A head becomes a tail, a tail becomes conductor, and a conductor becomes a head when exactly one or two of its eight neighbors are heads. That last clause is the whole machine. One or two lets a signal run down a wire (the tail behind it cannot be re-lit, so it never runs back) and split at a fork, and three or more stops it, which is what a diode and every gate are built from. A loop of wire with one electron on it is a clock, a wire tapped off the loop carries its pulses, and two wires that join make an OR. The field starts empty, so **draw the circuit**. A mark's brightness picks what it lays down, snapped to the nearest of the four states: black erases, a third gray is conductor, two thirds a tail, and white an electron head. `WireworldCell` names those levels (`.empty`, `.conductor`, `.head`, `.tail`), so a sketch writes `fill(WireworldCell.conductor.color)` and `.head.color`, and the usual way in is a circuit typed as a few lines of text, one character per cell, stamped with `drawRect` on the first frame. Make the cells large enough to read (a low field `scale`), because a circuit is a picture of its wiring. The raw `image` reads back the same four levels, made for `.gradientMap` with a color per state, and the classic picture is yellow wire with blue heads and red tails on black. The eight-cell neighborhood lets an electron cut a ring's corners, so a nine-by-five ring is a clock with a period of twenty. Edges wrap. See the `Simulation/Automata` example (its wire rule).
- **`.schelling(preference:vacancy:mobility:passes:seed:)`** Schelling's model of segregation, the board that showed how a mild preference sorts a neighborhood. Every cell is empty or holds an agent of one of two kinds. An agent is content when at least `preference` of its occupied neighbors are its own kind, and one that is not moves to an empty cell nearby, taking a cell where it would be content when there is one within reach and stepping toward one otherwise. Nobody on the board wants to live apart, and it sorts itself into patches anyway, more sharply than anyone on it asked for. That gap between what each agent wants and what all of them get is the result the model is famous for. The field **needs no seeding**. It starts as a seeded random mix of the two kinds with `vacancy` of the cells empty, because a sorted board is a fixed point, and the same `seed` replays the same run. The empties are where the movers go, so a board with few of them jams. `preference` is the demand: at 0 nothing moves, at 0.3 the board sorts into patches, at 0.5 it sorts hard. `mobility` is the pace, the chance an unhappy agent moves in a pass: at the default a board takes a couple of seconds to settle, which is the sorting you can watch, and at 1 every unhappy agent with somewhere to go moves at once. `passes` is how many moving passes run per frame. Each lets an agent step within an eight-cell block, and the blocks walk from pass to pass, so an agent can reach any empty cell within seven. Drawing stamps states, snapped to the nearest of three levels: black empties the cells it covers, mid gray places the first kind, white the second (`SchellingCell` names them: `.empty`, `.first`, `.second`), and the board sorts around the mark. The board has edges, as Schelling's did, so an agent at the edge has fewer neighbors, and one with no neighbors at all is content. The raw `image` is grayscale, empty black, the first kind mid, the second white, made for `.gradientMap` with a color per kind. See the `Simulation/Automata` example (its schelling rule).
- **`.ising(temperature:field:sweeps:seed:)`** the Ising model, the oldest model of a magnet and the simplest system with a phase transition. Every cell is a spin, up or down, and each spin wants to agree with its four neighbors. The field runs Metropolis Monte Carlo: a spin whose flip would lower the energy flips, and one whose flip would raise it flips anyway with probability `exp(-change / temperature)`, so heat shakes the order loose. Cold, the field magnetizes into domains that coarsen until one wins; hot, it is noise; at the critical temperature between them, `Sim.isingCriticalTemperature` (about 2.27, the default), clusters appear at every size, the famous picture of a phase transition. One sweep visits every spin once, as two checkerboard passes: a pass updates the cells of one color from the other color's spins, so no spin reads a neighbor that is changing under it. The field **needs no seeding**. It starts as a seeded random mix, the field at infinite temperature, and cooling it is the picture; the coins are a hash of the cell, the pass, and the seed, so the same `seed` replays the same run. `temperature` is the heat in units of the coupling between neighbors: at 0 only a flip that lowers or keeps the energy happens and the rule is deterministic, around 1 the domains are clean and coarsen slowly, at 4 the field is nearly noise. `field` is an outside pull on every spin, positive up and negative down, -4 to 4, where 4 overrides any neighborhood. `sweeps` is how many sweeps run per frame (1 to 32), the pace. Drawing stamps spins, snapped to two levels: white magnetizes a patch up and black flips it down (`IsingSpin` names them), and the field answers the patch. Edges wrap. The raw `image` is black for down and white for up, made for `.gradientMap` with a color per spin. See the `Simulation/Automata` example (its ising rule).
...
`compose { }` is the declarative form of everything above. Instead of making each off-screen layer, filtering it, and compositing it back by hand, you declare the whole stack as one block. Each `layer { }` is a drawing, `.post(...)` are its filters, and `.blended(...)` is the mode it composites with. Ollin manages the intermediate layers for you. The block is a result builder, `ComposeBuilder`, which is what lets it read as a sequence of declarations with `if` and `for` in it rather than as an array.
