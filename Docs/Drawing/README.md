...
- [`Clothoid`](./Clothoid.md) - the curve whose bend grows at a steady rate (the Euler or Cornu spiral): the easement that joins a straight run to a turn with no kink, the single curve that fits two points and two headings, a smooth path through waypoints, and polyline corners a vehicle could take
...
- [`Hyperbolic tiling`](./HyperbolicTiling.md) - regular tilings of the hyperbolic plane in the Poincaré disk: `hyperbolicTiling` (any {p,q} with (p-2)(q-2) > 4, typed tiles with depth and a two-coloring parity, and a `viewpoint` that pans across the endless tiling)
