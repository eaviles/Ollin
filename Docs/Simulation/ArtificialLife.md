...
A few kinds of particle, and one attraction-or-repulsion number for each ordered pair of kinds. From that alone come membranes, cells, chasers, and worms. It is Jeffrey Ventrella's *Clusters*, the widely copied "particle life". Every particle feels a short-range shove away from whoever is too close. Past a `beta` fraction of the interaction radius it feels an attraction or a repulsion instead, set by the matrix entry for the two kinds. Asymmetry in the matrix (red chases blue, blue flees red) is what keeps it alive.
...
The defaults are the paper's canonical set, `alpha = 180°` and `beta = 17°`. The `speed` default is the paper's, scaled from its `radius = 5` up to yours. Set all three as `pps.alphaDegrees`, `pps.betaDegrees`, and `pps.speed`. Density matters. `PPS.suggestedCount(for:in:)` picks a `count` that puts an open particle near the paper's 13 to 15 neighbors. That is where the cell-forming regime lives.
...
The two force terms pull against each other, and each does one job. **Growth is the only attractive term.** **Repulsion is the only term with an opinion at very short range.** The kernel is a *ring*, so two particles in the same place contribute almost nothing to each other's field. Growth on its own is perfectly happy to let them coincide.

There is no kernel weight to set, because it is not a free parameter. The weight is whatever makes the kernel integrate to one over the plane, so Ollin derives it from `muK` and `sigmaK`. That is what keeps `muG` meaning the same crowding when you move the ring. At the published ring it comes out as the 0.022 the paper prints.

Particles start packed in a disc at the middle, about one per unit of area. Scattered over a whole canvas instead, most would open outside everyone's kernel with nothing to organize with. The first thing you would see is a long minute of nothing.
...
A **recipe** is eight numbers. Three of them are how far a particle sees, the speed it likes, and the speed it can reach. The other five are the strengths of cohesion, alignment, separation, random steering, and pace-keeping. The world opens with a handful of random recipes shared out evenly, and from then on they compete.
...
`competition` is what "doing well" even means here, and it is the whole character of a run. Setting `transmits` to false gives you the model before the evolutionary layer. That is a fixed heterogeneous mixture, and it is worth seeing on its own.
...
- **Cost is neighbor density.** The hash makes the per-frame cost scale with the number of *near* pairs, not with all pairs. A larger `radius` gives denser neighborhoods, so it costs more than a larger `count` alone.
