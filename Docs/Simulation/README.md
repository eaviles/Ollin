...
- [`Crowds`](./Crowds.md) - `Crowd`, walkers that make room for each other, on the CPU. Each takes the velocity nearest its wish that keeps it clear of every neighbor for the next second, trusting the neighbor to do half. Obstacles and walls hold even in a jam, opposing streams sort into lanes, and `isJammed` marks the pressure.
- [`Coupled oscillators`](./Oscillators.md) - `Kuramoto`, a crowd of oscillators each running at its own pace and pulled toward the others, on the CPU. Below the critical coupling they drift apart; above it a locked group forms and the crowd falls into step. Read `coherence` to watch it happen, give it a `range` for a ring, lay it on a square or hex lattice or a graph of your own so patches lock and waves cross the field, and read `localCoherence` for where.
