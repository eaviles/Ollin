...
Rigid bodies live inside the 3D scene. Crates stack and topple, balls roll, and chains swing, all with real contact response. This is the 3D version of the [2D physics world](Physics.md)'s rigid side, and it works the same way. You build a [`World3D`](#world3d) once, add [`Body3D`](#body3d)s to it, step it each frame, and draw each body from its pose. Both worlds live in the same satellite, so `import OllinPhysics` brings you both.

Three other things live in the world beside the bodies, and none of them is a rigid body.

- A [`Character3D`](#characters) is a walking figure. You steer it from `draw()` instead of pushing it around with forces.
- A [`Vehicle3D`](#vehicles) is a chassis on sprung wheels that you drive.
- A [`SoftBody3D`](#softbodies) is a mesh whose vertices are simulated, so it drapes and squashes.

The solver behind all of this is [Jolt Physics](https://github.com/jrouwe/JoltPhysics). It is vendored and wrapped the same way Box2D backs the 2D side, so none of it shows up in the API.
...
A leaning tower of crates settles into a stable stack. `withBody(_:)` moves the 3D transform stack to a body's position and orientation, so whatever the block draws follows the body.

Distances are the 3D scene's world units, y-up, the same units the camera uses. The solver works in meters, and it behaves best with bodies roughly 0.1…10 units across. That is the scale the 3D examples already draw at. If your scene is built larger, `unitsPerMeter` rescales the conversion between the two.
...
`ground` is a wide static slab whose top face sits at the level you give it. The default is `nil`, which lets bodies fall forever. `advance(by:)` clamps the step to `maxTimestep`, which is 1/30 s, so a stalled frame cannot launch the scene. It runs one collision pass for every ~60 Hz of simulated time.

You manage bodies and joints the way the 2D world manages its own:
...
You read and drive a body through its pose:
...
`userData` is a free slot for anything the sketch wants to attach to a body, such as its color or its mesh. `collider` keeps the shape the body was created with, so a drawing loop can match a mesh to it without keeping a second array alongside.

<a name="motion"></a>

### Motion parameters

You can tell a body three things about its own motion. Each one is available when you add the body, and each one stays settable while the sketch runs.
...
**`freedom`** says which of the six directions the body may use. Three of them travel along a world axis, and the other three turn about one. Everything is free by default.
...
**`gravityScale`** is how strongly this one body answers the world's gravity. `1` is the same as everything else, `0` is weightless, and a negative value makes the body rise.
...
**`checksPath`** sweeps the body's shape along its whole path each step, instead of only testing where it ends up. It is off by default, and it is worth turning on for anything small and fast. A fast body can cover more than its own width between two steps. It is then in front of a thin wall at one step and past it at the next, without ever having touched it.
...
The check costs nothing while the body is slow. It only runs once a body covers a good fraction of its own size in a step. An ordinary throw therefore lands on exactly the same spot either way. What it does cost is a little accuracy in speed. A body that hits something at pace gives up the rest of its step where it struck. This is continuous collision detection, under a name that says what it does. Two neighbors answer the same problem themselves and need nothing turned on. A `Character3D` already tests its path as it walks, and a `Vehicle3D`'s wheels feel for the road by casting rays. So on a vehicle it is the chassis, `vehicle.body.checksPath`, that would want it if anything did.

<a name="collider3d"></a>

### Collider3D

A collider is the local shape of a body, centered on the body's origin. Position and orientation come from the body itself.
...
The upright shapes all stand along the body's y axis, so rotate the body to point one somewhere else. Their `height` matches the matching draw call (`drawCapsule`, `drawCylinder`, `drawCone`), so the draw call takes the collider's own numbers. The two tapered shapes slope between two radii. A `taperedCylinder` keeps flat caps, so equal radii make a plain cylinder, and a zero top radius is exactly `.cone`. A `taperedCapsule` rounds both ends with caps of different sizes, and both of its radii must be positive. A `.mesh` collider is for scenery, such as a loaded set piece or an exact sculpted form. It has no volume to take a mass from, so a dynamic body created with one is pinned in place. For a shape that moves, use `.hull`, a primitive, or a `.compound` of them.

<a name="compound"></a>

### Compound bodies

`.compound` fuses several colliders into one rigid body, such as a hammer, a table, or a windmill's blade cross. Each part is a child collider placed in the body's local space. The body's mass, balance, and inertia come from the whole assembly, so a lopsided tool tumbles the way a lopsided tool should.
...
`.part(_:at:rotated:axis:density:)` places any collider at a position and rotation inside the body. Everything defaults to the origin with no rotation, so a single offset part is also how you shift a shape away from its body's origin. A part's `density` is relative and multiplies the body's own, which is what makes the hammer's head heavy against its handle. Parts can nest, so a compound can hold another compound, and every part should be a solid shape. A `.mesh` or `.heightfield` part pins the body in place, the same as using one on its own.

Draw a compound the way you built it. `withBody` poses the whole body, and then you translate and rotate to each part's pose and draw its shape. The [`3D/Physics/Windmill`](../../Examples/3D/Physics/Windmill/) example does exactly that with a small recursive helper.

<a name="terrain"></a>

### Terrain and scenery

`.heightfield` turns a [`Heightfield`](../Generators/Terrain.md) into solid ground, sized exactly like its `mesh(width:depth:height:)`. That is a `width` × `depth` grid centered on the body's origin, and each sample is lifted to `height · value`. The collider and the drawn mesh trace one surface, so what rolls matches what renders:
...
The field is resampled onto a square power-of-two grid for the solver, so any grid shape works. That grid is at least as fine as the source resolution, and it is capped at 1024 samples per side. There is nothing beyond the field's edges, so bodies roll off into empty space. Like `.mesh`, a heightfield can only be static.
...
It walks the node tree and adds one static mesh body for each mesh node. The node's world transform is baked into the triangles, including nested groups and authored rotations and scales. Colliders take each mesh as authored, at rest, so skins and morph targets are not posed. `friction:` and `restitution:` apply to all of them, and you still draw the scene itself with `drawScene(_:)`.

<a name="joints"></a>

### Joints

`connect` links two bodies with a `Joint3D`. Anchors and axes are world coordinates, read at the moment you connect.
...
Two more kinds, [gears and a rack and pinion](#links), link one joint to another.

<a name="motors"></a>

### Motors, limits, and springs

Hinges and sliders can do more than swing free. You can bound their travel, power them, and put springs on their stops. Together those make doors that close themselves, windmills that turn, and drawers that stop at the end of their rails.

**Limits** are set on the joint kind itself. Both kinds measure from the pose at the moment of connecting, and that pose counts as 0, so the range must straddle zero:
...
**Motors** are two calls on the `Joint3D` that `connect` returns, one for each intent:
...
`drive(to:)` is a spring servo. `frequency` is how fast it pulls, where 2 is a slow door closer and 20 is a quick robot servo. `damping` at 1 settles cleanly, and a lower value overshoots and bounces. Both forms take `strength`, which caps the motor's torque in N·m or its force in N. The default is unlimited, so the motor simply reaches its target. A small value gives the motor something it can lose against, such as a door closer that a rolling ball can push through:

```swift
door.drive(to: 0, frequency: 1.2, strength: 60)
```

Driving is stateful. Set it once and the motor keeps pulling every step, until you call `stopMotor()` or another `drive`. To read where the joint sits right now, use `door.angle` in radians or `drawer.offset` in world units. Both are 0 at the connect pose.
...
`friction` is what makes a stiff old hinge. It is a constant resistance the joint's motion has to overcome while no motor is powering it. It is also what winds a spinning wheel down after `stopMotor()`. `softenLimits` swaps the hard stops for springs, so a gate thrown against its limit gives a little and bounces back. A `frequency` of 0 restores the hard stop. The other joint kinds have no axis to power, so a motor call on a `.ball`, `.distance`, or `.weld` notes once and does nothing. `.distance` has a spring of its own, the `stiffness` on that case.

A `.swingTwist` joint takes both motor calls too, plus one more of its own. A single number given to a joint that bends in every direction can only mean the roll about its own axis. So `drive(at:)` and `drive(to:)` run the twist and leave the bend alone. `joint.twist` reads that roll back, and `joint.angle` reads the bend. To point the bone somewhere, say where:
...
`drive(toward:)` pulls the joint's axis onto a world direction, rolled `twist` radians about itself. It is clamped to the joint's own cone and twist limits. Aim past the cone and the bone leans as far as it may. Set it each frame to follow something.

<a name="morejoints"></a>

### Tracks, ropes, and freedoms

Three more kinds each do something no hinge or slider can.
...
**`.pulley` is a rope over two hooks.** One end sits on each body. The total length of the rope is what ties them together, so one side rising means the other side falling:
...
Read the call as written: from the tray, over the left hook, across to the right one, and down to the weight. `ratio` is how many falls of rope hold the second side. A ratio of 2 is a block and tackle, so that side moves half as far and lifts twice as much. A rope resists being pulled longer, but it does not resist going slack, which is what lets both ends drop together. `taut: true` makes it a rigid linkage instead, so lifting one end drives the other one down. Both ends must be an ordinary or a static body. The solver reads a kinematic body wrongly here, so that case is refused with a note. Move a static end instead.
...
The freedoms are the same `Freedom3D` set a body's own [motion parameters](#motion) use, given in world axes at the moment of connecting. `travel` bounds every direction the body may move in, and `rotation` bounds every axis it may turn about. Both are measured from the connect pose, and leaving either one out runs it unbounded. `.allowing([])` is a weld, and `.allowing([.turnX, .turnY, .turnZ])` is a `.ball`. A single turn with a range is a hinge, which is a good way to see what the named kinds are made of.

<a name="links"></a>

### Gears and racks

The last two kinds link *joints* rather than bodies, because what they tie together is the motion those joints allow. Build each part's own joint first, and then connect the joints:
...
Both links take a hinge as their first joint. The second joint is a hinge for a gear, or a slider for a rack and pinion, and anything else notes once and does nothing. Each joint's moving part is the body that is not static. When both bodies can move, the moving part is the second body the joint was connected with. That matches the order every `connect` call is written in, with the frame first and the part that turns second. The shapes themselves never touch, so tell meshed wheels not to collide, or their plain cylinders will jam:
...
A `Contact3D` carries the pair, `a` and `b`, always in the same order. That order never means one of them is the body that moved. It also carries where they met in `point`, and the `normal` pointing from `a` toward `b`. `speed` is how fast they were closing when they met. It is measured *before* the solver answers the collision, so it reads the force of the impact rather than what survived the bounce. That is what you want for the volume of a clink or the size of a spark. Two helpers save you from working out which body is yours:
...
Contacts are reported per body **pair**. A crate resting on a mesh floor touches it along many triangles, and a compound body touches on several of its parts. Either way that is one `began` when it lands and one `ended` when it lifts.

An `ended` contact carries only the pair. By the time the solver notices a touch is over there is nothing left to measure, and the other body may already have been removed. So `point`, `normal`, and `speed` are all zero there.

The two sides are typed `any Colliding3D` rather than `Body3D`, because either one may be a [soft body](#softbodies). A cloth is a real thing in the world, and it lands on floors and sails into sensors. There is nothing to push it with, though, and no single pose to read, which is what the type says. `involves`, `other(than:)`, and `===` all work either way. Cast to the concrete kind when you want to act on it:
...
You can also ask each body directly, out of the same list:
...
`touching` has one thing worth knowing. The solver lets a settled body sleep, and a sleeping body stops reporting contacts. A stack that has come to rest therefore reads as touching nothing. That is the right answer for events, because nothing is happening, but it is a surprising one when you want to know what is there. When the question is what is resting in this region, use a sensor, which stays awake.

<a name="sensors"></a>

### Sensors

A sensor is a region rather than a solid. Things pass straight through it, and it reports each one.
...
A sensor sets its own motion. It is kinematic and never sleeps, whatever `kind` you asked for, so it goes on reporting bodies that have settled and fallen asleep inside it. That is what makes a tray or a pressure plate work:
...
A sensor never falls, so it stays where you put it. Move one by setting its `position`. To make a detector follow something, drive it from that body's pose each frame. Sensors are also invisible to `body(under:in:)` and `grabBody(at:in:)`, so the cursor's ray looks straight through them to the solid scene behind.

Sensors detect *moving* bodies, which means dynamic and kinematic ones. They do not detect static scenery, and they do not detect each other. So a trigger volume laid over the ground does not spend every step reporting the ground.
...
**A group collides with itself** until you say otherwise, so two crates in one group still stack. Naming the same group twice is how confetti falls through its own drift:
...
Each of them can also be moved between groups while it runs, through `body.group`, `character.group`, `vehicle.group`, `ragdoll.group`, and `softBody.group`. A rule written after things have already settled on each other still applies. The bodies are woken so the solver looks at the pair again.

A separate mechanism keeps a figure's own limbs from fighting each other, and groups never touch that mechanism. So two ragdolls in one group still collide with each other exactly as they should.
...
**The bookkeeping.** A world holds `World3D.maxCollisionGroups` groups, which is 64. Naming more than that leaves the extras in `.default`, rather than quietly folding them onto a group already in use. `world.collisionGroups` lists the groups it knows, in the order they were named, which is how you find a misspelled name. A typo makes a *new* group, so a rule written about it does nothing.

The worked example is `Examples/3D/Physics/Sieve`, which sorts three colors of bead down one ramp. The sorting itself is three `ignoreCollisions` calls.

<a name="queries"></a>

### Queries

`contacts` reports what the solver noticed while it stepped. A **query** asks the world something else, between steps. It asks what lies along this line, what a shape would run into, or what is inside this region right now. Every query answers immediately, none of them changes anything, and none of them needs a body to ask with.
...
**Rays.** `raycast(from:to:)` returns the nearest body along a segment. `raycastAll(from:to:)` returns every body along it, nearest first.
...
**Sweeps.** `sweep(_:from:to:)` slides a collider along a line without turning it, and returns the first thing it runs into. `sweepAll` returns all of them. A ray asks what is in the way, while a sweep asks whether something *fits*. A clearance probe asks that question. So does a camera that must not end up inside a wall, and so does a step a character is about to take.
...
`rotated:` and `axis:` turn the probe, which changes what fits. A wide, thin box goes through a narrow gap edge-on but not flat. A sweep that starts out already touching something reports a `distance` of 0. `mesh` and `heightfield` colliders describe scenery rather than a probe, so you cannot sweep or overlap with one. Use a hull, a compound, or a primitive instead.
...
A [sensor](#sensors) answers the same question *continuously* from a body that exists in the scene, which is what a pressure plate or a goal wants. An overlap answers it once, anywhere, with a shape that never existed in the scene.

**What a query sees.** It sees solid bodies, including static scenery and the `ground` slab. It also sees the ones the world keeps out of `bodies`, which are a character's stand-in and a ragdoll's limbs. So you can spot a walking figure through `character.body`. The **soft bodies** are in there too. A hanging sheet stops a sightline the way a wall does, and the hit names the `SoftBody3D`. One thing is deliberately transparent, a **sensor**, because a detector volume is a region to be inside rather than a surface to hit. Pass `includingSensors: true` to have sensors reported as well. `ignoring:` takes anything you want to look straight through, which is how something casts a ray from inside itself, or through a curtain:
...
Three settings decide what the geometry does to the character, and each one has a limit you can see:
...
Whether a push actually shifts something depends on `pushStrength` against what the body weighs. A 4 kg crate on an ordinary floor slides under the default 100 N. A 43 kg one does not, because ground friction alone asks for more than that. Make crates light if you want them scattered.
...
A character is swept through the world by hand rather than simulated as a body, so it is not in the scene by itself. It carries a stand-in that is in the scene, `walker.body`, which is a kinematic `Body3D` moving inside the capsule. That stand-in is what makes the character visible to everything else on this page. Sensors see it walk in, `world.contacts` names it, and `body(under:in:)` can pick it:
...
A `Vehicle3D` is a machine you operate rather than a body you push. It is a chassis carried on sprung wheels, with an engine and a gearbox behind the throttle. You set four numbers each frame, and the wheels do the rest by finding their own grip on whatever they are rolling over.
...
The chassis is an ordinary `Body3D`. `car.body` collides, takes impulses, reports contacts, and sits in `world.bodies` like anything else the sketch added. What makes it a vehicle is the constraint on top, which owns the wheels. Its weight is `mass`, 1500 kg by default, rather than a weight worked out from the shape's volume. Its center of mass also drops to the height of the wheel mounts. That is what stops a car rolling over the first time it turns hard.
...
All four hold until you change them, so set them every frame. `drive(throttle:steering:brake:)` sets three of them at once, and `coast()` lifts everything off.

Asking for the other direction while the vehicle is still rolling brakes first. The vehicle takes the new direction only once it has stopped, which is how a car with an automatic gearbox behaves. Press back at speed and you get the brakes. Press it again from a standstill and you reverse.

#### The wheels

A `Wheel3D` says where a wheel is bolted on and what it does. You build the wheels and hand them over, and afterwards the same objects report where each wheel actually ended up:
...
**Wheels level with each other along the vehicle share an axle**, worked out from where they sit rather than from the order you listed them. The engine turns an axle that has any driven wheel on it, so marking one of a pair marks its pair too. A lone wheel, such as each of a two-wheeler's, is an axle by itself. Each full pair is also tied by an anti-roll bar, which is what keeps the vehicle flat through a corner. That bar is `antiRollStiffness`, 1000 N/m by default, and `0` unties the pair.
...
`withWheel(_:)` moves the 3D transform stack to a wheel's pose, steering and spin included, the way `withBody(_:)` does for a body. A tire modeled as a cylinder along +y then lands correctly.

Two more parameters sit on the vehicle itself. `maxTilt` caps how far the chassis may lean from upright. It is `nil` by default, so the chassis rolls over like anything else. A value around `.pi / 3` keeps a car on its wheels over rough ground. `wheelContact` picks how the wheels find the ground.
...
A two-wheeler is the same call with `balances: true`, which adds the controller that holds it up and leans it into turns. It needs one thing a car does not, which is a real `casterAngle` on the front wheel. The trail that comes with a raked fork is what lets it hold a line instead of falling over at the first correction.
...
Running dead straight, even an unbalanced two-wheeler stays up, because nothing tips it. The balancing shows the moment something does tip it. Start it leaned over and it stands back up, and it leans into a corner rather than falling out of it.
...
`isTracked: true` builds the same machine on two tracks. The wheels become road wheels. They are split into a left band and a right band by **which side of the hull they sit on**. The three controls mean what they always did:
...
`trackSpeed(.left)` and `trackSpeed(.right)` read how fast each band is running over the ground, in world units per second. They are equal in a straight line, they differ through a turn, and they run opposite ways in a pivot. So they are what you scroll a drawn track by. `wheels(on:)` gives you one band's road wheels, front of the machine first. That is the order a drawing loop walks to lay a track around them.

Most of what a wheel offers means the same thing on a band. These do not:
...
A `Ragdoll3D` gives a skinned figure weight. Hand `addRagdoll(from:)` a loaded `Scene` that has a skin. It reads the skeleton, builds one rigid body for each joint, and sizes each body from the part of the mesh that joint actually moves. Stepping the world then answers a question the animation cannot answer: where the limbs end up when the world has a say.
...
`scene.apply(ragdoll)` is `apply(_:at:)` run backwards. The simulated bodies pose the joints, instead of a keyframe track posing them. It is exact, because each limb's body stands *at* its joint rather than in the middle of the bone. Writing the pose back therefore has nothing to undo. Everything downstream works as it always did, including skinning, morph targets, per-material parts, materials, shadows, and export.
...
**Limp or powered.** Left alone, the joints have limits and nothing else, which is the figure that falls downstairs. `drive(toward:)` gives it some strength back. It puts a motor on every joint, and each motor pulls toward the pose a scene is currently holding:
...
Keep the target scene and the drawn scene apart. A `Scene` is a value type, so a second copy is one assignment. A figure driven toward the scene it was just posed from has nowhere to pull. Set the pose it is chasing again each frame.

`strength` is the most torque a joint may use, in newton-meters, and it is the parameter that carries the expression. Set it high and the figure will not be moved. Set it low and heavy limbs sag out of the pose, which is how a figure reads as tired rather than switched off. `goLimp()` cuts the power. `pose(from:)` puts every limb back where a scene has it, which is how you stand a figure up again.

Nothing drives the root, so a powered figure still falls as a whole. The motors hold its *shape* rather than its place. To keep one on its feet, make the root limb kinematic with `ragdoll.limbs[0].body.kind = .kinematic`, and the figure hangs from its hips like a puppet. Setting `ragdoll.kind = .kinematic` instead makes the whole figure follow the driven pose exactly, and it shoves whatever is in its way.
...
`drawSoftBody(_:)` draws `softBody.mesh`, which is the source mesh with the simulation's positions and freshly derived normals. Everything else the mesh carried comes through untouched, so a textured sheet stays textured while it moves. Every renderer feature applies exactly as it does to any other mesh, including materials, shadows, reflections, and export.
...
**Pinning is how a cloth is hung.** The `pinned:` closure is handed each vertex of the mesh, in the mesh's own local space. It returns whether that particle is held in place. You can also move pins afterwards:
...
`move(_:to:)` pins the particle if it was free, and it drives the particle there by velocity. The sheet hanging off it is then dragged along rather than snapped. `nearestVertex(to:)` finds the vertex nearest a world point.

**The parameters.** All of them are scale-free, so the same number means the same thing on a handkerchief and on a marquee.
...
**Pushing one about.** Impulses, joints, and grabs do not apply to a soft body. It has no single pose or velocity for them to act on. What works is `applyForce(_:)`, which is spread evenly over the particles. That is how you apply wind:

```swift
banner.applyForce(Vector3(0, 0, gust))
```

**Taking hold of one** is the soft-body version of `grabBody` and `dragGrab`. It works by pinning the particle under the cursor rather than by adding a joint:
...
- **A query can find it.** `raycast`, `sweep`, and the overlap calls all see soft bodies. A hanging sheet blocks a sightline, and a `Hit3D` may name a `SoftBody3D`. To look through one, name it in `ignoring:`, or put it in a collision group the query does not ask as.

**What a soft body still cannot do.** The solver collides soft bodies with the rigid bodies around them. It does not collide them with each other, and it does not collide one with itself. So a sheet folded double passes through its own layers, which reads as a flicker where the two lie together. Impulses, joints, and grabs do not reach a soft body, and `body(under:in:)` answers only for solids, so use `grabSoftBody(at:in:)` there. Tearing is not offered either. A real tear has to split a shared vertex in two and rebuild the surface, which the solver cannot do while it runs.
...
A soft body's shape is usually a surface. A rope's shape is a *curve*, and the difference is more than one dimension. A rope is built from **rigid rods** rather than springs, and every rod holds an orientation of its own. That is what lets other geometry travel along it.
...
- **`stiffness`** is how much the rope resists being *stretched*. `1` is a steel cable, measured on a rope hung under its own weight at about 1% longer than its rest length. `0.5` lets it stretch by nearly a third, and at `0.2` it has doubled, which is a bungee.
...
A long, finely divided rope is the case where `iterations` matters. Stiffness spreads one rod per solver pass, so forty particles over six units needs about twenty passes before `bend: 1` is really rigid. Twenty particles is stiff at the default five.
...
`withSegment(_:)` stands the transform stack in the middle of a segment with **+y running along the rope**. That is the axis Ollin's cylinders, capsules, and cones stand on. A primitive drawn inside the block lies along the rope with no turning of its own. It is the segment's version of `withBody(_:)`:
...
The clearest use is a chain, where every other link is turned a quarter turn about the rope's own axis. You can only express that turn because each rod knows how it is rolled:
...
**The pose the figure is standing in when you build the cloth is the bind pose**. Nothing has to be authored in a modeling tool, and no weights have to be painted. Hang the cloth where it belongs and name the joints, and every later pose is read as the motion since then. `carriedBy:` is given a vertex in the mesh's own space, which is the same space `pinned:` reads. It answers with a joint's name, or `nil` for a part that is ordinary cloth. A name the skeleton does not have is skipped with a note. A typo therefore leaves that part hanging free, rather than silently doing something else.

**`pinned:` means held by whatever holds it.** A pinned vertex that a joint carries is held to the *figure*. A pinned vertex that no joint carries is held to the *world*, which is what it has always meant. So the same closure clasps a cape at the neck and pegs a banner to a line.

Three more numbers shape what the rest of the cloth may do. All three are **lengths in world units** rather than ratios you have to calibrate:

- **`sway:`** is how far a vertex may travel from where the skeleton puts it, and you give it the same way you give `pinned:`. `0` holds the vertex exactly there, and `.infinity`, the default, leaves it free to swing. A leash of `0.05` really does hold every particle within 5 cm of its skinned position. Grade it tight at the shoulders and loose at the hem, and that is what tells a mantle apart from a cloak. Watch **the sign** if you compute a value, because a negative one clamps to `0` and hard-skins that part into a board.
- **`backStop:`** is how far *behind* the carried surface a particle may be pushed before it is held back out. It keeps a cape out of the back it hangs on without waiting for a collision. `0.04` is a few centimetres of clearance. Behind means toward what carries the surface, whichever way its mesh is wound, so a cape hung with its plane turned either way is held off the figure the same.
- **`maxStretch:`** caps how far any particle may get from what holds it, as a multiple of the distance measured *along the cloth*. `1` is inextensible, `1.05` allows 5%, and `nil`, the default, leaves the job to the springs. This one is worth knowing about even for cloth that no skeleton carries. A heavy sheet hung from one edge stretches under its own weight however stiff you make it, and a cap fixes that for almost nothing. Measured on a 2-unit sheet weighing 8 kg, the springs alone let it hang 6% long. `maxStretch: 1` hangs it at exactly its own length.
...
A world can have water the same way it has ground. It is one property, and nothing has to opt in.

```swift
world.water = Water(level: 0)
```

Everything already in the world starts floating. What floats and what does not comes from the `density` each body was built with, measured against the water's density. So it is a number you were already setting:
...
Some arrangements are worth keeping. A heap of stones may take four hundred steps to settle. A stack may be knocked into a shape you liked, or you may spend a minute nudging a scene into place by hand. None of that can be written down as code, and running the simulation again does not give it back. `snapshot()` captures the whole world as it stands, and `restore(_:)` puts it back:
...
This is also the honest answer about determinism. Simulating is reproducible within one build, but the solver runs in floating point. A toolchain that moves one last bit moves the last bounce, which a toppling stack magnifies into a different heap. A saved arrangement has nothing left to compute, so it comes back the same anywhere. Continuing from a restore is exact for the pose and the motion. The solver's in-flight contact bookkeeping is not carried, so a scene captured mid-collision may drift where a settled one cannot. That covers a vehicle's wheels too, because what a wheel is rolling on is worked out afresh each step. A machine saved parked comes back parked and stays exactly where it was put. One saved at full throttle carries on from the same speed in the same gear, and then it wanders. A stack of crates saved mid-collapse does the same.
...
`load(contentsOf:resolving:)` takes the same closure. `PhysicsAsset` covers the two things worth naming, `.mesh(_:)` and `.heightfield(_:)`. `snapshot.assetNames` lists what a snapshot will ask for, so a sketch can check a file before restoring it. How a heightfield is sized in the world travels in the snapshot, because its width, depth, and height are three numbers rather than geometry. Only the samples are named.

Measured on a world with a 65² terrain, a five-thousand-vertex mesh, and twenty crates, holding it all takes **105 KB**. Naming it takes **1.1 KB**. On a smaller scene the difference is smaller. The default is still to hold everything, because that is what makes a file you can commit beside a sketch and open anywhere.
...
Four rules are worth knowing, because each one comes from the schema rather than from a choice made here:

- **A collider's shape is the prim's own geometry.** There is no shape attribute. A `Cube` with `PhysicsCollisionAPI` on it is a box, and a `Sphere` is a ball. A scale on the prim makes a bigger shape, because a solver's shapes carry no scale of their own.
- **A rigid body owns everything under it.** Several collider prims in one subtree make one body with a compound collider. That is how a hammer is a handle and a head rather than two loose pieces.
- **A collider with no rigid body over it is scenery**, and it never moves.
- **USD stands a capsule, cylinder, and cone on z** unless the prim's `axis` says otherwise, while Ollin's stand on y. A rod authored the default way therefore arrives lying down, because that is what the file says.

**What does not come across.** Three things are written in the file's own vocabulary rather than one this world has. They are articulations, joint drives with their limit API, and collision groups with filtered pairs. All three are left alone. The stage's `upAxis` and `metersPerUnit` are not applied either, which is the same contract the rest of scene import keeps. A scene arrives as authored.

Reading is lossy, and that is exactly why this direction works. A file's notion of a body is a description, and anything it does not say has a sensible answer here. The same loss would not be acceptable on the way out. That is why a world you want back *exactly* goes into Ollin's own snapshot instead. The two are complementary. Import to pick up an arrangement somebody else authored, and snapshot to keep one you found.

The worked example is [`3D/Physics/Imported`](../../Examples/3D/Physics/Imported/), whose `yard.usda` is hand-written and readable. It holds a seesaw on a hinge, a stack, a hammer, and a sign on a hinge held to the world. There is also a capsule authored the default way, so it comes in on its side.

<a name="grabbing"></a>

### Grabbing with the mouse

You reach into the scene with two sketch calls, one to pick up and one to drag.
...
`grabBody(at:in:)` casts a ray from the active camera through the canvas point. It hangs the body it hits on a soft drag spring at the touched point, and remembers how deep into the view that point sat. `dragGrab(_:to:)` then moves the body in the screen-parallel plane at that depth, so dragging feels like sliding it across the glass. The probe without the pickup is `body(under:in:)`, which returns the body and the world point the ray touched. For world-space dragging without a camera, use the lower-level `world.grab(_:at:)` with the joint's `target`.
...
Under it, `drawCollider(_:)` renders any `Collider3D` at the current transform, and it covers every case. The primitive shapes draw as themselves. A `.mesh` draws as its mesh, and a `.heightfield` as its terrain. That terrain mesh is built on each call, so cache it yourself when it is large. A `.compound` recurses through its parts, and a `.hull` draws as a marker sphere at each corner. That closes the quiet hole a hand-written switch leaves, where a `default: break` makes an exotic body simply invisible.

A body may be drawn as something other than its collider, such as a mesh or a figure. For that, `withBody(_:)` wraps `withState` and moves the 3D transform stack to the body's pose. The block therefore draws in body-local space, and every renderer feature applies untouched, including materials, shadows, and export:
...
The simulation is deterministic within a build. The same setup stepped the same way reproduces exactly, which is what seeded variations need while a sketch runs. Exact poses can shift across toolchain rebuilds, so physics scenes are not pinned by pixel snapshots. The behavioral test suite pins the solver instead, and [a snapshot](#snapshots) is how you keep an arrangement for good.
