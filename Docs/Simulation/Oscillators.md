...
- **`rates`** is every oscillator's rate at the last substep, in radians per second: its natural frequency plus the pull in force on it, the number its phase was moved by. It equals `frequencies` until the first `advance`. Carry a phase forward by `rate * seconds` for a motion blur or a trail, so a mark smears along its own path across a shutter interval instead of strobing, or color a dot by how far its rate sits from its natural pace, which is where the crowd is pulling it.
