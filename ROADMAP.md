...
Two named additions sit at the front of this queue:

- **Light in 2D, through radiance cascades.** A global-illumination field for a flat scene: emitters and occluders painted into a layer, the bounced light read back as another layer, so a light dragged with the pointer spills around a corner and picks up the color of the wall it grazes. There is no 2D lighting in Ollin today, and most of what this needs is already in place, since the jump-flooded `Filter.distanceField` measures a drawn layer's signed distance and the direction to its nearest edge, `sampleRaw` hands both to a user shader unmangled, and the ping-pong `SimField` path can carry the cascade ladder.
...
- **Scene content seen through glass, without ray tracing.** `Material.glass` refracts the environment, and on a ray-tracing GPU it traces the real scene behind the surface. Everywhere else a glass object shows the environment where the objects behind it should be, so a bottle standing in front of a drawn scene stays empty. A screen-space pass over the already-rendered color target would fill that in on any GPU, offsetting the lookup by the refracted direction, with the per-channel offset the shipped dispersion filters already compute.
...
- **Shader authoring tools.** Two small pieces around the shader a sketch writes itself. A resolver for `#include` in shader source would let one shader pull in another, so a user shader could reach Ollin's own helper library by name instead of copying functions, and the built-in segments could declare what they depend on instead of relying on their position in a fixed list. A `check` command beside `ollin new` would compile a shader file against the real device and report what it found, both the errors at their true line and the parameters the shader declares, which is the loop a person editing a shader outside a running sketch currently has no way to close.
...
A third layer is missing between those two. Someone who wants the idea behind a thing rather than its signature or a chapter about it has nowhere short to go: a reader asking what a layer is, what a batch retains, or why drawing happens through a recorder gets four lines in the README, a page of reference that assumes the answer, or a Guide chapter that takes an evening. A small set of concept pages in [`Docs/`](Docs/), each one screen long and linking down into the reference and across into the Guide, would sit between them. It is worth writing before the website rather than after, since the website would otherwise present the same gap with better typography.
