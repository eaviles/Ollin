...
- **A tuned set of knobs written back into the sketch.** Knobs turned in the inspector hold their values in the running process and lose them at quit, so a good set is typed back by hand. Writing them into the `@Param` declarations keeps the sketch the artifact, and the machinery is the byte scanner a shape drag already uses, pointed at a property's default rather than a draw call's arguments. See the [design notes](DESIGN-NOTES.md#authoring-and-editor-tooling).
...
- **The reference, offline, from the CLI.** `ollin docs <topic>` and `ollin examples <filter>` serving the hand-written [`Docs/`](Docs/) pages and the examples set out of a local checkout, for anybody with no network or no wish to switch to a browser. It is also the piece the machine-readable index below would be built on, so it can go first.
- **A front door an assistant can read, if that is the right call.** Ollin's agent-facing files point inward: `AGENTS.md` and `CLAUDE.md` describe how to work *on* the framework, and nothing describes how to help somebody *use* it. The outward version of that is a small, well-understood set of conventions (a machine-readable index of the reference, the offline CLI reference above, the same served over a tool protocol) and most people arriving now work with an assistant, so the reference reaching it accurately is the difference between good sketches and invented API. It is listed as an open question rather than planned work, because it sits close to a line this project draws deliberately (see the boundary under [authoring and editor tooling](#authoring-and-editor-tooling)): serving accurate reference so a person can write their own sketch falls on the permitted side, and anything that starts producing sketches does not. Where exactly the line falls here is @eaviles's call, and it is better settled before the repository opens than after something ships under it. What waits on that call is only the part that points an assistant at the reference, since the offline half earns its place for a person either way.
