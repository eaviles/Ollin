...
Ollin is pre-1.0, and the public API still changes freely. Semantic version tags and a changelog begin with `0.1.0`, so a SwiftPM project has a version to pin before the API settles: at major zero a breaking change or a new feature bumps the minor and a fix bumps the patch, a consumer pins `.upToNextMinor`, and each release names its breaking renames in its notes. Version 1.0.0 is the release where the API settles: from that point on every public rename ships a deprecation shim (the migration discipline described in [`CLAUDE.md`](CLAUDE.md)). This section names what that release waits on, and only that. Everything else on this page stays open for work at any time: the release does not wait on it, and it does not wait for the release.

- **Stabilization.** A last naming sweep over the public API before the tag, since the shim discipline locks the names in at the release; the test suite reliable end to end, with no flaky or machine-bound failures; and the hand-verification backlog (features that still wait on eyes, ears, or hardware) worked down.
- **The release itself.** The release notes, the `1.0.0` tag, and a README presentation pass. The ecosystem legs that need a public repository (DocC, a Swift Package Index listing, the extension index) follow the release rather than gate it.
- **What an open repository is expected to carry.** The surfaces a visitor sees beside the README, none of them settable from a file: the About description and topics, a social-preview image (best rendered with Ollin's own export), Discussions switched on with a Show and tell category and a pinned welcome thread, and a logged-out check that the issues link and the install URL resolve. The domains wire in here too: ollin.art becomes the canonical home the About field names, with ollin.run, ollin.live, and ollin.mov as permanent redirects to it. All small, and cheaper to set before the first stranger arrives than after. See the [design notes](DESIGN-NOTES.md#opening-the-repository), which also cover what the README presentation pass has to decide.
...
- **Depth from video, deeper.** The metric checkpoint of the video depth model, so a webcam's depth comes in meters and lifts into an `RGBDFrame` and a point cloud, once its license is settled; and the encoder on the Neural Engine with only the temporal head on the GPU, for twice the readings a second.
...
- **A sketch on the web, as an exported page.** Not a browser runtime for the framework, which stays out (the platform stance in `CLAUDE.md` says why), but an export like video: `--export-web` records what a sketch draws over a duration and writes a page that plays it back on WebGL2, its shaders running live on time and the mouse, its parameters as controls. The page comes standalone (one self-contained file) or inline (a fragment for a page of your own). The 2D paths cross whole: the analytic primitives as instance records, strokes and fills as their triangles, text through the glyph atlas, images as bundled textures, every effect and user shader as a translated fragment, SDF scenes as data. Lit meshes, ray tracing, compute work, and pictures that arrive as a texture every frame go to the video export, and the exporter names the call that stopped it. Page weight is the governor: a batch unchanged since the last frame is stored once, and a page past its budget is refused with the video pointer. The site's front page is the first customer.

See the [design notes](DESIGN-NOTES.md#new-output-surfaces).

## The sketch on a phone, and Swift Playgrounds

A sketch renders through one view on either window system, and `Apps/OllinSketchApp` is the reference project for wrapping one as an iOS app. What's ahead is the life around that: the Swift Playgrounds on-ramp and the smaller device affordances.
...
**Later.** Editing experiences the live-reload core makes possible, and where Ollin draws its line on AI. The public stance already holds (the README's "It's a tool for making art ... Ollin is not a generative-art model"), and it extends to any AI *feature*: **AI is a tool for operating the framework, never an author of sketches.** Helping wire a generator, suggest a filter, or move parameters toward a look is in scope; generating a whole sketch or its imagery from a prompt is deliberately out. Within that line:
...
- **The drag on the performance stage.** The performance host's editor buffer as a second place the same drag could write.
...
- **A field that takes a rule.** The inspector row for a parameter taking its rule as typed text, so a rule is written where the parameter is set rather than only in the sketch or the automation file, and pointing at where the text went wrong (`FormulaError` carries the character offset for exactly that).
- **AI at the controls.** Drive the parameters, wire and parameterize generators, and tune toward a described look, all as operations on the typed `@Param` and effect graph, with the artist composing the sketch.
