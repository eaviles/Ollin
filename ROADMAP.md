...
- **A USD scene reader.** The loaded scene graph reads glTF today (see [`Docs/3D/Scenes.md`](Docs/3D/Scenes.md)); USD carries the same node, camera, and light structure, so a structure-preserving USD read is the next leg of the import direction (Model I/O merges it to one mesh today, exactly as the OBJ/STL path does).
