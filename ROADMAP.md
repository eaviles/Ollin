...
- **More than moving a shape.** Reordering a shape by hand, which moves the call among its siblings rather than changing any of its numbers, so it is the one gesture the drag's own machinery cannot serve as it stands; and the performance host's editor buffer as a second place the same drag could write.
