...
The concerns of a piece that runs unattended for days in a gallery rather than for a session at a desk. What is ahead here is getting the piece onto the wall: projection mapping (a corner-pin / homography warp on the presented frame, with edge blending across projectors) and fullscreen output spanning several displays. Single-file sketches wait on one thing besides: the live host opens their window, so the sketch never opens its own, and everything a sketch declares in its `Installation` ([`Docs/Output/Installation.md`](Docs/Output/Installation.md)) applies on the path where it owns that window. The fix is a host that compiles the file and hands the sketch to `OllinApp.run`. A documented, supported way to run long rather than a hope. See the [design notes](DESIGN-NOTES.md#installation-mode).
