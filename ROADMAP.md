...
- **A native USD reader.** USD is the scene format of the Apple ecosystem, so it should be a format Ollin reads natively: a clean-room parser of its own (the posture the glTF reader set), not bounded by the platform importer, which drops lights, animation, and skinning. The arc runs through transform animation and UsdSkel with blend shapes, to reading structure and meshes natively. See the [design notes](DESIGN-NOTES.md#3d-mode).
