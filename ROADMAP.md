...
Two linked directions: a composable API for chaining and mixing shader-driven visuals fluently (in the spirit of Hydra's `osc().rotate().modulate(noise())`), and a separate live-coding performance app built on top of it. Both are distinct from `OllinLive`, which is edit-loop hot-reload, not a performance tool. A third strand is **broadening the 3D SDF combinators**: the sphere-traced field merging (`drawSDF3D`) growing toward the full ShaderPark "sculpting" experience. What's left there is a field *receiving* a mesh's shadow under a point or ray-traced light, the one direction the field-shadow integration doesn't yet cover (the field casts under every light, and receives under directional/spot). See the [design notes](DESIGN-NOTES.md#shader-composition-and-live-coding-not-started).
...
- **A procedural sky environment.** Image-based lighting works from a bundled or loaded HDRI today; the zero-asset complement is a procedural sky, synthesized at runtime (Apple's `MDLSkyCubeTexture` gives a physically-based sunlit sky from a few parameters), so `environment(.sky(sunElevation:…))` lights a scene with no asset at all and a sketch can animate the sun across the day.
...
- **A download indicator for remote HDRIs.** A downloaded or `highRes(_:)` environment fetches (and then decodes a large EXR) in the background; on a first run that's a silent wait. Surface progress in the terminal (the download's bytes and percent, then a "decoding" note) so it's clear what's happening rather than an unexplained pause.
- **A cache-clearing helper.** The environment cache (downloaded HDRIs plus their decoded-pixel blobs, under `~/Library/Caches/Ollin/Environments/`) and the downloaded ML model weights accumulate under known paths; a small `Scripts/clear-caches.sh` (honoring `OLLIN_ENVIRONMENT_CACHE`) would wipe them in one step.
