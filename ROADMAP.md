...
- **Fractals.** Escape-time sets (Mandelbrot, Julia) as a GPU `Generator`; iterated function systems and fractal flames; circle-inversion fractals and Kleinian limit sets.
...
- **Growth and morphogenesis.** Multi-scale Turing patterns, the Abelian sandpile, and the 3D forms of the shipped 2D growth systems (differential growth and reaction-diffusion over a mesh surface, the coral genre) once the meshing work below is in.
