...
- **Image quality and motion.** **Per-object motion vectors** (a velocity buffer giving temporal anti-aliasing exact history for world-space movers), **MetalFX** temporal upscaling for quality and headroom, **contact shadows** (short-range screen-space) to seat objects on surfaces, and per-object and camera **motion blur** over the same velocity buffer for cinematic movement.
