...
- **A GIF written as it is drawn.** The GIF export builds the whole animation in memory and lays the file down at the end, because the system's image writer keeps every frame it is handed. The cost is about ten bytes a pixel a frame, so a long loop at canvas size asks for gigabytes and a small machine swaps. The export says what it will cost before it starts, and `--gif-width` is the way down. What is ahead is a writer of our own that appends a frame at a time. The palette and the LZW coding are the work, plus the choice between one palette for the whole loop and one per frame. Video and the frame sequence both stream already, so this is the one export that does not.
