...
- **A parameter timeline.** The host surface over the knob tracks a sketch carries: a lane per knob, keys placed and dragged by hand, curve handles shaped by eye, and a playhead scrubbed over the deterministic clock, writing back to the same file the tracks already read. The live host reading that file on launch, and carrying it across a reload, belongs with it, and so does a per-knob keyframe affordance in the inspector row.
