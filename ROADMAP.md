...
- **A rule for the knobs a number cannot cover.** A point, a color, a rectangle, or a range driven by one rule per component, so the whole `@Param` set is reachable as typed text and not only the knobs that hold a single number. A field in the inspector that takes the rule, and points at where the text went wrong, belongs with it.
