...
The concerns of a piece that runs unattended for days in a gallery rather than for a session at a desk. What is ahead here: checkpoint and restore of generative state, restart on failure, and scheduled evolution. Each one joins the `Installation` a sketch already declares ([`Docs/Output/Installation.md`](Docs/Output/Installation.md)). Single-file sketches wait on one thing: the live host opens their window, so the sketch never opens its own. The fix is a host that compiles the file and hands the sketch to `OllinApp.run`. The other half is getting the piece onto the wall: projection mapping (a corner-pin / homography warp on the presented frame, with edge blending across projectors) and fullscreen output spanning several displays. A documented, supported way to run long rather than a hope. See the [design notes](DESIGN-NOTES.md#installation-mode).
