...
- **Accumulation canvas.** A render target the frame *doesn't* clear, so drawing piles up across frames — progressive refinement, long-exposure stills, classic paint-on-canvas sketches. Pulled forward from [layered effects](#layered-effects-and-compositing) as its own small feature because [depth-of-field particle rendering](#depth-of-field-particle-rendering-sandpainting) needs exactly this slice; drawn with the additive blend mode (`blendMode(.add)`) it already makes an 8-bit prototype playable before the float pipeline lands.
