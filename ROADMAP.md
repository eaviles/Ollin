...
- **Multi-loop traced outlines in vector export.** The marching-squares tracer behind the analytic shapes' vector outlines (`SDFOutline.trace`) collects segments for every boundary loop, but `stitch` walks only one closed loop and drops the rest, so a traced shape whose region boundary has more than one loop exports incomplete (the Cool S loses its interior lines; the raster render has them). The fix: stitch all loops, and record multi-loop traces as a `Shape` with even-odd winding instead of a single polygon.
