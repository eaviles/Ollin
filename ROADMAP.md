...
- **The rest of the sketch on the web page.** The [page export](Docs/Output/Web.md) carries the analytic shapes, strokes and fills, text and pictures, the composed and raymarched fields, the layered effects, and the parameters as controls; the doors still to open, in order: the site's front page drawn from the inline form, and strokes and fills as the points and the style the sketch gave, expanded on the page, which is what brings a dense line drawing under the page's weight limit. The effects that are a solve or a ladder on the Mac, lit meshes, ray tracing, compute work, and pictures that arrive as a texture every frame stay with the video export. Not a browser runtime for the framework, which stays out (the platform stance in `CLAUDE.md` says why).
