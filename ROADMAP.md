...
- **Richer 3D physics.** The 3D rigid-body world ships (a `World3D` of stacking, tumbling, swinging bodies with joints and mouse grabbing, backed by vendored Jolt); the solver underneath has far more to offer, and the aim is to expose its breadth under the same simple API: joint **motors and limits** (doors with springs, motorized wheels, ragdoll-ready swing-twist), **compound and more shapes** (multi-collider bodies, tapered forms, a height-field collider that takes a `Heightfield` directly, static colliders from a loaded `Scene`), **contact events and sensors** (a body enters a region, two bodies touch, read in `draw()` like mouse events), a **character controller** (walk a figure over any scene), **vehicles** (wheeled and motorcycle controllers), **ragdolls** (skeleton-driven, pairing with the scene-import skinning), **3D soft bodies** (cloth sheets and pressurized volumes beside the rigid world), and **buoyancy** (a water plane bodies bob in). Each lands as more capability on the shipped `World3D`/`Body3D` surface, an example per slice.
