...
- **The tethered edit loop.** The parameters and the shaders can both be instant against a phone (the remote inspector serves `@Param` over the usbmux tunnel, and Metal compiles shader source on the device); a Swift edit cannot be, since a device loads no pushed dylib, so its loop is a build and an install through `devicectl` with the inspector open beside it. On a warm build that round trip is under ten seconds even over Wi-Fi, so the loop is designed as a slow reload that carries the clock and the parameters across the relaunch, not as a compile break. [Design notes.](DESIGN-NOTES.md#swift-playgrounds-and-ios)
