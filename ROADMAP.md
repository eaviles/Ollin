...
- **The rest of the sketch on the web page.** The [page export](Docs/Output/Web.md) carries the analytic shapes and the layered effects; the doors still to open, in order: strokes and fills as their triangles; text through the glyph atlas and images as bundled textures; SDF scenes as data; parameters as controls wherever a shader reads them; a weight budget, with a page past it refused and pointed at video; and the site's front page drawn from the inline form. The effects that are a solve or a ladder on the Mac, lit meshes, ray tracing, compute work, and pictures that arrive as a texture every frame stay with the video export. Not a browser runtime for the framework, which stays out (the platform stance in `CLAUDE.md` says why).
