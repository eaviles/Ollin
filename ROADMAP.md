...
- **Richer 3D physics.** The 3D rigid-body world ships (a `World3D` of stacking, tumbling, swinging bodies with motored, limited joints, contact events, sensor regions, walking characters, and mouse grabbing, backed by vendored Jolt); the solver underneath has far more to offer, and the aim is to expose its breadth under the same simple API: **vehicles** (wheeled and motorcycle controllers), **ragdolls** (skeleton-driven, pairing with the scene-import skinning), **3D soft bodies** (cloth sheets and pressurized volumes beside the rigid world), and **buoyancy** (a water plane bodies bob in). Each lands as more capability on the shipped `World3D`/`Body3D` surface, an example per slice.
