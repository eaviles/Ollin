...
A Mac has no depth camera, inertial sensors, or spare Neural Engine for live perception; a tethered iPhone has all three. The idea: let the phone act as a sensor and on-device ML co-processor for a sketch that still renders on the Mac, capturing and perceiving (LiDAR point clouds, face and body tracking, segmentation, device motion, and more) and streaming typed results the sketch reads in `draw()`. Both a recorded RGBD clip and a tethered phone's live USB RGBD stream already reconstruct on the Mac as point clouds (see [`Docs/Record3D.md`](Docs/Record3D.md)), the kind of world-facing depth feed an Intel RealSense once gave openFrameworks. The work ahead grows that one channel into the full sensor stream — face mesh and blendshapes, body pose, segmentation mattes, device motion — and replaces the borrowed app with an Ollin-own iPhone capture app so the chain is ours end to end; using each frame's pose to fuse several into one world cloud comes with it. The wire protocol, the sensor catalog, and the build order are in the [design notes](DESIGN-NOTES.md#iphone-as-a-sensor-array-partly-shipped).

## RGBD feeds and depth-lifted perception

A colour image plus a depth map plus intrinsics — an "RGBD feed" — is the shape several sources already produce: a Record3D `.r3d` file, its live USB stream, the webcam paired with a depth model (the `3D/DepthCloud` example, no phone at all), and the iPhone capture app to come. `RGBDFrame` is essentially that type already; the work is to lift it into a small, source-agnostic abstraction so anyone can plug a depth source in one end and get 3D out the other, regardless of where the depth came from. Three capabilities compose over it: point clouds (shipped), **depth-lifted pose** — back-project a 2D skeleton's joints (from a Vision body tracker, or any 2D pose) through the depth and intrinsics into true metric 3D, more accurate than a single-image estimate and sitting in the same space as the cloud — and, later, depth-aware compositing and occlusion. It's entirely Mac-side, so it's independent of the iOS capture app and can land on its own; the depth-lift is the natural first piece.
