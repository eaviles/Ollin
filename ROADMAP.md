...
- **Skinned and morphing scene animation.** Skeletal skinning and morph targets, the deforming tier of a scene file's authored animation, over the loaded scene graph's keyframe playback (see [`Docs/3D/Scenes.md`](Docs/3D/Scenes.md)); a USD scene reader is a further leg of the same direction.
