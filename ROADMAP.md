...
- **Richer 3D physics.** The 3D rigid-body world ships (a `World3D` of stacking, tumbling, swinging bodies with motored, limited joints, contact events, sensor regions, walking characters, driveable vehicles, skinned-figure ragdolls, draping soft bodies, water bodies float in, and mouse grabbing, backed by vendored Jolt); the solver underneath has far more to offer, and the aim is to expose its breadth under the same simple API: **world queries** (ray casts, shape sweeps, and overlap tests a sketch can ask directly), **collision filtering** (saying that two things never touch), and **per-body motion knobs** (continuous collision detection, locked axes, a gravity scale). Each lands as more capability on the shipped `World3D`/`Body3D` surface, an example per slice.
