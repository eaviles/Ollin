...
- **Richer 3D physics.** The 3D rigid-body world ships (a `World3D` of stacking, tumbling, swinging bodies with motored, limited joints, contact events, sensor regions, walking characters, driveable vehicles, skinned-figure ragdolls, draping soft bodies, water bodies float in, world queries, collision groups, and mouse grabbing, backed by vendored Jolt); the solver underneath has far more to offer, and the aim is to expose its breadth under the same simple API: **the constraint kinds it has and `JointKind3D` does not** (path, gear, rack-and-pinion, pulley, six-DOF, plus driving a standalone swing-twist joint), **closing the soft-body seams** (cloth reaching `contacts`, per-particle buoyancy so a raft can be cloth), **tracked vehicles**, and **saving a world and loading it back**. Each lands as more capability on the shipped `World3D`/`Body3D` surface, an example per slice.
