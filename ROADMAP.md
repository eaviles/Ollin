...
- **Perfect-loop helpers.** The seamless loop is how generative motion gets shared: a `loop(duration:)` phase primitive (with an out-and-back ping-pong fold beside it) so looping progress stops being a hand-typed `truncatingRemainder` incantation, polar and toroidal noise sampling so a noise field tiles in time, and an `--export-loop` that renders exactly one period. Small additions to the math and export layers with outsized visibility; the Guide's motion chapter teaches the raw idiom today and would adopt these.
- **Bare shaping functions.** Top-level `smoothstep(_:_:_:)`, `step`, `fract`, and `clamp` scalars on the CPU side, spelled to match the shader library, so the shaping vocabulary a sketch learns carries verbatim into per-pixel shader code. The `Easing` catalog stays the curve library; these are its bare primitives.
- **Richer `@Param` controls.** A stepped or integer range so a whole-numbered knob (a wave count, a column count) snaps honestly instead of needing `.rounded()` in the sketch, and the control kit beyond sliders: toggles, pickers, a color well. The inspector surfaces in OllinLive, the gallery, and the live-coding host read one registry, so an addition reaches all three.
- **A `Timeline` worked example.** The keyframe sequencer has a docs section but no `Examples/Motion` entry; a small choreographed piece would give the docs page and the Guide's further-reading lists something to link.
