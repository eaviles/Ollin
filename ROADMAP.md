...
- **Strange attractors and chaotic maps.** Lorenz, Rössler, Aizawa, Clifford, Peter de Jong, and friends: iterate the map or integrate the system and plot the orbit, in 2D and 3D, as a point cloud, a density field, or a traced polyline. The logistic and Hénon maps and the bifurcation diagram sit here too. The 2D maps and density plots stand on the shipped pipeline, but the 3D continuous systems are made to be grabbed and orbited, so the interactive 3D form waits on [camera control](#camera-control-and-cinematic-moves) and is the natural first showcase for it. Reference for the 3D systems, their equations and parameters: [dynamicmath.xyz/strange-attractors](https://www.dynamicmath.xyz/strange-attractors/).
