...
- **Scene animation.** Playing a glTF file's authored animation over the loaded scene graph: node TRS keyframe tracks first, with skinning and morph targets the later tier. Pairs with structure-preserving scene import (see [`Docs/3D/Scenes.md`](Docs/3D/Scenes.md)); a USD scene reader is a further leg of the same direction.
