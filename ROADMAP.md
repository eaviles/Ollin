...
- **More model examples over `ModelTracker`.** The custom-model tracker runs anything converted to Core ML; a few well-known models would each make a strong example, with the weights always fetched by `Scripts/fetch-models.sh` rather than committed. Object detection (YOLOv3-Tiny) drops in as-is; style transfer and semantic segmentation each need one small new output surface on the tracker first. Candidates, licenses, and the surfaces involved are in the [design notes](DESIGN-NOTES.md#model-examples-and-modeltracker-surfaces-not-started).
