...
- **Perfect-loop rendering.** The seamless loop is how generative motion gets shared: polar and toroidal noise sampling so a noise field tiles in time, and an `--export-loop` that renders exactly one period of a sketch's loop. Small additions to the noise and export layers with outsized visibility.
