...
- **Perceptual color: OKLab, OKLCH, OKHSL.** `Color` conversions for the OKLab family, plus mixing that interpolates in a chosen space: OKLab for even gradients, OKLCH for hue and chroma dials, OKHSL for dials that always land on a displayable sRGB color. Interpolating through these spaces can leave the gamut, so a hue-preserving chroma clamp is part of the work. See the [color notes](DESIGN-NOTES.md#color-gradients-and-palettes-not-started).
- **Color ramps and palettes.** A gradient built from a list of colors, evenly spaced or with explicit stops, sampled with `color(at:)` the way `Palette` and `Colormap` already are; and discrete palettes, a few colors carried as a unit, with harmony builders computed in OKLCH and built-in collections only where the license is clean. Same [color notes](DESIGN-NOTES.md#color-gradients-and-palettes-not-started).
- **Gradient fills and strokes.** The ramp above as paint: `fill` and `stroke` accept a gradient beside a flat color (linear, radial, or along the path) on every shape path, SDF, capsule, and tessellated alike. Linear and radial map natively to SVG export. Design in the [color notes](DESIGN-NOTES.md#color-gradients-and-palettes-not-started).
