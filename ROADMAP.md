...
- **Richer 3D physics.** The 3D rigid-body world ships (a `World3D` of stacking, tumbling, swinging bodies with motored, limited joints, tracks, pulleys, gear trains, contact events, sensor regions, walking characters, driveable vehicles on wheels or tracks, skinned-figure ragdolls, draping soft bodies, water bodies float in, world queries, collision groups, mouse grabbing, and saving a settled world to load back later, backed by vendored Jolt); the solver underneath has more to offer, and the aim is to expose its breadth under the same simple API. Each slice lands as more capability on the shipped `World3D`/`Body3D` surface, with an example.
