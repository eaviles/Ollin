...
- **Gradient fills and strokes.** The ramp above as paint: `fill` and `stroke` accept a gradient beside a flat color (linear, radial, or along the path) on every shape path, SDF, capsule, and tessellated alike. Linear and radial map natively to SVG export. Design in the [color notes](DESIGN-NOTES.md#gradient-fills-and-strokes-not-started).
