...
- **A pass over the built-in material library.** Walked by eye in the material explorer (`Examples/3D/Materials/Explorer`), preset by preset under several lighting presets and backdrops: retune any built-in that doesn't read as its name, and add the everyday finishes still missing. Promoting the explorer's `Material` preset menu (`ParamChoices`) into the core rides along, so every host's inspector offers the library. See the [design notes](DESIGN-NOTES.md#material-and-scene-explorers).
...
- **A scene explorer.** The material explorer's stage pointed at a loaded scene file: orbit it or adopt its camera, switch lighting, backdrop, and fog, toggle the file's lights, and isolate a part by name. It shows the part names and the import's warnings before `ollin new --from-scene` writes the scene out as source. A read-only lens on the asset, never an editor. See the [design notes](DESIGN-NOTES.md#material-and-scene-explorers).
