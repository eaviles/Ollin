...
- **Typed core first, bare call second.** The p5-style calls (`background`, `drawCircle`) are sugar over a public, typed `Drawer`. Build a feature on the core, then add the bare call, so nothing is reachable only through the facade.
