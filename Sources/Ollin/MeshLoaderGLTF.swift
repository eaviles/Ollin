...
// the common mesh subset (POSITION + NORMAL + triangle indices, float positions/
// normals, embedded or external buffers); skeletal animation, morph targets, and
// materials are ignored, geometry only.

extension Mesh {

    /// Read positions, normals, and triangle indices from a glTF 2.0 file, `.gltf`
    /// (JSON, with the binary in an embedded data-URI or a sidecar `.bin`) or `.glb`
    /// (the self-contained binary container). Every mesh in the scene is baked through
    /// its node's world transform and merged into one `Mesh`; a primitive without
    /// normals gets smooth ones. Returns `nil` if the file can't be read or holds no
    /// triangles.
