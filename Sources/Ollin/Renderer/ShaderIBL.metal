...
// frame: they turn a loaded equirectangular HDRI (or a procedural sky cube) into the three
...
//
// `params` (buffer 0): x = cube face index 0…5, y = roughness (prefilter), z = source mip
// count, w unused.
...
fragment float4 ollin_ibl_irradiance(OllinIBLVaryings in [[stage_in]],
    ...
    float count = 0.0;
    const float dPhi = 0.025, dTheta = 0.025;
    for (float phi = 0.0; phi < 2.0 * OLLIN_IBL_PI; phi += dPhi) {
        for (float theta = 0.0; theta < 0.5 * OLLIN_IBL_PI; theta += dTheta) {
