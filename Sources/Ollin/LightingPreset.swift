...
public extension LightingPreset {
    ...
    /// reach for when you just want the scene to look 3D.
    static let standard = LightingPreset(
        ambient: Color(white: 0.28),
        lights: [
    ...
    /// A classic film three-point rig: a daylight **key** from the front-left, a
    /// cooler, dimmer **fill** from the front-right opening up the shadows, and a
    /// **rim** light raking from behind to separate the subject from its background.
    /// Low ambient, so the modeling comes from the lights rather than a flat fill.
    static let threePoint = LightingPreset(
        ambient: Color(white: 0.10),
        lights: [
            .directional(Color(kelvin: 5200), direction: Vector3(0.55, -0.55, -0.6), intensity: 1.0),
            .directional(Color(kelvin: 6500), direction: Vector3(-0.6, -0.2, -0.5), intensity: 0.35),
            .directional(Color(kelvin: 6800), direction: Vector3(0.1, -0.4, 0.9), intensity: 0.7),
        ])

    /// Warm, low, raking sun near the horizon with a cool sky bounce from above: the
    /// golden-hour look. Long, warm key shadows lifted by a soft blue fill.
    static let goldenHour = LightingPreset(
        ambient: Color(red: 0.18, green: 0.13, blue: 0.10),
        lights: [
            .directional(Color(kelvin: 3300), direction: Vector3(-0.75, -0.22, -0.4), intensity: 1.15),
            .directional(Color(kelvin: 8000), direction: Vector3(0.2, -0.9, 0.25), intensity: 0.35),
        ])

    /// High-contrast, single hard key: a neutral, almost white raking light with the
    /// shadows crushed nearly to black and only a whisper of cool fill, so unlit faces
    /// fall away into darkness. The noir / dramatic look; pairs well with
    /// `castShadows()`.
    static let noir = LightingPreset(
        ambient: Color(white: 0.015),
        lights: [
            .directional(Color(kelvin: 5800), direction: Vector3(-0.85, -0.3, -0.35), intensity: 1.55),
            .directional(Color(kelvin: 9500), direction: Vector3(0.6, -0.1, 0.5), intensity: 0.04),
        ])

    /// Bright, soft, even daylight from several directions: the studio softbox /
    /// product look. Low contrast, no deep shadows, surfaces read clean.
    static let studio = LightingPreset(
        ambient: Color(white: 0.45),
        lights: [
            .directional(Color(kelvin: 5600), direction: Vector3(-0.35, -0.6, -0.5), intensity: 0.6),
            .directional(Color(kelvin: 5600), direction: Vector3(0.45, -0.45, -0.4), intensity: 0.5),
            .directional(Color(kelvin: 6500), direction: Vector3(0.0, -1.0, 0.15), intensity: 0.4),
        ])

    /// Cool, dim, blue night light: a high-temperature "moon" key with a faint
    /// colder fill over a deep-blue ambient. Quiet and atmospheric.
    static let moonlight = LightingPreset(
        ambient: Color(red: 0.04, green: 0.06, blue: 0.11),
        lights: [
            .directional(Color(kelvin: 9500), direction: Vector3(-0.4, -0.7, -0.5), intensity: 0.7),
            .directional(Color(kelvin: 13000), direction: Vector3(0.5, -0.2, 0.4), intensity: 0.15),
        ])
}
