...
| [FormulaParts](FormulaParts/Sketch.swift) | a knob of more than one number under one rule per part: a rectangle whose size is ruled and whose place is not, a dot reading the rectangle back part by part, a ruled color and a ruled pair of ends (`drive(_:x:y:)`) |
...
| [Myriad](Myriad/Sketch.swift) | 8,100 noise-driven circles on the SDF path, thousands of shapes at full speed, with a knob swapping them for rounded rects (corner radius sweeping square→pill, each turned by its own angle) |
