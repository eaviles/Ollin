...
| [SolidType](Geometry/SolidType/) | A word extruded into a solid that catches the light and throws a shadow, and the same word a letter at a time, each nodding about its own center (`drawText3D`, `Mesh.text`, `Mesh.textGlyphs`). |
...
| [PhoneMarkers](Phone/PhoneMarkers/) | The pictures and objects the phone knows (tap **Markers**), found in the room. A city of columns rises off every print it recognizes, framed by the print's own edge; a scanned object arrives as the box its scan measured. Drop a picture into the app's folder, named with its printed width. Needs `import OllinPhone`. |
| [PhonePointer](Phone/PhonePointer/) | The phone held as a pointer (tap **Wand**): a beam out of the back of it lands on a ball, a press on the pad picks the ball up, sliding the thumb pushes it away or pulls it in, letting go drops it. Needs no LiDAR. Needs `import OllinPhone`. |
| [PhoneAttention](Phone/PhoneAttention/) | Where the phone's picture draws the eye (tap **Attention**): the heat map as a warm glow over the live frame, a frame around each region the model picks out, an eased bead trailing where the attention has been. Needs no LiDAR. Needs `import OllinPhone`. |
