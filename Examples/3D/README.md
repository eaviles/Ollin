...
| [Yard](Physics/Yard/) | A yard kept whole: a truck you drive with the arrows, a figure pacing across it, and a second figure lying where it fell. S writes the lot to a file and L reads it back, so a restore comes back mid-drive and mid-stride. The figure's skin stays the sketch's own asset. `snapshot()`, `world.vehicles`, `world.characters`, `world.ragdolls`, `scene.apply(ragdoll)`. |
