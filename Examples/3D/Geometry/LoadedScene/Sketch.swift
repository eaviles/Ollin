...
/// pivot, its authored transform composing underneath. The bundled `scene.gltf`
/// is a small stage authored by the project itself (regenerate it with
/// `Scripts/make-sample-scene.swift`).
///
...
final class LoadedScene: Sketch {
    ...
    override func draw() {
        ...
        // The authored view and lighting: the file's camera and lights, applied as
        // ordinary camera/light state. Fall back to an orbit for a camera-less file.
        camera(stage.camera ?? .orbiting(target: Vector3(0, 0.8, 0), radius: 6, elevation: 0.3))
