...
final class SpecularAntialias: Sketch {
    ...
    /// what makes the highlight too small to sample.
    @Param(0.05...0.30, icon: "circle.grid.3x3", group: "Scene")
    var ballSize = 0.11
    ...
    override func draw() {
        ...
        cameraShowcase(.sway(period: 34), target: Vector3(0, 0.1, 0),
                       radius: 9, elevation: 0.34, fieldOfView: .pi / 3.4)
        ...
        // The bed of balls: 33 by 33 of them over a six-unit square, each one
        ...
        for row in 0..<side {
            for column in 0..<side {
                let x = -3.0 + 6.0 * Double(column) / Double(side - 1)
                let z = -3.0 + 6.0 * Double(row) / Double(side - 1)
                balls.append(MeshInstance(
                    position: Vector3(x + random(-0.04, 0.04), ballSize,
                                      z + random(-0.04, 0.04)),
