...
final class RaymarchedDetailing: Sketch {
    override func draw() {
        ...
        let post = SDF3D.cylinder(radius: 0.42, height: 2.6).at(x: -2.9, y: 0.2, z: 0)
            .colored(Color(hex: 0x9adcf0))
        let bar = SDF3D.box(width: 2.2, height: 0.55, depth: 0.55).at(x: -2.2, y: 0.75, z: 0)
        ...
        let globe = SDF3D.sphere(radius: 1.0).colored(Color(hex: 0xff6f61))
            .engrave(.plane(normal: Vector3(0, 1, 0), offset: 0.0).at(x: 0, y: 0.2, z: 0),
                     depth: 0.05)
            .groove(.plane(normal: Vector3(0, 1, 0), offset: 0.0).at(x: 0, y: 0.75, z: 0),
                    depth: 0.06, width: 0.07)
        drawSDF3D(globe.at(x: -0.3, y: 0.2, z: 0))
        ...
        drawSDF3D(beaded.at(x: 2.0, y: 0.2, z: 0))
        ...
        drawSDF3D(ring.rotatedZ(0.5).at(x: 3.7, y: 0.6, z: 0))
    }
}
