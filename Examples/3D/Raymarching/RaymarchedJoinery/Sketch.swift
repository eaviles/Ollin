...
final class RaymarchedJoinery: Sketch {
    override func draw() {
        ...
        let head = SDF3D.hexPrism(radius: 0.55, height: 0.5).at(x: 0, y: 1.5, z: 0)
            .colored(Color(hex: 0xffb454))
        let shaft = SDF3D.cylinder(radius: 0.28, height: 1.9).at(x: 0, y: 0.55, z: 0)
            .colored(Color(hex: 0x9adcf0))
        let base = SDF3D.box(width: 2.2, height: 0.7, depth: 2.2).at(x: 0, y: -0.75, z: 0)
            .colored(Color(hex: 0x5f6f86))
        let scoop = SDF3D.sphere(radius: 0.55).at(x: 0.9, y: -0.35, z: 0.9)
        ...
        let cap = SDF3D.pyramid(base: 0.85, height: 0.8)
            .rotatedY(t * 0.7).at(x: 2.4, y: 0.92, z: 0).colored(Color(hex: 0xff6f61))
