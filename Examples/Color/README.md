...
| [ColorVision](ColorVision/Sketch.swift) | two palettes seen four ways, with a bar marking every pair that merges, and `Filter.colorVision` over the whole canvas on a parameter |
...
| [Mixing](Mixing/Sketch.swift) | the same two colors mixed in RGB, HSB, OKLab, OKLCH, OKHSL, and paint (Kubelka-Munk over spectra), band by band, each band's midpoint read back through `hue`/`saturation`/`brightness` |
...
| [PrintSeparation](PrintSeparation/Sketch.swift) | a sunrise poster split into three spot-ink printing masters, with a halftoned overprint preview and a view parameter to flip between them |
