...
| [ContourTrace](ContourTrace/Sketch.swift) | the camera's edges traced into vector contours and drawn as black line art over a faint feed; each contour is an Ollin `Shape`, so it can be filled, hatched, or sent to a plotter; pass a clip path on launch and the same detector traces the footage instead, because the `FrameSource` seam makes the source a one-line swap (`ContourDetector`, `shapes(in:)`, `VideoPlayer`) ; reads the bundled film where there is no camera |
| [HandTracking](HandTracking/Sketch.swift) | up to two hands drawn as 21-joint skeletons over the feed: finger bones, joint dots, and highlighted fingertips, tinted by which hand it is (`HandTracker`, `Hand`, `bones(in:)`) ; reads two open palms where there is no camera |
...
| [OpticalFlow](OpticalFlow/Sketch.swift) | motion measured as a field of vectors and drawn two ways, arrows colored by speed and a dust that rides them; reads the bundled film where there is no camera (`FlowTracker`) |
...
These examples read a camera, and they ask for camera permission on first run (the system prompts from `swift run`). Where there is no camera, most of them fall back to one of the bundled [sample photographs](../../Docs/Drawing/SamplePhotos.md) so the sketch still has something to read, and `--photo` takes the picture even where a camera would have worked, which is how a still of one is made. TrajectoryTracking and DigitReader are self-contained and need no camera at all. ContourTrace is a partial exception: handed a clip path on launch, it traces the file instead of the camera. DepthRelief, DepthContours, ObjectDetection, PaintByClass, PointLift, DigitReader, and TugOfWords also need their models downloaded once with `Scripts/fetch-models.sh`. The weights are fetched, never committed. StyleMirror wants a model you train yourself (below). Run one with `swift run Example-Vision-<Name>`, for example `swift run Example-Vision-WebcamFeed`.
