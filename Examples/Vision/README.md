...
These examples need a camera and ask for camera permission on first run (the system prompts from `swift run`) — except VideoTrace and TrajectoryTracking, which are self-contained and run with no camera at all. DepthRelief additionally needs its model downloaded once (`Scripts/fetch-models.sh` — the weights are fetched, never committed). Run one with `swift run Example-<Name>`, e.g. `swift run Example-WebcamFeed`.
