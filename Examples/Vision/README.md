...
| [ContourTrace](ContourTrace/Sketch.swift) | the camera's edges traced into vector contours and drawn as black line art over a faint feed, each an Ollin `Shape` that could be filled, hatched, or sent to a plotter; pass a clip path on launch and the same detector traces the footage instead, the one-line source swap the `FrameSource` seam buys (`ContourDetector`, `shapes(in:)`, `VideoPlayer`) |
...
| [Lift](Lift/Sketch.swift) | something lifted off the background, two ways: a `segmenter` parameter switches between people composited over a drawn, drifting gradient (background replacement in a few lines, the tinted matte doubling as the drop shadow) and the salient subject in a spotlight, the frame dimmed, the cutout at full color with a halo from the tinted matte (`PersonSegmenter`, `SubjectSegmenter`, `matte`, `cutout`, `count`) |
| [TuringMirror](TuringMirror/Sketch.swift) | the person matte steering chemistry: the matte drives a reaction-diffusion field's modulation layer, so one continuous simulation grows maze walls on your silhouette and spots everywhere else, reorganizing live as you move (`PersonSegmenter`, `SimField.modulation`, `.reactionDiffusion(feed:kill:toFeed:toKill:)`) |
| [PointLift](PointLift/Sketch.swift) | click a thing and it lifts out of the feed: point-prompted segmentation of whatever sits under the click, with shift-click trimming the mask and C letting it go; needs the fetched model (`PointSegmenter`, `pick`, `bounds(in:)`) |
...
| [DepthContours](DepthContours/Sketch.swift) | depth that holds still: contour lines of depth over the feed from a *video* depth model that reads each frame against the ones before it, so the lines move only with the scene; a `perFrame` parameter draws them from the single-image model instead and they crawl, which is the whole point of a video model; R re-anchors the depth scale; run `Scripts/fetch-models.sh` once to build the model (`DepthTracker`, `value(at:in:)`, `reset()`, `isolines`) |
| [FootageDepth](FootageDepth/Sketch.swift) | the whole clip's depth, read ahead of time: a bundled recording goes through the video depth model's published 32-frame window inference once (a progress bar counts the pass up; the result is cached), then contour lines of its depth follow the flyers by clip time, and `--export-video` lands them on the same frames every run, which a live tracker over a playing clip can't promise; pass a path on launch to read your own clip (`DepthClip`) |
...
| [TugOfWords](TugOfWords/Sketch.swift) | typed phrases as live parameters: two phrases pull on one rope by how well each matches the frame, and a phrase rewritten in the inspector (⌘/) re-anchors the rope; run `Scripts/fetch-models.sh` once to download the model (`ConceptTracker`, `confidence(of:)`, `similarity(of:)`) |
