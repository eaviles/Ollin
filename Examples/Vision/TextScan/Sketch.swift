...
/// `.accurate` reads small print, which is what the bundled photograph has: the
/// cover of a notebook on a desk. Once warm it costs about 140 ms a frame here,
/// but the *first* accurate pass on a Mac prepares the system's reader and can
/// take the better part of a minute, so the first run of this sketch sits empty
/// for a while and every run after it is quick.
@main
