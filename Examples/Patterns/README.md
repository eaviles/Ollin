...
| [GumowskiMira](GumowskiMira/Sketch.swift) | a Gumowski-Mira blossom as an accumulating density field: a nearly area-preserving map with a whisper of damping, a fresh orbit every frame, superimposed the way the classic plates are (`ChaoticMap.gumowskiMira`) |
...
| [Lichtenberg](Lichtenberg/Sketch.swift) | a Lichtenberg figure growing live: the dielectric-breakdown model solves the field around the discharge each step and grows where it is strongest, so the branches feel their way outward and arc to the rim (`DielectricBreakdown`) |
...
| [Kolam](Kolam/Sketch.swift) | one line launched between a field of dots at 45 degrees, bouncing off the edges until it closes: `rows` by `columns` dots make exactly gcd(rows, columns) loops, so 7 by 5 is unbroken and 6 by 4 is two |
...
| [Knotwork](Knotwork/Sketch.swift) | the kolam line given width and a rule about who passes over whom: the choice alternates along every cord, which is what holds a weave together instead of reading as a tangle |
...
| [HyperbolicTiling](HyperbolicTiling/Sketch.swift) | regular tilings of the hyperbolic plane through the Poincare disk, identical tiles meeting the same way at every vertex and repeating into a circular horizon, with a slow camera pan (`hyperbolicTiling`) |
...
| [Guilloche](Guilloche/Sketch.swift) | the engine-turned ornament of watch faces and banknotes: concentric wavy rings, each turned a hair against its neighbor, weaving into a braided moire that creeps with time (`guilloche`) |
...
| [ShapeGrammar](ShapeGrammar/Sketch.swift) | a window frame, one rule, and a lattice that cuts itself: a cell becomes two cells split by one straight line between two of its edges, applied over and over (`ShapeGrammar`) |
...
| [Wavefront](Wavefront/Sketch.swift) | a wave leaving a shoreline by Huygens' construction, tearing into pieces where it travels into a bend, the wavelets themselves on mouse hold (`huygensFront`) |
| [Caustic](Caustic/Sketch.swift) | the bright curve in the bottom of a cup, drawn as the envelope of the bounced rays, the source travelling from the middle out past the rim (`caustic`, `reflectedRays`, `envelope`) |
| [DeBruijn](DeBruijn/Sketch.swift) | a ring of sixty-four beads where every run of three colors appears once, and a reader that says where it is from three beads alone (`DeBruijnCode`, `deBruijnSequence`) |
| [Pentominoes](Pentominoes/Sketch.swift) | all twelve pentominoes fitted into a board, laid one at a time in the order the search found them, the tray of unused pieces emptying beside it (`Polyomino`, `tilePolyominoes`, `outlines`) |
| [PowerDiagram](PowerDiagram/Sketch.swift) | territories for circles that have sizes, each cell weighted so no boundary cuts its own circle, the plain Voronoi diagram of the same centers on mouse hold (`powerDiagram`, `WeightedSite`) |
| [FordCircles](FordCircles/Sketch.swift) | a circle for every fraction, arriving denominator by denominator and never overlapping, the touching pairs joined on mouse hold (`fordCircles`, `fareySequence`, `touches`) |
| [Spirolateral](Spirolateral/Sketch.swift) | a wall of walks that step 1, 2, 3 … and turn, each repeated until it comes home, the orders that never come home drawn pale and walking away, a fifth of a turn on mouse hold (`spirolateral`, `closes`, `run`) |
...
| [AntColony](AntColony/Sketch.swift) | an ant colony solving a tour, watched from above: the whole colony walks once a frame and re-lays its pheromone, so a haze over every pair condenses onto a short route (`AntColony`) |
| [CreasePattern](CreasePattern/Sketch.swift) | the flat sheet and the folding side by side: every line the paper is creased along, in the two directions a fold can go, beside the shape it closes into (`CreasePattern`) |
