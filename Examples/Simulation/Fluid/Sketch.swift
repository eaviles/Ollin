...
/// keeps a velocity field and a dye (colour) field across frames; each frame it carries
...
/// inject dye (the mark's colour) and pass `withField`'s `force:` to push the flow.
///
/// A single brush wanders on its own so the fluid is always moving; press and drag to
/// stir it yourself. The dye colour cycles over time, and a bloom gives the swirls glow.
/// See `Simulation/Simulation` and `Simulation/GameOfLife` for the single-field stateful sims.
@main
final class Fluid_Example: Sketch {
    ...
    override func setup() {
        ...
        prev = Vector2(width * 0.5, height * 0.5)   // the brush starts at centre (orbit's t = 0)
    }

    override func draw() {
        // The brush: the mouse while pressed, otherwise a smooth Lissajous orbit that
        // passes through the centre at t = 0, so there's no jump on the first frame.
        let brush: Vector2
        if mouseIsPressed {
        ...
        // gusts); its slowly cycling colour is the dye it lays down (hue/sat/value 0…1).
