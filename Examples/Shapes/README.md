...
| [Hulls](Hulls/Sketch.swift) | three ways to find the shape of a set of points (`concaveHull`/`alphaShape`/`convexHull`). The convex hull is drawn faint with its corner points lit, the concave hull cycles from loose to tight, and the alpha shape finds islands and a hole that neither hull can express |
