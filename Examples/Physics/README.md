...
| [Forces](Forces/Sketch.swift) | the force API in one windy yard: a gust leans on every loose shape through `applyForce` so leaves skitter where crates shuffle, a click kicks the nearest shape with `applyImpulse`, a paddle spun by `applyTorque` bats what drifts through, and a kinematic sweeper plows the floor unstoppably. A pinned lattice tints its springs by `Spring.strain`, and `World.pixelsPerMeter` rides a parameter (`applyForce`, `applyImpulse`, `applyTorque`, `Spring.pin()`). |
