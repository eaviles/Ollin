...
| [LineSpray](LineSpray/Sketch.swift) | a sphere of a hundred and fifty rings of light seen through a real lens: `LineSpray` scatters points along every line through a `Bokeh` ball into a running mean until bokeh appears, the camera, the lens, and the print are parameters, and `Drift` re-bends the forty thousand lines every frame across every core through `noiseFields` |
