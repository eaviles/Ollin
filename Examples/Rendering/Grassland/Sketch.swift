...
/// Half a million blades of grass with NO geometry anywhere. A `StrandField`
/// has no vertex buffer, no instance list, and nothing built in `setup()`: two
/// GPU stages grow every blade inside the draw call itself, from hashes of its
/// index. The camera runs the economy: tiles it cannot see are skipped whole,
/// and distant blades grow with fewer segments than near ones, which is the
/// kind of per-blade geometry change no instanced draw can express. The wind is
...
@main
final class Grassland: Sketch {

    @Param var detail = true   // off = full segments everywhere
    ...
    override func draw() {
        ...
        drawCaption("Strands: 500,000 blades grown inside the draw call")
    }
}
