...
///
/// `.serialized` inside a suite orders that suite's own tests and says nothing
/// about a sibling suite, so two suites that share something the whole Mac can
/// see still run at the same time. Core MIDI is exactly that: the endpoints are
/// machine-global and an input connects to *every* source on the Mac, so one
/// suite's teardown is the other's setup change, and a setup change reconnects
/// the other suite's input in the middle of the stream it is reading. The test
/// that notices is the one needing eight messages in a row to say anything (the
/// timecode walk), which is exactly the one that failed on a CI runner while
/// its three siblings passed in six tenths of a second each. The trait is
/// recursive (a serialized suite runs its sub-suites serially too), so this
/// parent is the whole mechanism. (Two *processes* can still collide, which no
