...
struct Rope3DTests {
    ...
    @Test func aRopeIsBuiltFromItsPolyline() throws {
        let world = World3D()
        let rope = try world.addRope(through: Rope3DTests.hanging(20), at: Vector3(0, 3, 0))
    ...
    @Test func coincidentPointsAreMerged() throws {
        let world = World3D()
        var points = Rope3DTests.sideways(10)
        points.insert(points[3], at: 4)
        points.insert(points[0], at: 1)
        let rope = try world.addRope(through: points, at: Vector3(0, 2, 0))
    ...
    @Test func aRopeOfOnePointIsRefused() throws {
        let world = World3D()
        #expect(throws: PhysicsError.self) { try world.addRope(through: [Vector3(0, 1, 0)]) }
        #expect(world.softBodies.isEmpty)
    }

    @Test func aPinnedRopeHangsWhereAFreeOneFalls() throws {
        func lowestPoint(pinned: Bool) -> Double {
            let world = World3D()
            world.ground = nil
            let rope = try! world.addRope(through: Rope3DTests.hanging(20),
    ...
    @Test func pinningNamesThePointsTheClosureChose() throws {
        let world = World3D()
        let rope = try world.addRope(through: Rope3DTests.sideways(10),
                                       at: Vector3(0, 2, 0),
                                       pinned: { $0.x < 0.25 })
    ...
    @Test func aStifferRopeStretchesLess() throws {
    ...
    @Test func aBendResistantRopeHoldsItselfOutWhereALimpOneHangs() throws {
        func drooped(_ bend: Double) -> Double {
            let world = World3D()
            world.ground = nil
            let rope = try! world.addRope(through: Rope3DTests.sideways(20),
    ...
    @Test func bendMeansTheSameAtAnyWeight() throws {
    ...
    @Test func bendMeansTheSameAtAnyLength() throws {
        func drooped(span: Double) -> Double {
            let world = World3D()
            world.ground = nil
            let spacing = span / 19
            let rope = try! world.addRope(through: Rope3DTests.sideways(20, spacing: spacing),
                                           at: Vector3(0, 50, 0), radius: 0.01,
    ...
    @Test func maxStretchCapsAHangingRope() throws {
    ...
    @Test func iterationsSteadyALongFineRope() throws {
        func drooped(_ iterations: Int) -> Double {
            let world = World3D()
            world.ground = nil
            let spacing = 6.0 / 39
            let rope = try! world.addRope(through: Rope3DTests.sideways(40, spacing: spacing),
                                           at: Vector3(0, 50, 0), radius: 0.01,
    ...
    @Test func everySegmentPointsAlongTheRope() throws {
        let world = World3D()
        world.ground = nil
    ...
    @Test func aRopeBuiltTurnedCarriesTurnedFrames() throws {
        let world = World3D()
        world.ground = nil
        let flat = try world.addRope(through: Rope3DTests.sideways(8), at: Vector3(0, 3, 0),
                                     radius: 0.02, pinned: { _ in true })
        let turned = try world.addRope(through: Rope3DTests.sideways(8),
    ...
    @Test func aRopeDrapesOverACrateRatherThanThroughIt() throws {
        let world = World3D()
        world.ground = 0
        world.addBody(.box(width: 1, height: 1, depth: 1), at: Vector3(0, 0.5, 0),
                      kind: .static)
        let rope = try world.addRope(through: Rope3DTests.sideways(24),
                                       at: Vector3(-1.2, 2, 0),
                                       radius: 0.05, mass: 1)
        run(world, steps: 300)
    ...
    @Test func aRopePushesALightBodyOutOfItsWay() throws {
        func pebbleTravel(withRope: Bool) -> Double {
            ...
            let pebble = world.addBody(.sphere(radius: 0.1), at: Vector3(0, 0.1, 0),
                                       density: 0.1)
            if withRope {
                try! world.addRope(through: Rope3DTests.sideways(20), at: Vector3(-1, 1.5, 0),
                              radius: 0.05, mass: 2)
            }
            run(world, steps: 240)
    ...
    @Test func aRopeFloatsAtTheDensityItIsGiven() throws {
    ...
    @Test func aRopeIsInvisibleToQueries() throws {
        let world = World3D()
        world.ground = -5
        let rope = try world.addRope(through: Rope3DTests.sideways(20),
    ...
    @Test func identicalRopesReplayIdentically() throws {
        func trace() -> [Vector3] {
            let world = World3D()
            world.ground = 0
            let rope = try! world.addRope(through: Rope3DTests.sideways(16),
                                           at: Vector3(0, 2, 0), radius: 0.03,
