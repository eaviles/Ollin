...
struct JSONTests {
    ...
    @Test func aFlagIsNotTheNumberOne() throws {
        let json = try JSON(text: #"{"flag": true, "one": 1}"#)
        #expect(json["flag"] == .bool(true))
    ...
    @Test func aQuotedNumberIsANumber() throws {
        let json = try JSON(text: #"{"a": "42", "b": "forty-two"}"#)
    ...
    @Test func aWholeNumberRounds() throws {
        let json = try JSON(text: #"{"a": 2.6, "b": -2.6}"#)
        #expect(json["a"].int == 3)
        #expect(json["b"].int == -3)
    }
    ...
    @Test func aHexStringReadsAsAColor() throws {
        ...
        let json = try JSON(text: ##"{"tint": "#ff0000", "other": "red-ish"}"##)
    ...
    @Test func anIndexPastTheEndAnswersNull() throws {
        let json = try JSON(text: #"{"items": [1, 2]}"#)
    ...
    @Test func theDottedFormReadsTheSameValue() throws {
        let json = try JSON(text: #"{"city": {"name": "Oslo"}}"#)
    ...
    @Test func loopingOverWhatIsNotThereRunsZeroTimes() throws {
        let json = try JSON(text: #"{"items": [1, 2, 3]}"#)
    ...
    @Test func keysComeBackSorted() throws {
        let json = try JSON(text: #"{"c": 1, "a": 2, "b": 3}"#)
    ...
    @Test func anArrayCanBeTheWholeDocument() throws {
        let json = try JSON(text: #"[{"n": 1}, {"n": 2}]"#)
        #expect(json.count == 2)
    ...
    @Test func aBareValueCanBeTheWholeDocument() throws {
        #expect((try? JSON(text: "42"))?.number == 42)
        #expect((try? JSON(text: #""hello""#))?.text == "hello")
        #expect((try? JSON(text: "true"))?.bool == true)
    }
    ...
    @Test func thePathFormReadsAFile() throws {
        ...
        let json = try JSON(contentsOf: url.path)
