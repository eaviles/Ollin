...
private let snapshotMetalCases: [SnapshotCase] = [
    ...
    SnapshotCase("effects-catalog",
                 note: "A fixed scene through three filters plus a generator tile. Pins the broader filter catalog: the packed-param plumbing, a fragment color/tone pass (posterize), the gradient-map LUT upload + sample, a neighborhood pass (Sobel edges), and the input-less generator path.",
                 make: { EffectsCatalog() }),
