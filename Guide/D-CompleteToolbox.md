...
| Polyominoes | Pieces of joined squares, the twelve pentominoes, and a search that fits a bag of them into a region exactly or says it cannot | [Ch 7](07-Tiles.md) | [Polyominoes](../Docs/Generators/Polyominoes.md) |
| Power diagrams | Voronoi cells for sites that carry a weight, so circles of different sizes get territories that never cut through them | [Ch 15](15-ShapesAsMaterial.md) | [Voronoi and Delaunay](../Docs/Drawing/Voronoi.md) |
| Ford circles | A circle for every fraction that touches its neighbors and overlaps nothing, over the Farey sequence and an exact `Fraction` type | [Ch 6](06-GridsAndRepetition.md) | [Ford circles](../Docs/Generators/FordCircles.md) |
| Classic curves | Phyllotaxis, Lissajous figures, roses, superellipses, supershapes, the spirograph gears, guilloche rosettes, the harmonograph, spirolaterals, and Chaikin corner-cut smoothing, all deterministic and none of them touching randomness | [Ch 15](15-ShapesAsMaterial.md) | [Classic curves](../Docs/Drawing/Curves.md) |
| Anamorphosis | A plate that reads only in a mirrored cylinder standing on the page, from one viewing spot: the picture wrapped on the glass at true size, points, contours and shapes mapped to the page, the arc an eye can see of a cylinder, and the search that reads a plate back | [Ch 15](15-ShapesAsMaterial.md) | [Anamorphosis](../Docs/Drawing/Anamorphosis.md) |
| Clothoid | The curve whose bend grows at a steady rate (the Euler or Cornu spiral): easements into a turn, the single curve fitting two points and two headings, smooth paths through waypoints, polyline corners a vehicle could take, and chains driven by distance | [Ch 15](15-ShapesAsMaterial.md) | [Clothoid](../Docs/Drawing/Clothoid.md) |
...
| Room | Several machines drawing one piece: they find each other by a name with no server, share values and `@Param` knobs, agree on one clock so motion stays in step, and take a seat each across a wall | [Ch 32](32-Installations.md) | [Room](../Docs/Integration/Room.md) |
...
| Knobs driven by a formula | `drive($knob, "190 + sin(time) * 80")` reads a knob's rule from text rather than from Swift source, so it can arrive from a file or a typed field, and one knob can be worked out from another | [Ch 31](31-SharingAndPerforming.md) | [Formula](../Docs/Helpers/Formula.md) |
...
| Parameter timeline | OllinLive's panel: scrub, turn a knob, click its diamond to place a key; lanes, curves shaped by eye, and the automation file every export reads | [Ch 31](31-SharingAndPerforming.md) | [Timeline](../Docs/Tools/Timeline.md) |
...
| A sketch as an app | `ollin new --kind mac-app` wraps a finished piece as a signed, double-clickable `.app` for a Mac without the toolchain, wearing a frame of itself as its icon | [Ch 32](32-Installations.md) | [A sketch as an app](../Docs/Output/App.md) |
