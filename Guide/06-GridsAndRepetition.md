...
- [Clipping](../Docs/Drawing/Drawing.md#clip): `withClip(shape) { }` confines everything drawn inside the block to a region, so a pattern can fill a star, a letter, or any vector shape without computing the intersection yourself; nested clips intersect. The [`Shapes/Clipping`](../Examples/Shapes/Clipping/Sketch.swift) example sweeps a lens across a striped star.
