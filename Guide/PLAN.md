...
| `3D/Scenes.md` (loadScene, drawScene, named nodes, authored cameras/lights) | Ch 17 | shown ("A mesh from a file" teaches the loadScene shape: the file's camera and lights as ordinary values, `scene["name"]` driving one node; worked example `3D/Geometry/LoadedScene`) |
...
| 3D: scene animation, live camera environment, 3D rigid bodies | Ch 17 (bodies also Ch 9) |
