...
| Area lights: `rectLight`/`diskLight`/`tubeLight` (`3D/3D.md`) | Ch 17 | shown (the light-kinds section introduces the family, the radiance/falloff rule, and the shape-reflection idea; the `3D/Lighting/AreaLights` example is the worked demo; `taught` with a figure is the goal when Ch 17 next gets a pass) |
