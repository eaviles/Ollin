...
| `Generators/SurfaceReconstruction.md` (`reconstructSurface`, `particleSurface`, points back to a mesh) | Ch 19 | taught ("From the cloud to a surface" with the staged corner sweep rebuilt solid: the plane fit, the camera path as orientation, the torn-rim honesty rule and its sweep-coverage reading, the ordinary-`Mesh` payoff, and the `particleSurface` sibling pointed at the reference page and the side-by-side example) |
