...
| Temporal anti-aliasing: `temporalAntialiasing` (`3D/3D.md#temporal-antialiasing`) | Ch 18 | shown ("Edges that settle": the render-different-estimates-and-average trick named as what reflections and probes already do, applied to every 3D edge; fixed-position MSAA's staircase versus the jitter average, camera-following history, the untouched 2D path, the average-within-the-frame export rule, the any-GPU note, and the TemporalAA example's mid-motion toggle; a figure is hard to earn honestly since the win is temporal, a still shows only the staircase melting, so the example carries the demonstration) |
