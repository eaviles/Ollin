...
| `Drawing/Attractors.md` (strange attractors, chaotic maps, `AttractorFlow`) | Ch 12, Ch 17 | taught (the 2D maps as density plates in Ch 12; the GPU tier in Ch 17's "A million riding the same field" with a one-orbit-versus-a-million figure: a velocity field read as which-way-is-this-point-moving, the four-line sketch, why it names no per-system numbers because `center`/`extent`/pace are measured from a CPU orbit at build, color-is-speed-brightness-is-crowd and why the ramp holds its brightness level, and the hair of jitter that turns one trajectory into a million) |
