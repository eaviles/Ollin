...
| `3D/Scenes.md` (loadScene, drawScene, named nodes, authored cameras/lights incl. USD's UsdLux kinds, authored animations via `apply(_:at:)` from glTF and USD timeSamples, skins and morph targets posed by drawScene, the USD structure read) | Ch 17 | shown ("A mesh from a file" teaches the loadScene shape: the file's camera and lights as ordinary values, `scene["name"]` driving one node, `apply(_:at:)` playing an authored animation on the sketch clock, the deforming tier (skins, morph `weights` as a hand-drivable node property), the USD leg in further reading; worked examples `3D/Geometry/LoadedScene` + `AnimatedScene` + `SkinnedScene` + `USDScene` + `USDAnimatedScene`) |
