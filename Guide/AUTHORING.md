...
- **One aside per paragraph, never nested.** The parenthesis is this repo's favorite tool and its most tiring one: a good sentence with two more sentences pushed inside it. Keep the aside that carries the reader, promote the next one to its own sentence, and delete the third. Most of them go without loss, and the sentence they were hiding in gets easier to read.
...
- **Don't claim what the page proves.** "A comprehensive framework" and "a powerful core" ask the reader to believe something they cannot check, and the evidence is usually sitting right below the sentence. Cut the adjective and let the list, the count, or the table make the case. Three habits belong to this rule. The intensifier that adds nothing ("a real motion layer", "the actual reason") comes out. The contrast against a strawman ("built for finishing work rather than demoing it") wins against nobody, so state what the thing does. And if no evidence follows the claim, the sentence is an opinion rather than a description, which is the deeper problem.
- **A heading gets the same bar as a sentence.** "A Metal core with no ceiling" and "Motion is the default, and it's handled" promise more than they say. Name the thing instead: "A Metal core you can extend", "Motion is the default". The bullet under it does the convincing.
- **A list after a colon reads as the complete set.** "The techniques are built in: Voronoi, L-systems, GPU fluids" tells the reader that those three are all there is. When the list is a sample, say so: mark the span ("from Voronoi to on-device computer vision"), or end with a link to the full catalog. This matters most on a front page, where a short list stands in for a large body of work.
- **Don't restate the line above.** A paragraph under a heading or a tagline should extend it, not reopen it. When the first sentence repeats the nouns of the line before it, the reader reads the same claim twice and learns nothing the second time. Cut to what comes next.
- **Prefer the plain verb to the idiom.** "The circle rides the pointer" and "the pointer goes" are figures of speech where a plain verb is shorter and clearer: follows, is hidden. This is the plain-words rule applied to verbs, and it matters for readers whose English is a second language, who can know every word in an idiom and still miss it.
...
When a chapter does split, split it at a seam the prose already has, and give each half the whole anatomy. A new chapter is not a slice. It needs its own hook, its own finished piece, its own credits, and its own "Go deeper", and the parent's credits paragraph for whatever left travels with it. Renumbering is `Scripts/guide-renumber.sh`, and `Scripts/check-links.sh` is what proves the move landed.

## Figures

Figure sketches live in `Figures/<NN-ChapterName>/<FigureName>.swift`, rendered to `Images/<NN-ChapterName>/<FigureName>.jpg` (or `.png`/`.gif`) by the runner. The reference pages reuse these images by relative path, and figures serving *only* a `Docs/` page live in the reference pages' own tree, `Docs/Figures/` (rendered to `Docs/Images/`, keyed to the runner as `Docs/<Name>.swift`), under the same conventions; `Scripts/check-links.sh` checks the `Docs/` references alongside the chapters'. One standing rule rides on that: a `Docs/` section documenting a `draw*` call carries a catalog figure of what the call draws (the `Docs/Figures/Draw<Name>.swift` set, modeled on `DrawArc.swift`: anchors dotted, the parameter that matters varied across a few panels), so a new drawing primitive ships with its figure. Conventions:

- A figure file is an ordinary Ollin sketch (a `class ... : Sketch`). **Listings and payoff figures are self-contained, with no dependencies beyond the framework**: readers copy their source, so nothing in them may lean on book tooling. **Diagrams and the `Docs/` catalog figures may also import `OllinDiagram`**, the book's own support library (the shared `DiagramTheme` plus `diagramFrame`/`diagramCaption`); their source is illustration, not something a reader retypes. New diagrams use the shared helpers; an older diagram carrying its own private `frame`/caption copy migrates when the figure is next touched, with its renders eyeballed, since the private copies leaked drawing state the scoped helpers do not.
- **Pin the seed in any figure that touches `random` or `noise`.** A sketch's `variation` is rolled fresh at launch, so an unseeded figure re-renders differently every run and shows up as churn in `git status` after each full gate. Call `seed(_:)` (or `noiseSeed(_:)`) in the figure. The check is to render it twice with `--only <name> --force`. The runner replaces a committed image only when the decoded pixels changed visibly: encoder bytes do not count, and neither does a move below the visible threshold, which the run lists as held. So a stable figure leaves `git status` clean and prints nothing about itself, while a dirty status or a line in the held list means the render itself moved. A figure that genuinely cannot reproduce carries `// figure: unstable` (below); never use that mark as a way out of pinning a seed.
...
- **A diagram figure renders a dark sibling for dark-mode readers.** Add `themed` to the directive (`// figure: frame=0 themed`), declare `@Param var darkTheme = false`, and key the palette on that parameter; the runner flips it and renders the same instance twice, writing `<Name>.jpg` beside `<Name>-dark.jpg`. One source, two renders: never fork a figure per theme. The palette is **`DiagramTheme`** in `OllinDiagram`, never hexes in the figure: declare `var theme: DiagramTheme { DiagramTheme(dark: darkTheme) }` and take `paper` / `ink` / `muted` / `border` / `card` / `accent` from it (washes and faint guides ride `theme.ink(alpha)` with the figure's own alpha). The values match the pages the figures are read on, light and dark, so a figure sits on the rendered page as part of it; that is also why no figure may carry its own copy of these colors, which is how the old per-figure hexes drifted. Colors that *depict content* (a source picture, a page being thresholded, a sketch's own artwork) stay put in both themes; only the diagram's paper, ink, and annotations flip. Two rules ride the double render: the figure must draw the same under a second `setup()` pass on the same instance (assign state in `setup`, never append), and a pinned seed must be re-pinned there so both variants agree. The `Docs/Figures/` diagram set is themed; a chapter diagram can adopt the same parameter, while payoff pieces (artwork) and a figure that is *all* depicted content (a canvas mock with no paper of its own) stay single-render. A themed figure embeds as a `<picture>` tag, the light `<img>` unchanged inside it:

  ```html
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="../Images/DrawArc-dark.jpg">
    <img src="../Images/DrawArc.jpg" alt="..." width="680">
  </picture>
  ```

  GitHub serves the `srcset` to dark-mode readers and the `img` to everyone else; `Scripts/check-links.sh` checks `srcset` paths beside `src` ones, and a dark sibling counts as referenced only through one, so an embed left as a plain `<img>` after theming its figure fails the orphan rule rather than quietly serving light-only.
...
- **A figure that analyzes a still with `waitFor` must not read the sketch's own properties inside the closure.** `waitFor` parks the main thread, and a sketch's properties (static ones too) are main-actor isolated, so a read from inside the closure waits on the parked thread and the figure hangs forever with nothing printed. Read the values into locals first. Chapter 30's `AttentionAndLabels` hit this and looked exactly like a Neural Engine deadlock.
- **A figure whose render genuinely cannot reproduce carries `// figure: unstable`.** The runner then verifies it (it still has to compile and render) without rewriting its committed image, so it stops showing up as a change. Three causes have earned the mark so far: GPU sims and culls whose accumulation order is decided by atomics (the particle sims, the caustic light, the culled landscapes), figures driven by system ML models Ollin does not control (a Vision fit, the speech and sound labels), and a few whose nondeterminism is measured but not yet diagnosed, each saying so in its own header. The directive is the source of truth for the current set; list it with `grep -rl "figure:.*unstable" Guide/Figures Docs/Figures` rather than trusting any written-out list, which has gone stale here four times. Use the mark only when the framework itself makes no reproducibility promise, or when two lossless back-to-back renders prove the pixels move; say which in the figure's header.

- **The probe sample is chosen for coverage, not for size.** The figures carrying `// figure: probe` include at least one per chapter, and at least two witnesses for each drawing path (SDF primitives, fringe strokes, triangulated fills, glyphs, images, shaders, layers and filters, simulation fields, 3D meshes, lights, shadows, raymarched fields, point clouds, instanced fields, vertical text, a texture small enough on screen to read its smaller copies, and the scene-behind layer read through a solid glass body). Prefer the cheapest honest witness, since the whole sample has to stay affordable: it costs about a minute of render time in total. **A figure marked `unstable` can never be a probe**, because its render is compared to nothing. When a new drawing path lands, mark a figure that exercises it, or the probe will pass a change it never looked at. The sabotage that proved this works moved 3 figures across the whole suite, and the sample held one of them.
...
Run the whole suite, not `--only`. It costs minutes cold (the runner prints where the time goes and which figures dominate), under a second when nothing changed, and about fifteen seconds after a framework change that moves no pixels, because of four things worth knowing about:
...
- **A framework change renders the probe sample first.** Most edits to `Sources/` move no pixels at all: a new function, a comment, a type nothing draws through. The figures marked `// figure: probe` render first and are compared to what the framework drew for them last time: each probe figure's last render is kept beside the cache (`Guide/.figure-baselines/`, gitignored), and the committed image stands in until there is one. All identical means the change was render-neutral, so the rest keep their cache entries, and the run goes on to whatever is stale on its own account (a new figure, an edited one) before it ends. Any probe that moved, or failed to render, hands the run back to the full re-render; a probe figure whose own source changed sits the sample out, since its render would move for its own reasons. `--no-probe` skips the sample. The baseline is the last render rather than the committed image because the two drift apart on purpose: a move below the visible threshold keeps the committed image, and once one probe figure sat a few invisible pixels off its image, every later framework change fell through to the full render, however neutral (2026-09-08: a Vision figure at a mean of 0.00 levels, and two half-hour renders killed for memory that day).
- **The work is split across worker processes.** Sharding rather than threads, because the expensive figures spend their time in `setup()` on the main actor, and only separate processes run those at once. The count follows the machine: at most four, never more than the cores less two, and one per 3 GB of memory, since each worker compiles its figures through `swiftc` beside its own Metal render and four of them on 8 GB put the machine into swap. `OLLIN_FIGURE_JOBS` overrides it for one run, and the runner says how long a full pass will take before it starts, from the last timings.
- **"Changed" means the pixels, never the bytes, and a move the framework made on its own has to be visible.** The JPEG and PNG encoders are not byte-deterministic, so a fresh render lands in a dot-temp and replaces the committed image only when the decoded pixels differ. A framework change that shifts anti-aliasing by a level or two moves pixels in most figures, and no reader can tell. Rewriting the JPEGs for that is what filled the repository's history. So such a move is rewritten only past a visible threshold: at least 0.2% of the pixels moved by more than 32 levels (a label, a shape, a highlight), or a mean difference of 4 levels or more per channel (a tone or palette shift). That mean is twice what the snapshot suite tolerates. Below that the committed image stays, and the run lists what it held. A figure whose own source changed is recorded on any move, since its author meant the new picture. `--exact` treats every figure that way, and `--compare <a> <b>` measures two images against the threshold. The numbers were calibrated against every figure rewrite in the history and live on `Movement` in the runner. A clean `git status` after a full run is the expected result, and churn under `Guide/Images` means something visibly moved.
