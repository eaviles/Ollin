...
Figure sketches live in `Figures/<NN-ChapterName>/<FigureName>.swift`, rendered to `Images/<NN-ChapterName>/<FigureName>.jpg` (or `.png`/`.gif`) by the runner. The reference pages reuse these images by relative path, and figures serving *only* a `Docs/` page live in the reference pages' own tree, `Docs/Figures/` (rendered to `Docs/Images/`, keyed to the runner as `Docs/<Name>.swift`), under the same conventions; `Scripts/guide-links.sh` checks the `Docs/` references alongside the chapters'. One standing rule rides on that: a `Docs/` section documenting a `draw*` call carries a catalog figure of what the call draws (the `Docs/Figures/Draw<Name>.swift` set, modeled on `DrawArc.swift`: anchors dotted, the parameter that matters varied across a few panels), so a new drawing primitive ships with its figure. Conventions:
...
- **A diagram figure renders a dark sibling for dark-mode readers.** Add `themed` to the directive (`// figure: frame=0 themed`), declare `@Param var darkTheme = false`, and key the palette on that knob; the runner flips it and renders the same instance twice, writing `<Name>.jpg` beside `<Name>-dark.jpg`. One source, two renders: never fork a figure per theme. The palette is **`DiagramTheme`** in `OllinDiagram`, never hexes in the figure: declare `var theme: DiagramTheme { DiagramTheme(dark: darkTheme) }` and take `paper` / `ink` / `muted` / `border` / `card` / `accent` from it (washes and faint guides ride `theme.ink(alpha)` with the figure's own alpha). The values match the pages the figures are read on, light and dark, so a figure sits on the rendered page as part of it; that is also why no figure may carry its own copy of these colors, which is how the old per-figure hexes drifted. Colors that *depict content* (a source picture, a page being thresholded, a sketch's own artwork) stay put in both themes; only the diagram's paper, ink, and annotations flip. Two rules ride the double render: the figure must draw the same under a second `setup()` pass on the same instance (assign state in `setup`, never append), and a pinned seed must be re-pinned there so both variants agree. The `Docs/Figures/` diagram set is themed; a chapter diagram can adopt the same knob, while payoff pieces (artwork) and a figure that is *all* depicted content (a canvas mock with no paper of its own) stay single-render. A themed figure embeds as a `<picture>` tag, the light `<img>` unchanged inside it:
  ...
  GitHub serves the `srcset` to dark-mode readers and the `img` to everyone else; `Scripts/guide-links.sh` checks `srcset` paths beside `src` ones, and a dark sibling counts as referenced only through one, so an embed left as a plain `<img>` after theming its figure fails the orphan rule rather than quietly serving light-only.
...
Run the whole suite, not `--only`. It costs about ten minutes cold, under a second when nothing changed, and about fifteen seconds after a framework change that moves no pixels, because of three things worth knowing about:
