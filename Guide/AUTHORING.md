...
- **A figure whose render genuinely cannot reproduce carries `// figure: unstable`.** Only three do: the GPU sims in `16-Simulations/ArtificialLife` and `FluidAndBlobs`, whose particle scatter order is decided by GPU atomics, and `21-Seeing/Trajectory`, whose Vision-model parabola fit drifts in its low-order bits across model/OS updates (it renders byte-identically within a session, so the drift only ever shows up as churn after an environment change). The runner then verifies them (they still have to compile and render) without rewriting their committed images, so they stop showing up as changes. This is not a way out of pinning a seed; use it only when the framework itself makes no reproducibility promise: a GPU race, or a system ML model Ollin doesn't control.
