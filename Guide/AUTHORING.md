...
- **A framework change renders the probe sample first.** Most edits to `Sources/` move no pixels at all: a new function, a comment, a type nothing draws through. The figures marked `// figure: probe` render first and are compared to the images committed beside them. All identical means the change was render-neutral, so the rest keep their cache entries and the run ends in about fifteen seconds. Any probe that moved, or failed to render, hands the run back to the full re-render. `--no-probe` skips the sample.
- **The work is split across up to four worker processes.** Sharding rather than threads, because the expensive figures spend their time in `setup()` on the main actor, and only separate processes run those at once.
