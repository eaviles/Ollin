...
- **"Changed" means the pixels, never the bytes.** The JPEG and PNG encoders are not byte-deterministic, so a fresh render lands in a dot-temp and replaces the committed image only when the decoded pixels differ. A clean `git status` after a full run is the expected result, and churn under `Guide/Images` means something real moved.
