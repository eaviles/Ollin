...
- **Cameras and lights resolve at load** into flat world-space arrays;
  animating a node later moves geometry only. Keeping them on their nodes
  becomes worth it when scene animation lands (the design note).
...
The bundled demo asset (`Examples/3D/Geometry/LoadedScene/scene.gltf`) is
generated by `Scripts/make-sample-scene.swift` (the `make-textured-cube.swift`
precedent): geometry, camera, and lights all authored in-repo, so the asset
