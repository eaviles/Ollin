...
**Drawing.** `Drawer.drawScene` is a ~15-line walk: save `modelMatrix`,
post-multiply the node's local transform, `drawMesh` the node's mesh, recurse,
restore. Reusing `drawMesh` wholesale is the point: materials, lights, shadows,
reflections, wireframe, batch/export/combine gating all apply with zero new
rules, and `SceneLoaderTests.drawSceneMatchesManualTransforms` pins that a
drawn scene is byte-identical to the equivalent manual transform-stack calls.
...
holding up to three `Sampler`s (translation/rotation/scale; morph `weights`
channels are skipped, there are no morph targets to drive). A sampler stores
...
The bundled demo assets (`Examples/3D/Geometry/LoadedScene/scene.gltf` and the
animated `Examples/3D/Geometry/AnimatedScene/scene.gltf`) are generated by
`Scripts/make-sample-scene.swift` / `make-animated-scene.swift` (the
`make-textured-cube.swift` precedent): geometry, camera, lights, and keyframes
all authored in-repo, so the assets carry no third-party license.
