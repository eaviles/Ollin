...
`Examples/3D/Geometry/LoadedScene/stage.usda` / `AnimatedScene/stage.usda` /
`SkinnedScene/stage.usda`) are generated by
...
`PhysicsSnapshotTests` (23) pins it; example `3D/Physics/Yard`.
...
`Shapes/Hulls`). Both in `Docs/Drawing/Geometry.md`.
...
`Patterns/ChaoticMaps`; snapshots `strange-attractor-3d` +
`clifford-attractor`.
...
`OllinApp.exportGIF` used to hand every frame to `CGImageDestination`, which keeps what it is given and lays the file down at finalize: the peak was ten bytes a pixel a frame (measured within 4% across two sizes and four lengths), 1.6 GB for a 150-frame 1080 square loop, and the export printed that figure past an eighth of the machine. `GIFWriter` (`Sources/Ollin/Export/GIFWriter.swift`) writes the format itself, a frame at a time into an open file handle. Its buffers are sized once at init (the drawn frame, two key planes, the index plane, the histogram, the lookup, the coder's dictionary), and nothing is kept between frames but the previous frame's keys.

**What a frame goes through.** The `CGImage` is drawn into the writer's own sRGB 8-bit context at the writer's size, which is also how `--gif-width` is applied (the export has no scaling path of its own) and how a Display P3 or float frame becomes sRGB bytes. Each pixel becomes a key, `0x00RRGGBB` with the premultiplication undone, or a clear key under half coverage. The keys are diffed against the previous frame's to find the changed box; unchanged pixels inside it take the transparent index and the frame is written at that box with disposal 1, a frame with no change as one transparent pixel that keeps its delay. A see-through picture (the first frame carries alpha and has a clear pixel) is written whole with disposal 2 every frame, since a pixel that goes from opaque to clear cannot be expressed by leaving it alone; the choice is made once, from the first frame. The system writer diffs after quantizing, so its crops run a pixel tighter where a source pixel moved a level and its index did not; that refinement is open.

**The table.** Colors are binned at six bits a channel (262,144 bins) with exact sums under each bin, and a median cut splits the box with the most pixels over the widest volume along its widest axis at the median pixel until 255 boxes, skipping a box whose bins sit within three levels RMS of its mean (an antialiased ramp does not need an entry per shade). Each box lands on the exact mean of its pixels, and entries within six levels of each other are then merged: a median split parts a dominant bin from its dithered neighbors first and isolates a neighbor later, which the split floor cannot see, and the present pass dithers, so before the merge the garden figure's flat sky flipped between two entries a level apart and its first frame cost 9157 bytes against the system's 6696; after it, one sky color and 4952. The table is sized to the smallest power of two above its colors plus the transparent entry, so a picture of under 64 colors codes in six bits, and the transparent index is the last entry. Mapping a pixel goes through a lookup indexed by its bin, filled on demand with the nearest entry to the bin center, so a table costs at most one nearest search per populated bin.

`.shared` chooses the table from the first frame, writes it as the global table, and measures its training distance (RMS over the frame). Every later frame's mapped pixels are read two ways. Their squared distance spread over the whole frame's pixel count against `max(8, 2 × training)` catches a whole picture drifting off its table: a fade from black reads 18 to 23 there. Their RMS over the moved pixels alone, when at least eight moved, against `max(12, 3 × training)` catches a thin thing of a new color that arrives late and stays: a pale tree growing eighteen pixels a frame over dark soil is almost no energy over the frame and the whole picture where it is, and without this trigger the Guide's garden drew its cream tree green. The strip a moving disc uncovers, colors that were under it when the table was chosen and that no streaming table can see, reads 10 to 15 over its pixels against a training of 6.5 and 2.5 over the frame, under both bars, which is the case the bars are set by. When a frame has left its table, its colors farther than eight levels from every entry join the global table: into its free entries by a seek and a write, never touching an entry a written frame points at; when there is no room, by growing the table to the next size that holds them, one streamed copy of the file so far behind a wider head (a written frame keeps its narrower codes and its own transparent index, both valid under the wider table), at most once per doubling; and only past the format's 255 colors does the frame, and every frame after it, carry a local table. `.perFrame` chooses per frame, the first as the global table and every later one local.

**The coder** is the format's LZW: codes from the table's width plus one up to twelve bits, a clear code emitted when the dictionary fills (a deferred clear is not used), codes packed least significant bit first into 255-byte sub-blocks, the dictionary an open-addressing hash of 8192 slots keyed by prefix and byte that never passes half full. The code width steps when the next free code reaches the current width, one step later than a decoder that adds an entry per code read, and the end-of-information code honors the same lag. It is byte-identical to ImageIO's on the same index image: the system writer's own decoded first frame re-coded through it lands on the system's byte count.

**Measured** (`GIFWriterTests`, `GIFMemoryTests`, 2026-09-17), against the system writer on the same frames: on a flat ground with antialiased rings and a crossing disc the whole file is 12% smaller, 5500 against 6226 bytes, with a 64-entry table; on a two-axis gradient with a crossing disc, every pixel its own color, the first frame is the system's plus 5% at a lower error (2.56 against 2.63 levels mean absolute, since the two tilings differ) and every sub-frame lands on the same rectangle within a few bytes; every frame of every probe decodes within 1.25 times the system's error. On the Guide's garden figure (83 frames at 420 square, a cream tree and lichen arriving over dark soil) the file is 63.7 KB against the system's 67.0, with no local table and no pixel more than 32 levels from the system's. A 480² writer holds the same bytes after one frame and after sixty, the process grows under 40 MB over 190 more frames where holding them would add 175, and a real export's sampled peak at 120 frames is within 40 MB of the same sketch at 20.
