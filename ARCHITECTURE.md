...
animated `Examples/3D/Geometry/AnimatedScene/scene.gltf`, and the deforming
`Examples/3D/Geometry/SkinnedScene/scene.gltf`) are generated by
`Scripts/make-sample-scene.swift` / `make-animated-scene.swift` /
`make-skinned-scene.swift` (the `make-textured-cube.swift` precedent):
geometry, camera, lights, keyframes, skins, and morph targets all authored
in-repo, so the assets carry no third-party license.
