...
family (`.usdz`/`.usdc`/`.usda`/`.usd`) through a Model I/O hierarchy walk:
each `MDLObject` becomes a `SceneNode` (name, `transform?.matrix ?? identity`
as the local transform, children recursed), an `MDLMesh` object converts
through `Mesh.readMDLMesh` (the per-mesh reader factored out of
`loadViaModelIO`, whose merged output is byte-identical after the refactor;
`MeshLoaderTests` pin it), and cameras collect on a second walk composing
world transforms, resolved through the same pose logic as glTF
(`Scene.resolveCamera(projection:near:far:world:sceneCenter:)`, the extracted
shared tail: -z view axis, up column, bounds-center target). Probed importer
facts the design rests on: node-local mesh vertices (transforms are *not*
baked), quads arrive pre-triangulated, children come back alphabetized rather
than in authored order, `MDLCamera.fieldOfView` is the vertical angle in
degrees, a USD orthographic aperture is in tenths of a world unit (height =
`sensorVerticalAperture / 10`), and `property(with: .baseColor)` prefers an
authored preview-surface `diffuseColor` over the importer's gray default.
The hard boundary: **USD lights do not survive the import** (light prims
arrive as bare grouping nodes with even their transforms dropped), and USD
animation/skinning aren't read either, so a USD `Scene` is structure + meshes
+ cameras with `lights` empty (documented, and pinned by
`usdSceneCarriesNoLights`, which doubles as the tripwire if the importer ever
learns them). `SceneLoaderTests` pin the walk over an inline `.usda` fixture;
the `usd-scene` snapshot pins the whole pipeline over the bundled court.
...
`Examples/3D/Geometry/SkinnedScene/scene.gltf`, and the USD
`Examples/3D/Geometry/USDScene/stage.usda`) are generated by
`Scripts/make-sample-scene.swift` / `make-animated-scene.swift` /
`make-skinned-scene.swift` / `make-usd-scene.swift` (the
