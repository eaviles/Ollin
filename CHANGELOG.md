...
- **A sketch as a web page.** `--export-web` records what a sketch draws over a duration and writes a page that plays it back in a browser, one self-contained file or, with `--inline`, a fragment for a page of your own, the shapes drawn by the framework's own shader carried from Metal to GLSL. A sketch that declares `loopDuration` records one lap and the page wraps it without a seam. The analytic shapes cross; anything else stops the export and names the call. [A sketch as a web page](Docs/Output/Web.md).
